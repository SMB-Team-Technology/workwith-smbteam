...
OUTPUT_PATH = "mcadoo-estate-planning/McAdoo Estate Planning, LLC_July 9, 2026_Sales_Companion.pdf"
...
story.append(Paragraph("Sales Companion  |  July 9, 2026  |  Rep: Dan Bryant", S["subtitle"]))
...
snap = [
    ...
    [Paragraph("Kim McAdoo (attorney) / Scott Holt (ops)", S["snap_value"]),
     Paragraph("$3M+ (HubSpot est., not stated on call)", S["snap_value"]),
     Paragraph("~4-6", S["snap_value"]),
...
story.append(b("<b>NAP inconsistencies.</b> Avvo, Justia, and Yelp list three different addresses/phone numbers."))
...
story.append(b("Revenue is $3M+ (HubSpot-sourced estimate, not stated on the call) — qualifies for Platinum."))
...
story.append(b("Team size was not explicitly confirmed on the call — confirm actual headcount live; a larger confirmed team could support a Master's Circle upgrade."))
story.append(b("Revenue is $3M+, but without a confirmed 5+ dedicated staff count, eligibility rules keep this at Elite Coach Plus rather than Master's Circle."))
...
story.append(Paragraph("Grow My Ads is a generalist vendor with no confirmed cost-per-case tracking. Platinum replaces that spend with legal-specific management, NAP cleanup, and geo-pages competitors like Morgan Legal Group and Roman Aminov already have.", S["objection_a"]))

story.append(Paragraph('"Our reviews already beat the competition — why do we need SEO work?"', S["objection_q"]))
story.append(Paragraph("True — 4.9 stars and 273 reviews beat every named competitor — but three different addresses and phone numbers across Avvo, Justia, and Yelp are undermining those trust signals in local rankings. NAP cleanup protects the reputation already built.", S["objection_a"]))
...
story.append(Paragraph('"We don\'t know our exact revenue or case value — how can we trust these ROI numbers?"', S["objection_q"]))
story.append(Paragraph("All estimates use disclosed Estate Planning practice-area defaults. Establishing cost-per-case tracking in the first 90 days is exactly what turns these projections into confirmed numbers going forward.", S["objection_a"]))
