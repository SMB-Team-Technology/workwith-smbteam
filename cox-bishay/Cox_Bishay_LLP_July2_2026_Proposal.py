"""
Audit PowerPoint — Cox & Bishay, LLP
July 2, 2026 | Rep: Randy Gold
Generated from completed Growth Audit sections.
"""
...
# ═══════════════════════════════════════════════════════════════════
# FILLED VALUES — all extracted from completed audit sections
# ═══════════════════════════════════════════════════════════════════

FIRM_NAME   = "Cox & Bishay, LLP"
SALES_REP   = "Randy Gold"
OUTPUT_PATH = "cox-bishay/Cox_Bishay_LLP_July2_2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL",  "No digital lead gen"),
    ("RED",   "CRITICAL",  "No form / after-hours"),
    ("AMBER", "IN PROGRESS", "2 partners, 1 paralegal"),
    ("AMBER", "IN PROGRESS", "No financial baseline"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

FINDINGS = [
    ("neg", "Not in local 3-pack for any search — every Chester County prospect finds a competitor."),
    ("neg", "3 active NAP errors (Yelp, Lawful.com, Martindale) suppress local search authority."),
    ("neg", "No contact form or after-hours path — the website cannot convert visitors into leads."),
    ("pos", "35 years of credentials: Super Lawyers, Avvo 9.6, Best of the Main Line — strong foundation."),
]

COMPETITORS = [
    ("Ciccarelli Law Offices", "618 reviews", "Family law pages · multi-channel"),
    ("McCallin Law, LLC",      "4.5+ rated",  "32 yrs family law · local SEO"),
    ("Petrelli Previtera",     "13 reviews",  "W. Chester landing pages"),
]
CLIENT_REVIEWS      = "~9 reviews (Avvo)"
CLIENT_REVIEWS_NOTE = "Google count unconfirmed"
...
SMB_MODEL_DESC = (
    ...
    "When all four pillars are in place, Cox & Bishay can attract the right clients "
    "and grow the firm without growing their personal workload."
)

GOAL_HEADLINE = "$500K → $1M revenue"
GOAL_DBM      = "Grow on your terms — right clients, not just more cases"

PRIORITIES = [
    (
        "Build the", "Marketing Engine", "1D4ED8",
        [
            "Rebuild site with forms + Chester County pages",
            "Launch Google Ads for divorce + custody searches",
            "Get Google Screened with LSA — top of page",
            "Fix 3 NAP errors to unlock local 3-pack rankings",
            "Run Meta ads to reach families before they search",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Add contact form to every page — 24/7 inquiry path",
            "After-hours auto-response — no lead goes cold",
            "Create intake script for first contact calls",
            "Track every lead from first call to signed",
            "Speed up follow-up — call back within 1 hour",
        ],
    ),
    (
        ...
        [
            ...
            "Define clear roles for partners + paralegal",
            "Build KPI dashboard: leads, closes, revenue",
            "Track monthly revenue and profit by case type",
            "Build a profit plan — know what you keep, not just earn",
        ],
    ),
]
...
PACKAGES = [
    ...
    (
        "ELITE COACH PLUS",
        "$3,200", "$3,497/mo",
        "Weekly coaching · Masterminds · Workshops · Systems",
        "6D28D9",
    ),
]
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $3,500–$8,000/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE      = "$10,000"
CONSERVATIVE_LABEL  = "Conservative  (3–4 cases/mo):"
CONSERVATIVE_RESULT = "~$35,000 revenue · ~10× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (10–11 cases/mo):"
AGGRESSIVE_RESULT   = "~$110,000 revenue · ~14× ROAS"

TIMELINE = [
    ("Day 1",    "Coaching kickoff + intake audit launched"),
    ("Day 14",   "Google Ads + LSA campaigns go live"),
    ("Week 3",   "NAP cleanup — Yelp, Lawful.com, Martindale fixed"),
    ("Month 1",  "Website rebuild begins; review gen deployed"),
    ("Month 3",  "New website live; local 3-pack eligibility improving"),
]

CLOSING_QUOTE = (
    '"The right families in Chester County are searching right now. '
    'The firm they find should be the one with 35 years behind it."'
)
