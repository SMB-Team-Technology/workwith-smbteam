...
# Report metadata
FIRM_NAME      = "Calderaro Law Group"
SALES_REP      = "Randy Gold"
OUTPUT_PATH    = "calderaro-law-group/Calderaro Law Group_August 11, 2026_Proposal.pptx"
...
PILLARS = [
    ...
    ("AMBER", "AMBER",    "20% close, no system"),
    ("AMBER", "AMBER",    "30 case slots open"),
...
FINDINGS = [
    ("neg", "100% of new business comes from referrals alone"),
    ("neg", "A 20% close rate leaves 30 of 120 case slots empty"),
    ("neg", "No profit tracking exists behind the $3.6M goal"),
    ("pos", "90-95% case approval rate once a client signs"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Davies & Associates",       "~79 reviews", "4.9★ Trustindex"),
    ("Kurzban Tetzeli & Pratt",   "115 reviews", "4.7★ AV Preeminent"),
    ("Rothrock Immigration",      "29 reviews",  "5.0★ Top Rated"),
]
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $3.6M by 2027"

SMB_MODEL_DESC = (
    "All four pillars must work together. Missing any one means growth "
    "stalls regardless of ad spend. When all four work, Renata can finally "
    "stop being the firm's only source of new business."
)

GOAL_HEADLINE = "$1M → $3.6M revenue"
GOAL_DBM      = "A firm that grows without waiting on referrals"
...
PRIORITIES = [
    (
        ...
        [
            "Launch Google PPC for E2, EB5, L, O visa terms",
            "Activate Local Service Ads in target markets",
            "Add a Spanish-language ad campaign track",
            "Fix NAP inconsistency across directories",
            "Stop waiting on the next referral to arrive",
        ],
    ),
    (
        ...
        [
            ...
            "Raise close rate from 20% toward the 30% goal",
            "Add an attorney bio to build buyer trust",
            "Track every lead through to a signed case",
            "Convert leads you already have — no new spend",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Add FCOO support as case volume grows",
            "Track profit by visa case type",
            "Fill the 30 open case slots with real clients",
            "Build coaching cadence for the 14-person team",
            "Free Renata from being the only growth engine",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$7,497", "$8,997/mo",
        "Google PPC · LSA · Local SEO · Meta Ads · Attorney bio",
        "1D4ED8",
    ),
    (
        "MASTER'S CIRCLE",
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $5,000–$50,000/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE     = "$4,500 (est.)"
CONSERVATIVE_LABEL = "Conservative (10 cases/mo):"
CONSERVATIVE_RESULT = "$45,000 revenue · 9.0× ROAS"
AGGRESSIVE_LABEL   = "Aggressive (126 cases/mo):"
AGGRESSIVE_RESULT  = "$567,000 revenue · 11.3× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Launch Google PPC + LSA for visa terms"),
    ("Day 14",  "Add attorney bio, fix NAP across directories"),
    ("Week 2",  "Kick off Master's Circle group coaching"),
    ("Week 3",  "Roll out defined intake follow-up sequence"),
    ("Month 3", "Review lead volume vs. the 30 open case slots"),
]

CLOSING_QUOTE = (
    '"I want new clients to find us — not the other way around."'
)
