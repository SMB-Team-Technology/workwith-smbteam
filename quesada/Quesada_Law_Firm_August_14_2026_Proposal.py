...
# Report metadata
FIRM_NAME      = "Quesada Law Firm"
SALES_REP      = "Jacob Meissner"
OUTPUT_PATH    = "quesada/Quesada_Law_Firm_August_14_2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL", "100% referral-based"),      # Lead Generation
    ("RED",   "CRITICAL", "Owner handles all intake"),  # Intake
    ("AMBER", "AMBER",    "3-person team, no layer"),   # Team
    ("AMBER", "AMBER",    "3-yr revenue plateau"),      # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "100% referral-based acquisition — invisible in organic search results"),
    ("neg", "Owner personally handles every intake call with no delegation in place"),
    ("neg", "Revenue plateaued at ~$700K/year for three straight years"),
    ("pos", "5.0-star Google rating — higher than every named competitor"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Michael Mirer, P.A.",   "306 reviews", "Ex-prosecutor · top organic rank"),
    ("Armando Hernandez, P.A.", "203 reviews", "Nat'l Trial Lawyers Top 100"),
    ("Wichman Law Firm",      "100+ reviews", "Active content, Super Lawyers"),
]
CLIENT_REVIEWS      = "~36 reviews"
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $1,000,000"

SMB_MODEL_DESC = (
    "All four pillars must work together. Missing any one means growth stalls "
    "regardless of ad spend. When all four work, Marco can finally step away "
    "from the firm and trust it still runs."
)

GOAL_HEADLINE = "$700K → $1M revenue"
GOAL_DBM      = "Step away from the firm — and trust it still runs"
...
PRIORITIES = [
    (
        ...
        [
            ...
            "Launch Search & LSA for criminal defense + DUI",
            "Close the visibility gap vs. Mirer & Hernandez",
            "Add Meta retargeting and video ad presence",
            "Refresh the site's dormant blog & content",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "69CD2B",
        [
            "Delegate intake so calls no longer wait on you",
            "Build a defined follow-up process for every lead",
            "Move the contact form above the fold",
            "Strengthen review generation on a 5.0-star base",
            "Free up hours currently spent on personal intake",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "003A59",
        [
            "Bring in a Fractional CTO to lead AI automation",
            "Connect Dropbox & Manus into agentic workflows",
            "Build systems that run without daily oversight",
            "Create visibility into what breaks the $700K plateau",
            "Set the foundation to eventually step out of law",
        ],
    ),
]
...
# Template supports a maximum of 2 cards. The two shown here are the packages that
# launch first per the client's own stated sequencing (capacity, then leads); the
# third recommended package, Fractional CTO Level 1 ($3,297/mo), is reflected in the
# bundle total below and in the Phase 3 timeline/closing note, consistent with its
# place as the third phase of the plan rather than an immediate launch item.
PACKAGES = [
    (
        "FULL SERVICE MARKETING — STARTER",
        "$4,997", "$5,697/mo",
        "Google Search & LSA · Meta Ads · GBP · Website & Local SEO",
        "0091C9",
    ),
    (
        "ELITE COACH PLUS",
        "$3,200", "$3,497/mo",
        "Weekly group coaching · Intake delegation framework",
        "003A59",
    ),
]

BUNDLE_TOTAL   = "$11,494 / mo"
BUNDLE_SAVINGS = "Save $1,497/mo by bundling (incl. Fractional CTO L1)"

AD_SPEND_NOTE = (
    "+ Recommended ad spend: $5,500–$20,000/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE     = "$4,000 (est.)"
CONSERVATIVE_LABEL = "Conservative  (3.8 cases/mo):"
CONSERVATIVE_RESULT = "$15,280 revenue · 2.8x ROAS"
AGGRESSIVE_LABEL   = "Aggressive  (20.5 cases/mo):"
AGGRESSIVE_RESULT  = "$82,000 revenue · 4.1x ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Elite Coach Plus kickoff begins"),
    ("Day 14",  "Intake delegation framework built"),
    ("Week 2",  "Google Search & LSA campaigns prepped"),
    ("Week 3",  "Marketing Starter officially launches"),
    ("Month 3", "Fractional CTO L1 onboarding begins"),
]

CLOSING_QUOTE = (
    '"Build a well-oiled machine that can operate without my daily '
    'involvement — so I can finally have my time back, and one day, '
    'walk away from the practice of law on my own terms."'
)
