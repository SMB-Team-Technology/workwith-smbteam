...
# Report metadata
FIRM_NAME      = "McFall Legal"
SALES_REP      = "Jacob Meissner"
OUTPUT_PATH    = "mcfall-legal/McFall Legal_July 24, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "RED",   "Referrals only, no system"),   # Lead Generation
    ("RED",   "RED",   "One form, no follow-up"),       # Intake
    ("AMBER", "AMBER", "2 attorneys, no staff"),         # Team
    ("AMBER", "AMBER", "No profit plan tracked"),        # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Legacy firm name still listed on 3 major directories"),
    ("neg", "No geo-landing pages, unlike competitor Daly Law Offices"),
    ("neg", "No defined intake follow-up process behind the contact form"),
    ("pos", "4.8 stars, 21 reviews — already beats both local rivals"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Daly Law Offices",   "13 reviews", "Built geo-landing pages"),
    ("Ceraul Law Offices", "15 reviews", "No geo-pages found"),
    ("Cramer Swetz et al.", "99 reviews", "Regional scale leader"),
]
CLIENT_REVIEWS      = "21 reviews (4.8★)"
...
SMB_MODEL_DESC = (
    ...
    "When they do, Alan can finally step back and know McFall Legal runs without him."
)

GOAL_HEADLINE = "$180K → $360K revenue"
GOAL_DBM      = "Step back and know Jason can run it"
...
PRIORITIES = [
    (
        ...
        [
            "Launch Google Ads & LSA in Bangor market",
            "Turn Google into a pipeline that runs itself",
            "Fix legacy name on Avvo, FindLaw, Yellow Pages",
            "Build geo-pages for Bangor, Wind Gap, Easton",
            "Close the SEO gap with Daly Law Offices",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Build a defined consultation follow-up process",
            "Add after-hours coverage for evening inquiries",
            "Track every lead from form to signed client",
            "Stop leads slipping through with no process",
            "Free Alan from personally chasing every lead",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Start weekly Elite Coach group coaching",
            "Build a documented profit plan and CPA tracking",
            "Give Jason real ownership of firm systems",
            "Create a team that runs without Alan there",
            "Turn casework into a business worth handing down",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "Google Ads · LSA · Local SEO · Directory & NAP cleanup",
        "1D4ED8",
    ),
    (
        "ELITE COACH",
...
AGGRESSIVE_RESULT  = "$33,000 revenue · 4.4× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Kickoff call & Google/LSA campaign build begins"),
    ("Day 14",  "Ads launch across Bangor/Northampton County"),
    ("Week 2",  "Elite Coach coaching sessions begin"),
    ("Week 3",  "Directory cleanup & geo-pages published"),
    ("Month 3", "Full intake process & CPA tracking in place"),
]

CLOSING_QUOTE = (
    '"The firm keeps growing whether I\'m the one closing every case or not '
    '— and Jason inherits something built to last."'
)
