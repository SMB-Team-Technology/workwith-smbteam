...
# Report metadata
FIRM_NAME      = "McNeil Law Firm"
SALES_REP      = "Jacob Meissner"
OUTPUT_PATH    = "mcneil-law-firm/McNeil_Law_Firm_June_22_2026_Proposal.pptx"
...
PILLARS = [
    ("AMBER", "AMBER", "Ads run; no attribution"),
    ("AMBER", "AMBER", "26% close; no system"),
    ("AMBER", "AMBER", "Solo attorney ceiling"),
    ("RED",   "CRITICAL", "Revenue −30% YoY"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings
FINDINGS = [
    ("neg", "Revenue down 30% to $720K — ad spend has zero attribution to show what works"),
    ("neg", "Page 2 rankings; competitors with 100+ reviews own every 3-pack position"),
    ("neg", "No financial plan — slow season arrives each year with no pipeline built for it"),
    ("pos", "26% qualified close rate — conversion grows significantly with a defined intake system"),
]

# Competitor table
COMPETITORS = [
    ("McIlveen Family Law Firm",          "102 Google reviews", "3-pack dominant · family law"),
    ("Parker Bryan Family Law",           "Not verified",        "9 Super Lawyers · downtown Raleigh"),
    ("Gailor Hunt Davis Taylor & Gibbs",  "Not verified",        "Est. 1994 · NC family law authority"),
]
CLIENT_REVIEWS      = "67 reviews"
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $1.5M"

SMB_MODEL_DESC = (
    "Lead Generation, Intake, Team, and Profit must all work together — "
    "fixing one without the others only moves the bottleneck. "
    "When all four pillars work, John can stop dreading the slow season "
    "and run a firm with a full pipeline, every month."
)

GOAL_HEADLINE = "$720K → $1.5M revenue"
GOAL_DBM      = "Know the pipeline is full — never dread another slow season"
...
PRIORITIES = [
    (
        ...
        [
            "Replace in-house PPC with tracked Google Ads",
            "Fix LSA account; build reviews from 67 to 100+",
            "Add Meta retargeting for website visitors",
            "Build city pages for Cary, Durham, Wake Forest",
    ...
    (
        ...
        [
            "Move contact form above fold — stop losing warm leads",
            "5-minute speed-to-lead response protocol",
            "After-hours capture: voicemail + form autoresponse",
            "Follow-up sequence for unresponsive qualified leads",
            "Know which ads send cases — not just clicks",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Monthly P&L review — stop flying blind on numbers",
            "Hire paralegal — more cases, not more hours",
            "Defined org structure: intake + operations roles",
            "6-month pipeline forecast to plan for slow season",
            "Coaching framework tied to the $1.5M goal",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$7,847", "$9,697/mo",
        "Google Ads · LSA · website · review acquisition · reporting",
        "1D4ED8",
    ),
    (
        "ELITE COACH PLUS",
        "$3,200", "$3,497/mo",
        "Weekly group coaching · masterminds · workshops",
        "6D28D9",
    ),
]

BUNDLE_TOTAL   = "$11,047 / mo"
BUNDLE_SAVINGS = "Save $2,147/mo by bundling"

AD_SPEND_NOTE = (
    "+ Recommended ad spend: $3,500–$5,000/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE     = "~$4,100 (blended)"
CONSERVATIVE_LABEL = "Conservative  (6 cases/mo):"
CONSERVATIVE_RESULT = "~$24,600 revenue · ~7× ROAS"
AGGRESSIVE_LABEL   = "Aggressive  (11 cases/mo):"
AGGRESSIVE_RESULT  = "~$45,100 revenue · ~9× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Launch Google Ads with conversion tracking"),
    ("Day 14",  "Audit & optimize LSA account"),
    ("Week 2",  "Website homepage conversion optimization"),
    ("Week 3",  "Launch structured review acquisition process"),
    ("Month 3", "Coaching kickoff + $1.5M accountability framework"),
]

CLOSING_QUOTE = (
    '"The pipeline is full. The slow season is just a line on a chart. '
    'Payroll is not a conversation anymore — because the system built it that way."'
)
