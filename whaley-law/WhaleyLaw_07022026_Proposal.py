...
======================================
Whaley Law | Jacob Meissner | July 2, 2026
"""
...
# ═══════════════════════════════════════════════════════════════════
# FILL — Whaley Law specific values
# ═══════════════════════════════════════════════════════════════════

FIRM_NAME   = "Whaley Law"
SALES_REP   = "Jacob Meissner"
OUTPUT_PATH = "whaley-law/WhaleyLaw_07022026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL",  "Website down; zero ads"),
    ("RED",   "CRITICAL",  "No coverage after hours"),
    ("RED",   "CRITICAL",  "Solo — zero staff"),
    ("AMBER", "AMBER",     "Felony priced at $7K"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

FINDINGS = [
    ("neg", "Website SSL expired — new clients get a security error and leave"),
    ("neg", "18 reviews at 4.1★ — invisible in the 3-pack across all practice areas"),
    ("neg", "Stewart handles all intake alone — missed calls = missed cases at $1,100+"),
    ("pos", "Revenue doubled YoY — $15K to $30K/month in 12 months"),
]

COMPETITORS = [
    ("Lemley Law Partners",  "102 reviews", "4.9★ · owns criminal defense 3-pack"),
    ("Ludwig Law Firm",      "55 reviews",  "4.9★ · Google Ads for traffic + criminal"),
    ("Punch Work Law",       "mixed",       "running ads for nursing board niche"),
]
CLIENT_REVIEWS      = "18 reviews"
CLIENT_REVIEWS_NOTE = "← You are here"

STAGE_TEXT = "Stage 3: Solo Practitioner  →  Goal: Stage 6, Law Firm Owner"

# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $1,000,000/Year"

SMB_MODEL_DESC = (
    "Lead Generation, Intake, Team, and Profit must all work together — "
    "fixing one without the others just creates a new bottleneck. "
    "When all four pillars are firing, Stewart can reach $1M annually "
    "and finally step back from being the person everything flows through."
)

GOAL_HEADLINE = "$300K → $1M+ annual revenue"
GOAL_DBM      = "Work a 40-hour week. Take real vacations."

PRIORITIES = [
    (
        "Build the", "Marketing Engine", "1D4ED8",
        [
            "Restore website — make the firm findable again",
            "Launch Google Ads for traffic tickets in Little Rock",
            "Run paid ads targeting nursing board defense niche",
            "Review campaign — close the 84-review gap fast",
            "Build local pages for Dallas County + Clark County",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "24/7 answering service — no more missed calls",
            "Restore web contact form on rebuilt website",
            "Build follow-up sequence for unconverted leads",
            "Capture every traffic ticket lead at $1,100/case",
            "Scale intake capacity before ad volume increases",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Raise felony fees — $7K toward $12K+ market rate",
            "Hire part-time VA — free 10+ admin hours per week",
            "Weekly coaching: pricing, profit, and milestones",
            "Know which practice area earns the most per hour",
            "Build a firm that earns when Stewart is offline",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$4,847", "$5,697/mo",
        "Website rebuild · Google Ads · LSA · Local SEO · Reviews",
        "1D4ED8",
    ),
    (
        "ELITE COACH",
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $5,500–$11,500/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE      = "$1,600"
CONSERVATIVE_LABEL  = "Conservative  (5–6 cases/mo):"
CONSERVATIVE_RESULT = "$8,500 revenue · 1.5× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (13–14 cases/mo):"
AGGRESSIVE_RESULT   = "$22,000 revenue · 2.0× ROAS"

TIMELINE = [
    ("Day 1",    "Kickoff · SSL fix begins · coaching onboards"),
    ("Week 2",   "Website rebuild + Google Ads go live"),
    ("Week 4",   "Review campaign — past clients contacted"),
    ("Week 6",   "After-hours answering service fully active"),
    ("Month 3",  "25+ reviews · 3-pack eligibility in reach"),
]

CLOSING_QUOTE = (
    '"A firm that generates $1M a year — so Stewart can work normal hours, '
    'take real vacations, and stop carrying the whole thing alone."'
)
