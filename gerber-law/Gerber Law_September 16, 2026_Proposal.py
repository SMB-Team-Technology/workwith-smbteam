...
# Report metadata
FIRM_NAME      = "Gerber Law"
SALES_REP      = "Jacob Meissner"
OUTPUT_PATH    = "gerber-law/Gerber Law_September 16, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL", "85-90% referrals"),
    ("AMBER", "AMBER",    "No tracking system"),
    ("AMBER", "AMBER",    "Owner does it all"),
    ("AMBER", "AMBER",    "No ROI tracking"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Review presence trails competitors despite 29 years in practice"),
    ("neg", "20 years of ad spend showed no correlation to case volume"),
    ("neg", "No lead tracking risks losing referrals already earned"),
    ("pos", "Modern, mobile-friendly site with 30+ SEO pages already built"),
]

# Competitor table — review counts are unconfirmed pending a live Google
# check, so qualitative comparisons are used instead of specific numbers.
COMPETITORS = [
    ("Goldman Babboni Fernandez Murphy & Walsh", "More reviews", "large regional brand"),
    ("David Harris Law",                          "Higher rated", "hyper-local Venice rival"),
    ("Dannheisser Injury Law",                    "Strong presence", "30+ yr Sarasota rival"),
]
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $3M/Year"

SMB_MODEL_DESC = (
    "All four pillars — lead generation, intake, team, and profit — "
    "must work together. When they do, Maria can finally let 29 years "
    "of earned trust do the work instead of carrying it herself."
)

GOAL_HEADLINE = "$1M → $3M revenue"
GOAL_DBM      = "Let 29 years of trust work for you"
...
PRIORITIES = [
    (
        ...
        [
            "Launch AI-search-ready content across your site",
            "Close the Port Charlotte visibility gap",
            "Unify reviews across Venice and Sarasota",
            "Optimize both Google Business Profiles",
            "Make your reputation impossible to miss online",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "69CD2B",
        [
            "Build a referral follow-up process",
            "Add after-hours lead coverage",
            "Track every lead from call to case",
            "Protect the referrals 29 years earned",
    ...
    (
        ...
        [
            "Start monthly coaching for delegation",
            "Hand off vendor decisions, not just tasks",
            "Track cost-per-case on every dollar spent",
            "Build a team that frees up your time",
            "Grow revenue without losing your weekends",
        ],
    ),
]
...
PACKAGES = [
    (
        "WEB + SEO STARTER",
        "$3,497", "$3,997/mo",
        "SEO content · AI search visibility · GBP · reviews",
        "0091C9",
    ),
    (
        "COACH ESSENTIALS PLUS",
        "$1,997", "$2,497/mo",
        "Group coaching · monthly 1:1 · delegation frameworks",
        "003A59",
    ),
]

BUNDLE_TOTAL   = "$5,494 / mo"
BUNDLE_SAVINGS = "Save $1,000/mo by bundling"

AD_SPEND_NOTE = (
    "+ Future ad spend (Phase 2): $9,200-$50,000/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE     = "$5,000-$8,000 (industry benchmark)"
CONSERVATIVE_LABEL = "Conservative  (2-3 cases/mo):"
CONSERVATIVE_RESULT = "$13K-$19.5K revenue · ~2.1x ROAS"
AGGRESSIVE_LABEL   = "Aggressive  (~17 cases/mo):"
AGGRESSIVE_RESULT  = "$110.5K revenue · ~2.2x ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Kick off AI-search content sprint"),
    ("Day 14",  "Launch review-generation push, both offices"),
    ("Week 2",  "Unify and optimize both Google Business Profiles"),
    ("Week 3",  "Begin monthly coaching sessions"),
    ("Month 3", "Baseline intake follow-up process live"),
]

CLOSING_QUOTE = (
    '"I don\'t need more ad spend I can\'t track — I need my reputation '
    'to finally show up everywhere people are searching."'
)
...
FONT = "Poppins"
LOGO_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "Design Files", "smb_team_logo.png")
