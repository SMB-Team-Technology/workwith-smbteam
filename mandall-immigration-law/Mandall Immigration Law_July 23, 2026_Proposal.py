...
# Report metadata
FIRM_NAME      = "Mandall Immigration Law"
SALES_REP      = "Nick Holderman"
OUTPUT_PATH    = "mandall-immigration-law/Mandall Immigration Law_July 23, 2026_Proposal.pptx"
...
PILLARS = [
    ("AMBER", "AMBER",    "Swings $37K-$111K/wk"),
    ("RED",   "CRITICAL", "Owner does own admin"),
    ("RED",   "CRITICAL", "No delegation exists"),
    ("AMBER", "AMBER",    "No reinvest strategy"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "One ad account swings $37K to $111K weekly with zero reporting"),
    ("neg", "75-90 day file delays drag cash flow and delay retainer payments"),
    ("neg", "Owner still personally handles payment collection and file opening"),
    ("pos", "Scaled to a ~$2M revenue run rate in under two years on drive alone"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Green & Spiegel",   "1,182 reviews", "40+ yrs · dominant organic"),
    ("Matkowsky Immigration", "846+ reviews", "Google + Microsoft remarketing"),
    ("Kingwell Immigration",  "390 reviews", "5.0★ · appeals specialist"),
]
...
SMB_MODEL_DESC = (
    ...
    "together. When they do, Austin can finally step back from admin work and "
    "travel while the firm keeps growing toward his $100M vision."
)

GOAL_HEADLINE = "$2M/yr → $6M/yr"
GOAL_DBM      = "Step away from admin — and finally travel"
...
PRIORITIES = [
    (
        ...
        [
            "Rebuild Google Ads with full performance reporting",
            "Turn Google Ads into a pipeline you can see, not guess",
            "Relaunch Meta retargeting — warm leads only",
            "Start a review-generation program at every case close",
            "Expand geo landing pages beyond Toronto & Ottawa",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Cut file completion from 90 days to 2-3 weeks",
            "Replace Apple Notes with real lead tracking",
            "Speed up second retainer collection at submission",
            "Add live chat to capture visitors who won't call",
            "Build a defined follow-up sequence for every lead",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Move payment collection off Austin's desk",
            "Build a delegation plan for file opening",
            "Add real accountability for underperforming staff",
            "Turn 55-60% margins into a reinvestment plan",
            "Give Austin a real day off without the firm stalling",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$10,497", "$12,497/mo",
        "Google Ads · Meta Retargeting · Website/SEO · Reporting",
        "1D4ED8",
    ),
    (
        "ELITE COACH PLUS",
        "$3,200", "$3,497/mo",
        "Weekly coaching · Delegation playbook · Mastermind access",
        "6D28D9",
    ),
]

BUNDLE_TOTAL   = "$13,697 / mo"
BUNDLE_SAVINGS = "Save $2,297/mo by bundling"

AD_SPEND_NOTE = (
    "+ Recommended ad spend: $4,200–$56,000/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE     = "$4,500 (est.)"
CONSERVATIVE_LABEL = "Conservative  (6 cases/mo):"
CONSERVATIVE_RESULT = "$27,000 revenue · 6.4× ROAS"
AGGRESSIVE_LABEL   = "Aggressive  (93 cases/mo):"
AGGRESSIVE_RESULT  = "$418,500 revenue · 7.5× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Launch rebuilt, fully reported Google Ads account"),
    ("Day 14",  "Relaunch Meta retargeting, warm leads only"),
    ("Week 2",  "Confirm PageSpeed/GBP data, start review program"),
    ("Week 3",  "Coaching kickoff — build the delegation plan"),
    ("Month 3", "Real lead tracking system live, replacing Apple Notes"),
]

CLOSING_QUOTE = (
    '"Step back from the admin, and let this firm finally fund '
    'the life I built it for."'
)
