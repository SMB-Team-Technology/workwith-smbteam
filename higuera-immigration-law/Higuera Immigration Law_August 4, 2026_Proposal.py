...
# Report metadata
FIRM_NAME      = "Higuera Immigration Law"
SALES_REP      = "Dan Bryant"
OUTPUT_PATH    = "higuera-immigration-law/Higuera Immigration Law_August 4, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL", "No paid channel"),
    ("RED",   "CRITICAL", "No delegated team"),
    ("RED",   "CRITICAL", "Solo + ICs only"),
...
FINDINGS = [
    ("neg", "No paid lead channel — invisible in every search tested"),
    ("neg", "Every intake call runs through Danja personally"),
    ("neg", "No fixed team — owner cannot step away from the firm"),
    ("pos", "Already vetting an IC attorney to delegate removal cases"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("American Immigration Law Ctr", "156 reviews", "Same building"),
    ("Carlos E. Sandoval, P.A.",     "144 reviews", "Same building"),
    ("Parjus Law",                   "22 reviews",  "Biz. imm. niche"),
]
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $250K/Year"

SMB_MODEL_DESC = (
    "All four pillars — leads, intake, team, and profit — have to work "
    "together. When they do, Danja can finally step back and let the firm "
    "run without her in the middle of every case."
)

GOAL_HEADLINE = "$125K → $250K revenue"
GOAL_DBM      = "Danja works fewer hours, present for her kids"
...
PRIORITIES = [
    (
        ...
        [
            "Launch LSA management for imm. inquiries",
            "Get Google Screened above building rivals",
            "Fix NAP — remove stale Ohio phone number",
            "Build citations on Avvo, Birdeye, Yelp",
            "Add geo-pages for Davie, Sunrise, Ft. Lauderdale",
        ],
    ),
    (
        ...
        [
            ...
            "Route new LSA leads off Danja's own phone",
            "Track every inquiry from call to signed case",
            "Fix homepage CTA — form is buried in footer",
            "Confirm mobile speed once site rebuild lands",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Elite Coach: map org structure and KPIs",
            "Sequence first hire after removal-defense IC",
            "Build a hiring plan funded by LSA lead flow",
            "Set a profit plan tied to real case data",
            "Track cost per case once ads are live",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$2,600", "$3,497/mo",
        "Weekly group coaching · masterminds · delegation plan",
        "6D28D9",
    ),
    (
        "RECOMMENDED LEAD-GEN ADD-ON — LSA MANAGEMENT",
        "$900", "No stand-alone",
        "LSA setup & management · pay-per-lead · reporting",
        "1D4ED8",
    ),
]

BUNDLE_TOTAL   = "$3,500 / mo"
BUNDLE_SAVINGS = "Save $897/mo by bundling Elite Coach"

AD_SPEND_NOTE = (
    "+ Recommended ad spend: $3,000-$5,400/mo paid directly to Google/LSA"
)

AVG_CASE_VALUE     = "$4,500 (est.)"
CONSERVATIVE_LABEL = "Conservative  (~6 cases/mo):"
CONSERVATIVE_RESULT = "$26,000 revenue · 8.7× ROAS"
AGGRESSIVE_LABEL   = "Aggressive  (~13 cases/mo):"
AGGRESSIVE_RESULT  = "$56,000 revenue · 10.4× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Elite Coach kickoff — map org & KPIs"),
    ("Day 14",  "LSA management campaign goes live"),
    ("Week 2",  "NAP citations cleaned up across directories"),
    ("Week 3",  "New lead follow-up sequence built"),
    ("Month 3", "First delegation hire sequenced & funded"),
]

CLOSING_QUOTE = (
    '"A firm that runs without me — so I can be at pickup, at recitals, '
    'at bedtime, not catching up on calls."'
)
