"""
Audit PowerPoint — Sivia Business & Legal Services
...
# Report metadata
FIRM_NAME   = "Sivia Business & Legal Services"
SALES_REP   = "Dan Bryant"
OUTPUT_PATH = "sivia-business-legal-services/Sivia_Business_Legal_Services_June_2_2026_Proposal.pptx"
...
PILLARS = [
    ...
    ("AMBER", "IN PROGRESS", "Clio active, gaps TBD"),
    ("AMBER", "IN PROGRESS", "Todd drives new biz"),
    ("AMBER", "IN PROGRESS", "No P&L by location"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

FINDINGS = [
    ("neg", "No paid ads — competitors claim digital lead flow Sivia misses"),
    ("neg", "4 reviews vs. Blake's 52+ — credibility gap in every practice area"),
    ("neg", "7 offices with no geo pages — footprint invisible to local search"),
    ("pos", "Clio Grow active + modern website — intake foundation in place"),
]

COMPETITORS = [
    ("Blake Behme Links Raney, P.C.", "52 reviews", "#1 estate plan & biz law"),
    ("Wendler & Zinzilieta, PC",      "79 reviews", "Dominates PI, Metro East IL"),
    ("Goldenberg Heller & Rowland",   "Avvo 9.8",   "52-yr firm, 10 overlap areas"),
]
CLIENT_REVIEWS      = "4 reviews"
CLIENT_REVIEWS_NOTE = "← You are here"

STAGE_TEXT = "Stage 4: Small Business Manager  →  Goal: Stage 6, Law Firm Owner"

# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach Consistent Client Flow"

SMB_MODEL_DESC = (
    "Lead Generation, Intake, Team, and Profit — "
    "all four pillars must work together. "
    "When they do, Todd can step back from being "
    "the firm's primary rainmaker and let the systems generate clients."
)

GOAL_HEADLINE = "7 Offices  →  Digital Lead Engine"
GOAL_DBM      = "Step back — the firm generates clients on its own"
...
PRIORITIES = [
    (
        ...
        [
            ...
            "Close review gap: 25+ Google reviews in 90 days",
            "Build geo pages for all 7 offices + nearby cities",
            "Launch Meta retargeting for non-converting visitors",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Embed contact form above fold — remove redirect",
            "After-hours response protocol across all 7 offices",
            "Standardize intake tracking in Clio Grow",
            "First response under 1 hour, every location",
            "Stop losing after-5pm leads to faster competitors",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "KPIs and scorecards for all 7 office locations",
            "Quarterly P&L review by office and practice area",
            "Build management layer — less on Todd's desk",
            "Separate firm brand from Todd's personal name",
            "Coaching to standardize ops across all offices",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$4,847", "$5,697/mo",
        "Google Ads · LSA · SEO · Meta Ads · GBP — all 7 offices",
        "1D4ED8",
    ),
    (
        "ELITE COACH PLUS",
        "$3,200", "$3,497/mo",
        "Weekly coaching · intake standards · KPIs · financials",
        "6D28D9",
    ),
]
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $6,500–$15,000/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE      = "$3,000"
CONSERVATIVE_LABEL  = "Conservative  (8 cases/mo):"
CONSERVATIVE_RESULT = "$24,000 revenue · 3.7× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (21 cases/mo):"
AGGRESSIVE_RESULT   = "$63,000 revenue · 4.2× ROAS"

TIMELINE = [
    ("Day 1",   "Google Ads + LSA campaigns launched"),
    ("Day 14",  "All 7 GBP listings rebuilt and live"),
    ("Week 2",  "Website intake form embedded + optimized"),
    ("Week 3",  "Elite Coach Plus kickoff meeting"),
    ("Month 3", "Review campaign at 25+ new Google reviews"),
]

CLOSING_QUOTE = (
    '"The reputation Todd spent 20 years building is there — '
    'the digital system to turn that reputation into a predictable flow of new clients is not, yet."'
)
