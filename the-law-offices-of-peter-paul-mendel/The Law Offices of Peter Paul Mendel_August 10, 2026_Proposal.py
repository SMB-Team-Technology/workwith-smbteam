...
# Report metadata
FIRM_NAME      = "The Law Offices of Peter Paul Mendel"
SALES_REP      = "Jacob Meissner"
OUTPUT_PATH    = "the-law-offices-of-peter-paul-mendel/The Law Offices of Peter Paul Mendel_August 10, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED", "CRITICAL", "Referrals only"),      # Lead Generation
    ("RED", "CRITICAL", "No screening filter"), # Intake
    ("RED", "CRITICAL", "Solo, zero staff"),     # Team
    ("RED", "CRITICAL", "$500K AR history"),     # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "$500K in uncollected fees nearly ended his last practice"),
    ("neg", "3 conflicting addresses & phone numbers across directories"),
    ("neg", "Zero staff — Peter personally handles every case and dollar"),
    ("pos", "A real, working referral landing page already exists (subdomain)"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Paul J. Carter, APC",  "5.0 peer rating", "Same zip code · Long Beach"),
    ("Gayle Eskridge / ELM", "Active Yelp",     "2026 Avvo Client's Choice"),
    ("ROSS Mediation Svcs.", "11 reviews",       "2,800+ mediations since '93"),
]
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $40,000/Month"

SMB_MODEL_DESC = (
    "All four pillars — leads, intake, team, and profit — must work "
    "together. Before any paid marketing, Peter's priority is building "
    "the referral, intake, and collections systems so he can finally "
    "build a fifty-mediator platform without risking another dollar of "
    "uncollected fees."
)

GOAL_HEADLINE = "$3.5K → $40K/month"
GOAL_DBM      = "A platform of mediators, never chasing unpaid bills"

# Each priority: (line1, line2, accent_color_hex, [5 bullet strings])
PRIORITIES = [
    (
        "Build the", "Referral Engine", "1D4ED8",
        [
            "Standardize address & phone across directories",
            "Update Avvo/FindLaw off old practice areas",
            "Build referral-partner outreach cadence",
            "Add geo pages once volume can support them",
            "Position for pay-per-case ads later, not now",
        ],
    ),
    (
        "Fix Intake &", "Filter Looky-Loos", "0F766E",
        [
            "Build mediation-specific qualifying script",
            "Screen for multi-party buy-in before calls",
            "Turn referrals into scheduled mediations",
            "Reduce time wasted on one-sided callers",
            "Free up hours for real case work",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Hire first support role for billing/admin",
            "Deploy AI billing/AR tools before scaling",
            "Track mediation revenue separate from legacy work",
            "Build hiring roadmap toward more mediators",
            "Protect against another $500K AR loss",
        ],
    ),
]
...
# NOTE: Only one package is recommended this round (Elite Coach) — no marketing
# package or ad spend, per the transcript-stated call-purpose and budget-reality
# overrides documented in sections/section_11_workings.txt. The second card slot
# is intentionally left empty rather than force-fitting a marketing package.
PACKAGES = [
    (
        "ELITE COACH",
        "$2,600", "$3,497/mo",
        "Weekly coaching · Referral systems · Intake filter · AR/billing tools",
        "6D28D9",
    ),
]
...
AD_SPEND_NOTE = (
    "No ad spend this round — revisited once referral/intake systems are proven "
    "and a pay-per-signed-case vendor is identified"
)

AVG_CASE_VALUE     = "~$3,000 (est., mediation)"
CONSERVATIVE_LABEL = "This round:"
CONSERVATIVE_RESULT = "No ad spend recommended yet"
AGGRESSIVE_LABEL   = "Future phase:"
AGGRESSIVE_RESULT  = "Modeled once a vendor is chosen"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Kick off Elite Coach — map referral network"),
    ("Day 14",  "Draft mediation intake qualification script"),
    ("Week 2",  "Deploy AI billing/AR/follow-up tooling"),
    ("Week 3",  "Clean up NAP across all directories"),
    ("Month 3", "Define first hire; explore ad vendors later"),
]

CLOSING_QUOTE = (
    '"Fifty mediators, all paid upfront — and I never have to chase '
