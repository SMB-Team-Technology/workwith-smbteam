...
# Report metadata
FIRM_NAME      = "JD Law Group"
SALES_REP      = "Jacob Meissner"
OUTPUT_PATH    = "jd/JD_Law_Group_July_09_2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL",  "0 indexed pages on PI site"),
    ("AMBER", "AMBER",     "65% close rate — declining"),
    ("AMBER", "AMBER",     "3-person team, no ops role"),
    ("AMBER", "AMBER",     "Revenue down 85% from 2025"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Case volume collapsed 88% (criminal) and 82% (PI) year-over-year — firm on pace for 25% of 2025 revenue"),
    ("neg", "roswelllegal.com has zero indexed pages; chancoschiffer.com still live, indexed, same NAP — authority split"),
    ("neg", "No LSAs, no PPC, no Meta ads running — every search for criminal defense and PI in Roswell goes to competitors"),
    ("pos", "117 reviews at 4.7 stars — qualifies for Google Screened LSA status in both practice areas right now"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("The Ford Law Firm",      "17+ reviews",  "Criminal · Google AI results"),
    ("Hammers Car Accident",   "400+ reviews", "PI · 4× JD's review count"),
    ("Speights Law PC",        "90+ reviews",  "Criminal · Roswell LSA likely"),
]
CLIENT_REVIEWS      = "117 reviews"
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $250,000/Month"

SMB_MODEL_DESC = (
    "Lead generation, intake, team, and profit must all work together — "
    "fixing one without the others stalls growth. "
    "When all four are running, Doug gets to spend his time on cases, "
    "not managing the business behind them."
)

GOAL_HEADLINE = "Recover to $3M/yr revenue"
GOAL_DBM      = "Practice law — not manage the business"
...
PRIORITIES = [
    (
        ...
        [
            "Redirect chancoschiffer.com → jdgroup.law",
            "Launch LSA — pay only per qualified call",
            "Run Google Ads on conversion-optimized pages",
            "Optimize GBP — unlock Roswell local 3-pack",
            "Build content that wins Google AI citations",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Add above-fold form — capture after-hours leads",
            "60-min response: text + call + email protocol",
            "Designate dedicated intake owner role",
            "Fix Avvo + Yelp — update to JD Law Group name",
            "Automate intake so no lead goes unanswered",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Analyze margin: criminal vs. PI per case type",
            "Hire intake/ops coordinator — free Doug's time",
            "Weekly KPI dashboard — track case recovery",
            "Track cost per case — know your true ROI",
            "Build ops structure so Doug practices law",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$7,397", "$8,997/mo",
        "Website rebuild · Google Ads · LSA · Local SEO · GBP",
        "1D4ED8",
    ),
    (
        "ELITE COACH PLUS",
...
CONSERVATIVE_RESULT = "~$58,500 revenue · 7.8× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (44-46 cases/mo):"
AGGRESSIVE_RESULT   = "~$299,000 revenue · 10× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Coaching kickoff + strategy session"),
    ("Day 14",  "LSA applications submitted — both practice areas"),
    ("Week 2",  "chancoschiffer.com 301 redirect live"),
    ("Week 3",  "Google Ads + landing pages launched"),
    ("Month 3", "Local pack visibility + case volume tracked"),
]

CLOSING_QUOTE = (
    '"I want to get back to actually practicing law — solving cases, '
    'not managing a business that should be running itself."'
)
