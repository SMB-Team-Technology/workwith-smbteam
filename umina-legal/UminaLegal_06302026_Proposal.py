...
# Report metadata
FIRM_NAME      = "Umina Legal"
SALES_REP      = "Nick Holderman"
OUTPUT_PATH    = "umina-legal/UminaLegal_06302026_Proposal.pptx"
...
PILLARS = [
    ("AMBER", "IN PROGRESS",  "One paid channel, no backup"),   # Lead Generation
    ("RED",   "CRITICAL",     "Ryan handles every call"),        # Intake
    ("RED",   "CRITICAL",     "2-person, no delegation layer"),  # Team
    ("AMBER", "IN PROGRESS",  "No books or CRM visibility"),     # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Ryan personally handles every intake call — the structural bottleneck that causes burnout before the firm ever reaches $2M."),
    ("neg", "Slavey & Shumaker dominates with 101 Google reviews at 4.8 stars while Umina Legal's reviews page is completely empty."),
    ("neg", "Google Ads at $3K/month is the only lead source — three satellite offices generate zero paid leads today."),
    ("pos", "$100K/month revenue with Super Lawyers recognition — a strong foundation the right systems can scale to $2M."),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Slavey & Shumaker",      "101 reviews, 4.8 stars", "3-pack dominant"),
    ("The Moore Law Firm",     "~40 reviews, 4.7 stars", "AV Preeminent"),
    ("Mountaineer Criminal",   "9 reviews, 4.1 stars",   "Criminal-only domain"),
]
CLIENT_REVIEWS      = "0 confirmed reviews"
CLIENT_REVIEWS_NOTE = "← You are here"

# Stage strip (right panel, Slide 1)
STAGE_TEXT = "Stage 4: Small Business Manager  →  Goal: Stage 5, Law Firm CEO"

# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $2M/Year"

SMB_MODEL_DESC = (
    "SMB Team builds four pillars together — marketing infrastructure, "
    "intake systems, a self-managing team, and financial clarity. Each "
    "pillar reinforces the others. When Ryan has all four in place, the "
    "30-hour week he described stops being a goal and becomes the natural "
    "output of a firm built the right way."
)

GOAL_HEADLINE = "$1.2M → $2M annual revenue"
GOAL_DBM      = "Shift from operator to owner — step away and trust the firm runs"
...
PRIORITIES = [
    (
        ...
        [
            "Turn Google into a multi-city pipeline",
            "Launch LSA to rank above every paid ad",
            "Build reviews to close gap on Slavey & Shumaker",
            "Expand to Parkersburg, Charleston, Waynesburg",
            "Rebuild website for mobile speed and leads",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Hire intake coordinator — Ryan exits the phones",
            "Script the call to lift the 15% close rate",
            "Clean-slate Practice Panther for lead tracking",
            "Track lead-to-case rate by channel",
            "Install after-hours coverage so no lead waits",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Define assistant role with clear accountability",
            "Engage bookkeeping — know what the firm keeps",
            "Build hiring plan for the intake coordinator",
            "Install monthly P&L and KPI visibility",
            "Delegate operations so Ryan focuses on cases",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$8,397", "$9,997/mo",
        "Google Ads · LSA · Website rebuild · Review gen · Local SEO",
        "1D4ED8",
    ),
    (
        "ELITE COACH PLUS",
        "$3,200", "$3,497/mo",
        "Weekly coaching · Criminal defense mastermind · 90-day plan",
        "6D28D9",
    ),
]

BUNDLE_TOTAL   = "$11,597 / mo"
BUNDLE_SAVINGS = "Save $1,897/mo by bundling"

AD_SPEND_NOTE = (
    "+ Recommended ad spend: $7,500–$15,000/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE     = "$5,000–$7,500 (criminal defense default)"
CONSERVATIVE_LABEL = "Conservative  (7–9 cases/mo):"
CONSERVATIVE_RESULT = "~$40,000 revenue · ~5× ROAS"
AGGRESSIVE_LABEL   = "Aggressive  (16–20 cases/mo):"
AGGRESSIVE_RESULT  = "~$135,000 revenue · ~9× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",    "Expanded Google Ads + satellite campaigns launched"),
    ("Day 14",   "Website rebuild begins + review gen workflows active"),
    ("Week 2",   "Practice Panther clean-slate rebuild initiated"),
    ("Week 3",   "Elite Coach Plus kickoff + intake delegation plan built"),
    ("Month 3",  "LSA filed + 10+ new Google reviews targeted"),
]

CLOSING_QUOTE = (
    '"I want to shift from operator to owner — the firm needs to run without me at the center of everything."'
)
