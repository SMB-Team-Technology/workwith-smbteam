"""
Audit PowerPoint — Kopchian Law (July 30, 2026)
======================================
Generated from audit_pptx_template.py. Layout engine unmodified — only
FILL placeholders replaced.

Output: kopchian-law/Kopchian Law_July 30, 2026_Proposal.pptx
"""
...
# Report metadata
FIRM_NAME      = "Kopchian Law"
SALES_REP      = "Nick Holderman"
OUTPUT_PATH    = "kopchian-law/Kopchian Law_July 30, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED", "CRITICAL", "100% referrals, no site"),
    ("RED", "CRITICAL", "Solo intake, no process"),
    ("RED", "CRITICAL", "No staff, no delegation"),
    ("RED", "CRITICAL", "Expenses on credit cards"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Weekly revenue already falls short of debt obligations"),
    ("neg", "No website exists — domain redirects to a parked GoDaddy page"),
    ("neg", "1.0-star Google rating vs. hundreds of reviews for competitors"),
    ("pos", "Referral reputation is strong enough to sustain the firm today"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Alena Shautsova Law",  "318 reviews", "4.7★ · content-rich site"),
    ("Hargis Law Firm",      "100+ reviews", "5.0★ · 11yr Avvo streak"),
    ("Gottfried & Gottfried","252 reviews", "40+ yrs in NYC market"),
]
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $7,000-$8,000/Week"

SMB_MODEL_DESC = (
    "All four pillars must work together. "
    "When lead generation, intake, team, and profit all work, Adam can finally step away from casework and trust the firm runs."
)

GOAL_HEADLINE = "$286K → $390K revenue"
GOAL_DBM      = "Become the owner, not just the player"
...
PRIORITIES = [
    (
        ...
        [
            "Rebuild the website — the domain is parked today",
            "Fix NAP mismatch across 3 directory listings",
            "Repair the 1.0-star Google Business Profile",
            "Launch a Google Search campaign, no ads exist now",
    ...
    (
        ...
        [
            "Build a defined intake process — none exists today",
            "Add a follow-up sequence for undecided prospects",
            "Capture after-hours inquiries you now lose to voicemail",
            "Convert more of the referrals you already generate",
            "Stop losing cases you already paid for by referral",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Begin delegating tasks Adam now handles solo",
            "Start Elite Coach to build real operating systems",
            "Create a basic profit plan — none exists today",
            "Replace the credit-card cycle with a cash buffer",
            "Free Adam from being the firm's only employee",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$3,497", "$3,797/mo",
        "Website rebuild · Local SEO · NAP fix · GBP repair · Google Ads",
        "1D4ED8",
    ),
    (
        "ELITE COACH",
        "$2,600", "$3,497/mo",
        "Weekly group coaching · Practice masterminds · Workshops",
        "6D28D9",
    ),
]

BUNDLE_TOTAL   = "$6,097 / mo"
BUNDLE_SAVINGS = "Save $1,197/mo by bundling"

AD_SPEND_NOTE = (
    "+ Recommended ad spend: $3,000–$6,000/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE     = "$4,500"
CONSERVATIVE_LABEL = "Conservative  (~4 cases/mo):"
CONSERVATIVE_RESULT = "$18,000 revenue · 6.0× ROAS"
AGGRESSIVE_LABEL   = "Aggressive  (~11 cases/mo):"
AGGRESSIVE_RESULT  = "$49,500 revenue · 8.3× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Kick off Elite Coach and website rebuild"),
    ("Day 14",  "NAP corrected across all directory listings"),
    ("Week 2",  "Google Business Profile review push begins"),
    ("Week 3",  "New website launches with practice area pages"),
    ("Month 3", "Conservative Google Search campaign live"),
]

CLOSING_QUOTE = (
    '"Marketing without systems would waste ad spend and overwhelm the firm — '
    'systems first, then growth that actually holds."'
)
