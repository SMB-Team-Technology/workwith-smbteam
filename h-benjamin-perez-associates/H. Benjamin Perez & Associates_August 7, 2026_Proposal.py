...
# Report metadata
FIRM_NAME      = "H. Benjamin Perez & Associates, P.C."
SALES_REP      = "Jacob Meissner"
OUTPUT_PATH    = "h-benjamin-perez-associates/H. Benjamin Perez & Associates_August 7, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "RED",   "Invisible in search"),   # Lead Generation
    ("RED",   "RED",   "No after-hours cover"),  # Intake
    ("AMBER", "AMBER", "Owner approval gate"),    # Team
    ("AMBER", "AMBER", "No profit plan yet"),     # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Invisible in organic search across all three practice areas"),
    ("neg", "After-hours calls go uncovered — costly for criminal defense"),
    ("neg", "27 of 39 probate steps still done manually, capping capacity"),
    ("pos", "4.9/5.0 Avvo rating and Super Lawyers recognition already earned"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Richard Roman Shum",     "217 reviews", "Divorce · outranks us"),
    ("Robert Tsigler",         "46+ reviews", "Criminal defense lead"),
    ("Stephen Bilkis & Assoc.", "9 reviews",  "Has geo-pages, we don't"),
]
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach 3x Growth"

SMB_MODEL_DESC = (
    "All four pillars — lead generation, intake, team, and profit — have to work "
    "together, or growth stalls no matter how much you spend. When intake and "
    "casework run without needing Hector's hand on every step, the firm can "
    "finally grow toward 3x without everyone working three times as hard."
)

GOAL_HEADLINE = "Goal: 3x revenue in 1 year"
GOAL_DBM      = "Hector stops being the bottleneck"
...
PRIORITIES = [
    (
        ...
        [
            "Reclaim visibility competitors already have",
            "Fix the NAP mismatch splitting local SEO",
            "Restart content after 3 years dormant",
            "Build borough pages like Bilkis already has",
            "Turn Google into a pipeline you don't run",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Deploy AI coverage for after-hours calls",
            "Automate 27 of 39 manual probate steps",
            "Sync intake data into Clio automatically",
            "Free the team from repetitive case prep",
            "Turn manual intake into a system that runs itself",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Build a real profit plan behind the 3x goal",
            "Track cost-per-case to know true margins",
            "Free Hector from being the single point of failure",
            "Formalize systems so they outlast one person",
            "Let Ben approve on proof, not just trust",
        ],
    ),
]
...
# Single-package engagement this round — no Full Service Marketing or coaching
# package is being recommended (Call-Purpose Override; see section_11_workings.txt).
PACKAGES = [
    (
        "LEGAL AI WORKFORCE — FRACTIONAL CTO LEVEL 1",
        "$3,297", "$3,797/mo",
        "Dedicated CTO · Probate & intake automation · After-hours AI coverage",
        "1D4ED8",
    ),
]

BUNDLE_TOTAL   = "$3,297 / mo"
BUNDLE_SAVINGS = "Save $500/mo by bundling"

AD_SPEND_NOTE = (
    "No paid ad spend this round — this is an automation-only engagement"
)

# NOTE: This engagement has no ad spend/ROAS to project, so the right-hand card
# on Slide 3 is repurposed below (see build_slide3) from "Projected Return on Ad
# Spend" to "Why Fractional CTO, Not Ads" — same box, same layout, honest content.
AVG_CASE_VALUE     = "$3,797/mo (1 automation)"
CONSERVATIVE_LABEL = "This engagement:"
CONSERVATIVE_RESULT = "$0 ad spend — automation only"
AGGRESSIVE_LABEL   = "Next phase:"
AGGRESSIVE_RESULT  = "Marketing scoped after Phase 1 proof"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Fractional CTO kickoff, audit Clio & tools"),
    ("Day 14",  "After-hours AI call coverage goes live"),
    ("Week 2",  "Map every step of the probate workflow"),
    ("Week 3",  "First automated probate steps deployed"),
    ("Month 3", "Intake synced to Clio, time savings reported"),
]

CLOSING_QUOTE = (
    '"When the repetitive work runs itself, growing 3x doesn\'t mean '
    'any of us working three times as hard."'
)
...
def build_slide3(prs):
    ...
    # "Why Fractional CTO, Not Ads" card — repurposed from the default ROI/ad-spend
    # card because this engagement has no ad spend or ROAS to project (Call-Purpose
    # Override; see section_11_workings.txt). Same box/positions, adapted labels.
    add_rect(slide, 4.96, 1.12, 4.82, 1.44, fill=ROI_BG)
    add_text(slide, "WHY FRACTIONAL CTO, NOT ADS",
             5.14, 1.18, 4.52, 0.24, 8, ROI_GREEN, bold=True)
    add_text(slide, "Transcript price match:", 5.14, 1.46, 2.10, 0.28, 9, rgb("1E293B"), bold=True)
