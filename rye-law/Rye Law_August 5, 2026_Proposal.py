...
OUTPUT_PATH    = "rye-law/Rye Law_August 5, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL", "Zero paid presence"),
    ("AMBER", "AMBER",    "Off-site form only"),
    ("AMBER", "AMBER",    "Owner entrenched"),
    ("AMBER", "AMBER",    "$50K AR uncollected"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Zero paid presence across all 5 practice areas while named rivals dominate every search"),
    ("neg", "An off-site contact form and no live chat let interested prospects vanish before converting"),
    ("neg", "Rick remains personally entrenched in daily ops with no leadership layer to fill in"),
    ("pos", "A stable 4-person team and ~350 active files already in motion, ready to scale"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Chaudhary Law Office", "~300 reviews", "4.8★ · Immigration"),
    ("Zinati Kay",           "310-474 reviews", "4.9★ · Real Estate"),
    ("Simple Divorce",       "57 reviews", "4.8★ · Same street"),
]
...
SMB_MODEL_DESC = (
    "All four growth pillars — leads, intake, team, and profit — have to work "
    "together. When they do, Rick can finally step back from the day-to-day and let Rye Law run."
)

GOAL_HEADLINE = "Referrals → Predictable Pipeline"
...
PRIORITIES = [
    (
        ...
        [
            "Launch Google Ads for immigration & family law",
            "Turn referrals into a trackable, paid pipeline",
            "Fix local SEO across all 4 GTA offices",
            "Add Meta retargeting on your existing audience",
    ...
    (
        ...
        [
            "Replace the off-site form with a native one",
            "Add after-hours coverage so no lead goes cold",
            "Build a follow-up sequence for every new lead",
            "Track every lead source for the first time",
            "Turn existing case volume into faster signings",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Document delegation for your 3 law clerks",
            "Collect the $50K in outstanding AR faster",
            "Build real visibility into case profitability",
            "Create a plan instead of reacting to cash flow",
            "Free Rick from being the firm's only engine",
        ],
    ),
]
...
CLOSING_QUOTE = (
    '"I want a business that runs independently — so I get more freedom, not more to manage."'
)
