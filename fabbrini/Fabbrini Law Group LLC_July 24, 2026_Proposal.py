...
# Report metadata
FIRM_NAME      = "Fabbrini Law Group LLC"
SALES_REP      = "Nick Holderman"
OUTPUT_PATH    = "fabbrini/Fabbrini Law Group LLC_July 24, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL", "0/6 local searches"),
    ("AMBER", "AMBER",    "No after-hours cover"),
    ("RED",   "CRITICAL", "Owner builds own tools"),
    ("RED",   "CRITICAL", "No KPI dashboard yet"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Invisible in all 6 local searches tested despite 25 years in practice"),
    ("neg", "NAP fragmented across 4+ addresses, splitting local search signals"),
    ("neg", "No ops leader — owner personally builds the firm's AI tools and dashboard"),
    ("pos", "Strong reputation: 4.7–4.8 stars across roughly 90–150 reviews"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Disparti Law Group",              "3,400+ reviews", "Dominant PI visibility"),
    ("Salvi, Schostok & Pritchard",     "200+ reviews",   "Tops every PI search"),
    ("Rosenfeld Injury Lawyers",        "Active PPC",     "Heavy paid search spend"),
]
CLIENT_REVIEWS      = "90–150 reviews"
CLIENT_REVIEWS_NOTE = "← Absent from search"
...
SMB_MODEL_DESC = (
    ...
    "together. When they do, Jim can finally step away from casework and lead the "
    "firm strategically, using real numbers instead of guesswork."
)

GOAL_HEADLINE = "Referrals → Predictable Case Flow"
GOAL_DBM      = "Step away from casework — and trust it runs"
...
PRIORITIES = [
    (
        ...
        [
            "Consolidate NAP across all listings & directories",
            "Launch Google Search Ads for PI and criminal defense",
            "Enroll in Local Service Ads for pay-per-lead cases",
            "Add Meta retargeting so no visitor is lost for good",
    ...
    (
        ...
        [
            "Add after-hours coverage so no caller goes unanswered",
            "Build a defined follow-up sequence for every new lead",
            "Track speed-to-lead so no case slips through cracks",
            "Make the contact form visible on the homepage",
            "A team that converts leads without you on every call",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Bring in a Fractional COO Director to run operations",
            "Build the KPI dashboard you're building alone",
            "Free you from being the firm's own AI tool builder",
            "Join Master's Circle for peer coaching at your stage",
            "Run the firm from the numbers, not the case files",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$15,997", "$18,997/mo",
        "Website · Local SEO · Google Ads · LSA · Meta Ads",
        "1D4ED8",
    ),
    (
        "MASTER'S CIRCLE + FCOO DIRECTOR",
        "$8,394", "$10,794/mo",
        "Fractional COO Director · Weekly coaching · Masterminds",
        "6D28D9",
    ),
]
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $7,500–$30,000/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE     = "$6,500 (default estimate)"
CONSERVATIVE_LABEL = "Conservative  (2–3 cases/mo):"
CONSERVATIVE_RESULT = "$16,900 revenue · 2.3× ROAS"
AGGRESSIVE_LABEL   = "Aggressive  (12–13 cases/mo):"
AGGRESSIVE_RESULT  = "$81,900 revenue · 2.7× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Launch Google Search & LSA campaigns for PI"),
    ("Day 14",  "Merge duplicate GBP listings & fix NAP data"),
    ("Week 2",  "Kick off Master's Circle + FCOO Director coaching"),
    ("Week 3",  "Launch Meta retargeting using existing blog content"),
    ("Month 3", "Geo-targeted landing pages live for nearby suburbs"),
]

CLOSING_QUOTE = (
    '"I want to escape daily legal work and focus on business strategy — '
    'and finally run this firm from the numbers, not the case files."'
)
