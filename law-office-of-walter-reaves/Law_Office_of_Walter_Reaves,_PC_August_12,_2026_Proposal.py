...
# Report metadata
FIRM_NAME      = "Law Office of Walter Reaves, PC"
SALES_REP      = "Randy Gold"
OUTPUT_PATH    = "law-office-of-walter-reaves/Law_Office_of_Walter_Reaves,_PC_August_12,_2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL", "LSAs paused"),
    ("RED",   "CRITICAL", "2 people, no backup"),
    ("AMBER", "AMBER",    "One-person team"),
    ("RED",   "CRITICAL", "Revenue cut in half"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Losing ~5-6 cases/month (~$20K-$24K) to missed and unfollowed calls."),
    ("neg", "Revenue cut roughly in half by a failed merger, with no profit plan guiding the rebuild."),
    ("neg", "Zero active paid lead generation - LSAs paused while competitors run active campaigns."),
    ("pos", "45 years of board-certified reputation and an Avvo 'Superb' rating support the rebuild."),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Dunham & Jones",      "Not verified", "Active ads, payment plans"),
    ("Dunnam & Dunnam",     "Not verified", "\"Best Law Firm in Waco\""),
    ("Russell D. Hunt Sr.", "6 (Avvo)",     "A+ BBB, 50+ yr veteran solo"),
]
CLIENT_REVIEWS      = "5 (Avvo, 4.8\u2605)"
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $500K+ Revenue"

SMB_MODEL_DESC = (
    "All four pillars \u2014 leads, intake, team, and profit \u2014 must work together. "
    "When they do, Walter can finally rebuild the practice without carrying every part of it personally."
)

GOAL_HEADLINE = "$250K \u2192 $500K+ revenue"
GOAL_DBM      = "Rebuild the practice, not just survive it"
...
PRIORITIES = [
    (
        ...
        [
            "Reactivate Local Service Ads - pay per lead",
            "Turn Google into a pipeline that runs without you",
            "Build Temple & Killeen pages to catch existing demand",
            "Refresh the blog across every practice area",
            "Put the Spanish FAQ library to work generating leads",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "69CD2B",
        [
            "Stop losing 5-6 cases a month to missed calls",
            "Build a follow-up process for every lead",
            "Give the overloaded assistant real backup",
            "Convert leads already coming in - no new ad spend",
            "Track every call so nothing falls through the cracks",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "003A59",
        [
            "Weekly coaching to build structure beyond Walter",
            "Get real visibility into cost per case",
            "Set a specific, trackable revenue-recovery target",
            "Build structure so the firm isn't one-person-deep",
            "Revisit AI Workforce once revenue clears $500K",
        ],
    ),
]
...
# Only one package recommended in Phase 1 — marketing is deferred to Phase 2
# per the Budget-Reality and Call-Purpose overrides (see section_11_workings.txt).
PACKAGES = [
    (
        "RECOMMENDED COACHING PACKAGE",
        "$2,600", "$3,497/mo",
        "Weekly coaching \u00b7 Practice masterminds \u00b7 Intake & financial focus",
        "003A59",
    ),
]
...
AD_SPEND_NOTE = (
    "+ Phase 2 ad spend (once stabilized): $2,000-$5,000/mo direct to Google/LSA"
)

AVG_CASE_VALUE      = "$4,000"
CONSERVATIVE_LABEL  = "Conservative (Phase 2, ~2 cases/mo):"
CONSERVATIVE_RESULT = "$6,250 revenue \u00b7 3.1\u00d7 ROAS"
AGGRESSIVE_LABEL    = "Aggressive (Phase 2+, ~11 cases/mo):"
AGGRESSIVE_RESULT   = "$42,400 revenue \u00b7 3.5\u00d7 ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Elite Coach kickoff - stabilize intake & follow-up"),
    ("Day 14",  "Build lead-tracking workflow for Walter & assistant"),
    ("Week 2",  "Set up basic monthly financial tracking"),
    ("Week 3",  "Define a repeatable follow-up sequence for leads"),
    ("Month 3", "Evaluate reactivating Local Service Ads"),
]

CLOSING_QUOTE = (
    '"A 45-year reputation like this deserves a practice built to outlast '
    'one bad year \u2014 not just survive it."'
)
