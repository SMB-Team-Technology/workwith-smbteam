...
# Report metadata
FIRM_NAME      = "Mountain State Attorneys"
SALES_REP      = "Jacob Meissner"
OUTPUT_PATH    = "mountain-state-attorneys/Mountain_State_Attorneys_August_14_2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "RED",   "LSAs paused, no SEO"),   # Lead Generation
    ("RED",   "RED",   "Josh handles it all"),   # Intake
    ("AMBER", "AMBER", "Solo + 1 paralegal"),     # Team
    ("AMBER", "AMBER", "No cost tracking"),       # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Only paid channel (LSAs) has been dark for 5+ weeks"),
    ("neg", "Josh personally handles every intake call, no delegation"),
    ("neg", "No hiring blueprint despite actively interviewing attorneys"),
    ("pos", "96% personal close rate — proven sales skill, ready to scale"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Susanne Gustin, Atty at Law", "290 reviews", "4.8★ crim. defense"),
    ("Intermountain Legal, P.C.",   "169 reviews", "family + crim overlap"),
    ("JR Law Group",                "132 reviews", "Best of SLC award"),
]
CLIENT_REVIEWS      = "Single digits"
...
SMB_MODEL_DESC = (
    ...
    "together. When all four work, Josh can finally shift from doing every case "
    "himself to building the firm he envisions."
)

GOAL_HEADLINE = "$400K → $1M+ revenue"
GOAL_DBM      = "Build a firm that runs without you"
...
PRIORITIES = [
    (
        ...
        [
            "Resume LSAs — proven $190/lead channel",
            "Stand up Google Business Profile & reviews",
            "Fix the unreviewed, 0.0-rated Avvo profile",
            "Turn daily landlord-tenant calls into revenue",
    ...
    (
        ...
        [
            ...
            "Add after-hours coverage — no live chat today",
            "Document Josh's 96% close-rate process",
            "Build a process any future hire can run",
            "Free Josh from answering every single call",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "003A59",
        [
            "Build a hiring & onboarding blueprint",
            "Track average case value & cost per case",
            "Set up a referral pipeline for declined cases",
            "Plan profit, not just revenue, toward $1M",
            "Give Josh 70-80% of his time back for growth",
        ],
    ),
]
...
PACKAGES = [
    (
        "ELITE COACH",
        "$2,600", "$3,497/mo",
        "Weekly coaching · hiring blueprint · intake process",
        "0091C9",
    ),
    (
        "FULL SERVICE MARKETING — STARTER",
        "$4,997", "$5,697/mo",
        "GBP · Website · Google Ads · LSA · Meta Ads",
        "003A59",
    ),
]
...
CONSERVATIVE_LABEL  = "Conservative  (6-7 cases/mo):"
CONSERVATIVE_RESULT = "$22.75K revenue · 4.1× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (16 cases/mo):"
AGGRESSIVE_RESULT   = "$56K revenue · 4.9× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Kick off Elite Coach — hiring blueprint begins"),
    ("Day 14",  "Resume LSAs at a controlled, measured budget"),
    ("Week 2",  "Stand up GBP, start review generation"),
    ("Week 3",  "Correct Avvo profile, fix NAP consistency"),
    ("Month 3", "Referral pipeline live for landlord-tenant calls"),
]

CLOSING_QUOTE = (
    '"Josh has already proven he can close almost every case that reaches him — '
    'the empire he envisions starts with the foundation, not more casework."'
)
