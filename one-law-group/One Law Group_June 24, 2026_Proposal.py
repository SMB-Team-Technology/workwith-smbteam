"""
Audit PowerPoint — One Law Group
SMB Team — 3-slide proposal generated from Growth, Profit, and Freedom Roadmap
"""
...
FIRM_NAME   = "One Law Group"
SALES_REP   = "Jonathan Farace"
OUTPUT_PATH = "one-law-group/One Law Group_June 24, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL",  "100% referral-only"),
    ("AMBER", "AMBER",     "No after-hours intake"),
    ("AMBER", "AMBER",     "Emanuel runs all ops"),
    ("AMBER", "AMBER",     "No cost-per-case data"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

FINDINGS = [
    ("neg", "No ads running while competitors spend $40K–$150K/mo in Beverly Hills"),
    ("neg", "120 reviews vs. Omega's 641 — invisible in 3-pack for all key PI terms"),
    ("neg", "Emanuel handles payroll, HR, accounting — 5 hrs/wk left for rainmaking"),
    ("pos", "5.0 stars, 120+ reviews — prime credentials for LSA and 3-pack ranking"),
]

COMPETITORS = [
    ("Omega Law Group",          "641 reviews", "Dominates BH 3-pack + paid search"),
    ("The Dominguez Firm",        "243+ reviews", "$40K–$150K/mo ads across LA market"),
    ("Beverly Hills Injury Firm", "119 reviews",  "Same building — same search terms"),
]
CLIENT_REVIEWS      = "~120 reviews"
CLIENT_REVIEWS_NOTE = "← You are here"

STAGE_TEXT = "Stage 4: Small Business Manager  →  Goal: Stage 6, Law Firm Owner"

# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $10M"

SMB_MODEL_DESC = (
    "Lead Generation, Intake, Team, and Profit Plan "
    "must all work together — missing any one means "
    "growth stalls no matter how hard you work. "
    "When all four run, Emanuel can focus on "
    "rainmaking and litigation — and the firm "
    "grows without him managing everything."
)

GOAL_HEADLINE = "$2.4M → $10M revenue"
GOAL_DBM      = "Be the attorney again — not the office manager"

PRIORITIES = [
    (
        "Build the", "Marketing Engine", "1D4ED8",
        [
            "Turn Google into a PI pipeline that runs 24/7",
            "Launch LSA — first position on Beverly Hills searches",
            "Close the 500-review gap; own the local 3-pack",
            "Spanish-language campaign to reach more PI prospects",
            "Revive blog — SEO content that compounds monthly",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Define 24/7 lead response protocol for PI leads",
            "Sub-5-minute response to every new web inquiry",
            "Fix NAP inconsistency — remove ranking suppression",
            "After-hours coverage so Emanuel doesn't lose cases",
            "Track contact-to-signed conversion rate weekly",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Define ops manager role — free 15+ hrs/wk for Emanuel",
            "Delegate payroll, HR, admin off Emanuel's plate",
            "Build monthly profit dashboard: margin per case type",
            "Clarify cost-per-case to protect margins at $10M",
            "Graduate to Stage 5 — firm grows without you daily",
        ],
    ),
]
...
PACKAGES = [
    ...
    (
        "ELITE COACH PLUS",
        "$3,200", "$3,497/mo",
        "Weekly coaching · PI mastermind · quarterly workshops",
        "6D28D9",
    ),
]
...
AGGRESSIVE_RESULT   = "$750,000 revenue · 19× ROAS"

TIMELINE = [
    ("Day 1",    "Launch Google Ads + LSA for Beverly Hills PI"),
    ("Day 14",   "Fix NAP, Avvo, and GBP across all directories"),
    ("Week 6",   "Intake protocol + sub-5-min response standard live"),
    ("Month 2",  "Coaching kickoff: baseline metrics + goal milestones"),
    ("Month 3",  "Campaign review: optimize top-performing ad sets"),
]

CLOSING_QUOTE = (
    '"The reputation is already built. The system to make it work at scale — that\'s what we build together."'
)
