...
# Report metadata
FIRM_NAME      = "Cooper & Cooper, P.A."
SALES_REP      = "Randy Gold"
OUTPUT_PATH    = "cooper-cooper-p-a/Cooper & Cooper, P.A._July 13, 2026_Proposal.pptx"
...
PILLARS = [
    ("AMBER", "AMBER",    "Spend up, leads flat"),   # Lead Generation
    ("AMBER", "AMBER",    "20-25% close vs 50%"),    # Intake
    ("RED",   "CRITICAL", "Stuck in casework"),      # Team
    ("RED",   "CRITICAL", "Profit is murky"),        # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Ad spend doubled to ~$20K/mo, new clients still flat at ~15/mo"),
    ("neg", "Only 20-25% of consults convert to retainers vs. a 50% goal"),
    ("neg", "Charlean stuck in casework — firm can't run without the owners"),
    ("pos", "4.9-star, 50+ review profile already beats every named competitor"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Owenby Law, P.A.",             "BBB A+ accred.", "All practice areas"),
    ("Florida Women's Law Group",    "15 reviews",     "Same agency conflict"),
    ("Head, Moss, Fulton & Griffin", "Estab. firm",    "Probate ad presence"),
]
CLIENT_REVIEWS      = "50+ reviews"
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $2.3M Revenue"

SMB_MODEL_DESC = (
    "Lead Generation, Intake, Team, and Profit must all work together — "
    "miss one and growth stalls no matter how much you spend on ads. "
    "When all four click, Anthony and Charlean can finally step away and trust the firm runs itself."
)

GOAL_HEADLINE = "$1M → $2.3M revenue"
...
PRIORITIES = [
    (
        ...
        [
            "End the conflicted-agency relationship",
            "Launch dedicated Probate/Estate ad campaigns",
            "Fix mobile site speed (47/100 today)",
            "Re-tune LSA targeting for better leads",
    ...
    (
        ...
        [
            "Qualify PPC leads on fee fit early",
            "Build a follow-up cadence before Lawmatics",
            "Lift consult-to-retainer from 20-25% to 50%",
            "Add fast, structured intake for after-hours leads",
            "Stop losing clients you already paid to reach",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Hire a senior attorney to absorb casework",
            "Free Charlean to lead, not litigate",
            "Get a bookkeeper-built profit plan live",
            "Track cost-per-case by practice area",
            "Build a firm that runs without you in October",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$7,397", "$8,997/mo",
        "Google Ads · LSA · Meta Ads · SEO · Website",
        "1D4ED8",
    ),
    (
        "MASTER'S CIRCLE",
        "$4,600", "$4,997/mo",
        "Weekly coaching · Masterminds · Leadership plan",
        "6D28D9",
    ),
]
...
CONSERVATIVE_RESULT = "$23,000 revenue · 6.6× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (37-38 cases/mo):"
AGGRESSIVE_RESULT   = "$150,000 revenue · 10.7× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Launch non-conflicted Google/LSA campaigns"),
    ("Day 14",  "Fix mobile PageSpeed & site conversion"),
    ("Week 2",  "Kick off Master's Circle coaching"),
    ("Week 3",  "Deploy PPC fee-qualification & follow-up"),
    ("Month 3", "Dedicated Probate campaigns live & converting"),
]

CLOSING_QUOTE = (
    '"Take the October vacation — and come back to a firm that never missed a beat."'
)
