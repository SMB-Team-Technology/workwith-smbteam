...
# Report metadata
FIRM_NAME      = "Burdick Law"
SALES_REP      = "Jacob Meissner"
OUTPUT_PATH    = "burdick-law/Burdick_Law_September_18_2026_Proposal.pptx"
...
PILLARS = [
    ("RED", "CRITICAL", "No website or GBP"),
    ("RED", "CRITICAL", "No intake process"),
    ("RED", "CRITICAL", "Solo, no staff"),
    ("RED", "CRITICAL", "No revenue tracked"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "No website, GBP, or online presence exists to capture private clients"),
    ("neg", "No intake process — a future inquiry could go straight to voicemail"),
    ("neg", "Solo practice — Vanessa handles every case and task alone"),
    ("pos", "70-80 cases handled in just 4 months — strong drive and capacity"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
# Competitor review counts are unconfirmed via live Google Maps — qualitative only, no exact figures.
COMPETITORS = [
    ("Spodek Law Group",      "Many reviews", "3 NYC offices · multi-domain ads"),
    ("Robert Tsigler PLLC",   "Many reviews", "Homicide page · 24/7 CTAs"),
    ("Lebedin Kofman LLP",    "Many reviews", "Dedicated homicide-defense page"),
]
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $700K-$800K in Profit"

SMB_MODEL_DESC = (
    "All four pillars must work together. Missing any one means growth "
    "stalls no matter how much you spend on marketing. When intake, team, "
    "and profit systems are in place, Vanessa can finally choose her cases."
)

GOAL_HEADLINE = "Court-appointed → $700K-$800K profit"
GOAL_DBM      = "Choose your clients — not the court's"
...
PRIORITIES = [
    (
        ...
        [
            "Launch a website with attorney bio & case results",
            "Claim and optimize a Google Business Profile",
            "Build a dedicated felony & homicide landing page",
            "Add directory listings (Avvo, Justia, FindLaw)",
            "Turn search traffic into a pipeline that runs itself",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "69CD2B",
        [
            "Build a defined intake process before ads launch",
            "Add after-hours coverage for urgent inquiries",
            "Create a follow-up sequence for every inquiry",
            "Track which inquiries convert to retained clients",
            "Never miss the case that changes everything",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "003A59",
        [
            "Make the first support hire",
            "Build a private hourly or flat-fee structure",
            "Track case value and margin from day one",
            "Add AI Workforce Pro once the first hire is made",
            "Step back from doing everything yourself",
        ],
    ),
]
...
# Only one package is recommended for Phase 1 — no marketing package (see roadmap).
PACKAGES = [
    (
        "COACH ESSENTIALS PLUS",
        "$2,497", "",
        "Group coaching · Monthly 1:1 call · Intake & fee-structure frameworks",
        "003A59",
    ),
]

BUNDLE_TOTAL   = "$2,497 / mo"
BUNDLE_SAVINGS = "Sold standalone — no bundling discount applies"

AD_SPEND_NOTE = (
    "Future ad spend (Phase 3 estimate): $5,000-$18,750/mo — not part of Phase 1"
)

AVG_CASE_VALUE      = "$25,000-$150,000 (est.)"
CONSERVATIVE_LABEL  = "Conservative (Phase 3, 1-3 cases/mo):"
CONSERVATIVE_RESULT = "$25K-$75K revenue · ~20x return"
AGGRESSIVE_LABEL    = "Aggressive (Phase 3 upside):"
AGGRESSIVE_RESULT   = "$18.75K spend · ~23x return"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Coaching kickoff begins"),
    ("Day 14",  "Intake process & follow-up sequence defined"),
    ("Week 2",  "Private fee-structure roadmap built"),
    ("Week 3",  "First support hire identified"),
    ("Month 3", "Website & GBP build scoped for Phase 3"),
]

CLOSING_QUOTE = (
    '"My vision is a self-managing firm — one where I can finally step '
    'into a hands-off, CEO-level role."'
)
