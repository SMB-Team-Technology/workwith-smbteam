...
# Report metadata
FIRM_NAME      = "Leibowitz Law Firm PLLC"
SALES_REP      = "Jonathan Farace"
OUTPUT_PATH    = "leibowitz-law-firm/Leibowitz_Law_Firm_PLLC_June_8_2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL", "Not in 3-pack"),         # Lead Generation
    ("RED",   "CRITICAL", "Broken form, no fallback"), # Intake
...
FINDINGS = [
    ("neg", "Invisible in 3-pack — fewer than 10 reviews vs. 1,207 for top competitors"),
    ("neg", "3 NAP directory errors actively suppressing local rankings — zero-cost fix"),
    ("neg", "Broken contact form sends motivated prospects to competitors before Jake knows"),
    ("pos", "16-year bilingual PI attorney with 4-county South Texas reach — rare advantage"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Carabin Shaw",          "1,207+ reviews", "4.9★ · Dominates 3-pack + paid"),
    ("George Salinas",        "414+ reviews",   "4.9★ · Paid + LSA active"),
    ("Thomas J. Henry Law",   "Statewide",      "Mega-firm · Full SA presence"),
]
CLIENT_REVIEWS      = "<10 reviews"
...
SMB_MODEL_DESC = (
    ...
    "a gap in any one pillar stalls growth regardless of ad spend. "
    "When all four are working, Jake can be fully present for his family "
    "and choose only the high-value cases worth his time."
)

GOAL_HEADLINE = "$500K → $1M revenue"
GOAL_DBM      = "Present for family — firm runs without you"
...
PRIORITIES = [
    (
        ...
        [
            "Practice area pages in English and Spanish",
            "Fix NAP in 3 directories — lift local rankings",
            "Launch review program — goal: 50+ in 6 months",
            "Claim + optimize Google Business Profile fully",
            "Turn Google into a bilingual PI case pipeline",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Repair broken contact form on leibolaw.com",
            "Add above-fold intake form to homepage",
            "Set up after-hours call capture or routing",
            "Map the intake process — no more missed leads",
            "Every contact path works before ads ever run",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Hire intake coordinator — Jake steps back",
            "Build KPI framework with FCOO Advisor coaching",
            "Track case profit to focus on highest-value work",
            "Org structure that runs without Jake in every call",
            "Systems that protect family time, not add hours",
        ],
    ),
]
...
PACKAGES = [
    (
        "WEB & SEO STARTER",
        "$4,847", "$5,697/mo",
        "Practice area pages · local SEO · GBP · review program",
        "1D4ED8",
    ),
    (
        "FCOO ADVISOR",
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $7,500/mo paid directly to Google (Phase 2)"
)

AVG_CASE_VALUE     = "$25,000"
CONSERVATIVE_LABEL = "Conservative  (2 cases/mo):"
CONSERVATIVE_RESULT = "$50,000 revenue · 6.5× ROAS"
AGGRESSIVE_LABEL   = "Aggressive  (5 cases/mo):"
AGGRESSIVE_RESULT  = "$125,000 revenue · 8× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Onboard + practice area pages begin, form fix"),
    ("Day 14",  "GBP optimized + NAP fixed across directories"),
    ("Week 2",  "Review program live + bilingual geo pages built"),
    ("Week 3",  "FCOO coaching begins — intake process mapped"),
    ("Month 3", "25+ new reviews, rankings rising, team plan set"),
]

CLOSING_QUOTE = (
    '"Jake’s expertise and bilingual reach are already the right foundation '
    'for the dominant PI firm in South Texas — the only thing missing is the '
    'digital visibility and team structure to match what he has already built."'
)
