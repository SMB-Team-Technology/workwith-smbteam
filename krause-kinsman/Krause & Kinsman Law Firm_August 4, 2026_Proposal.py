...
# Report metadata
FIRM_NAME      = "Krause & Kinsman Law Firm"
SALES_REP      = "Nick Holderman"
OUTPUT_PATH    = "krause-kinsman/Krause & Kinsman Law Firm_August 4, 2026_Proposal.pptx"
...
PILLARS = [
    ("AMBER", "AMBER", "Zero IUL presence"),
    ("AMBER", "AMBER", "Slow status updates"),
    ("AMBER", "AMBER", "Bandwidth-limited"),
    ("AMBER", "AMBER", "No case-value data"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "100,000+ existing clients get no automated status updates"),
    ("neg", "Zero social presence while 2 named rivals build IUL case volume"),
    ("neg", "No landing page or intake path exists yet for the new IUL vertical"),
    ("pos", "Reputation already leads local rivals — 4.7★, 400-700+ reviews"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("RP Legal LLC",         "400+ IUL cases",   "national IUL leader"),
    ("Stoltmann Law",        "Active in IUL",    "content-first rival"),
    ("Peterson & Associates","126-324 reviews",  "local mass tort rival"),
]
CLIENT_REVIEWS      = "400-700+ reviews"
CLIENT_REVIEWS_NOTE = "← Reviews strong here"

# Stage strip (right panel, Slide 1)
STAGE_TEXT = "Stage 5: Law Firm CEO  →  Goal: Stage 6, Law Firm Owner"

# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach 100 IUL Cases in 60 Days"

SMB_MODEL_DESC = (
    "All four pillars — marketing, intake, team, and profit — must work "
    "together. When they do, Robert and Adam can grow into new cases "
    "without losing your time back."
)

GOAL_HEADLINE = "0 → 100 IUL cases (first 60 days)"
GOAL_DBM      = "Grow into IUL — without losing your time back"
...
PRIORITIES = [
    (
        ...
        [
            "Launch paid + organic social for IUL cases",
            "Produce AI-avatar videos, no filming needed",
            "Target IUL/MLM fraud search + social terms",
            "Build content library rivals can't match",
            "Turn social into a pipeline that runs itself",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Build an IUL landing page + intake path",
            "Automate status updates for 100K+ clients",
            "Cut inbound calls with proactive video updates",
            "Route new IUL leads into an intake flow",
            "Free the team from status-call volume",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Track cost-per-case on the 100-case goal",
            "Set a profit benchmark before scaling spend",
            "Add a dedicated owner for IUL content ops",
            "Scale IUL cases without new partner hours",
            "Build a system Robert & Adam can trust",
        ],
    ),
]
...
PACKAGES = [
    (
        "OMNISOCIAL AI 360 (CONTENT + ADS)",
        "$5,444", "$5,794/mo",
        "Paid + organic social · IUL content funnel · reporting",
        "1D4ED8",
    ),
    (
        "AI AVATAR VIDEO GROWTH ADD-ON",
        "$1,950", "$1,950/mo",
        "Avatar video production · client status updates · scripts",
        "6D28D9",
    ),
]

BUNDLE_TOTAL   = "$7,394 / mo"
BUNDLE_SAVINGS = "Save $350/mo by bundling"

AD_SPEND_NOTE = (
    "+ Recommended ad spend: $25,000–$50,000/mo paid directly to Meta/social"
)

AVG_CASE_VALUE      = "Not stated — confirm w/ firm"
CONSERVATIVE_LABEL  = "Conservative ($25K/mo spend):"
CONSERVATIVE_RESULT = "~50 cases/mo target; value TBD"
AGGRESSIVE_LABEL    = "Aggressive ($50K/mo spend):"
AGGRESSIVE_RESULT   = "~50 cases/mo target; value TBD"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Kick off IUL landing page + intake build"),
    ("Day 14",  "Launch paid + organic social distribution"),
    ("Week 2",  "Ship first AI-avatar videos + update template"),
    ("Week 3",  "Begin automated updates to mass tort clients"),
    ("Month 3", "Review 60-day case count vs. cost-per-case"),
]

CLOSING_QUOTE = (
    '"We built this firm without having to be everywhere ourselves — '
    'the IUL vertical shouldn\'t be any different."'
)
