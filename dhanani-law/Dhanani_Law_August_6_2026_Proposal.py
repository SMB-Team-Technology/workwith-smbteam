...
# Report metadata
FIRM_NAME      = "Dhanani Law Firm, LLC"
SALES_REP      = "Jacob Meissner"
OUTPUT_PATH    = "dhanani-law/Dhanani_Law_August_6_2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "RED",   "Referral-only, no channel"),   # Lead Generation
    ("AMBER", "AMBER", "Form buried, unproven"),        # Intake
    ("RED",   "RED",   "Solo bottleneck"),              # Team
    ("RED",   "RED",   "No margin visibility"),         # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Rahim is the sole bottleneck — past freelance hires failed"),
    ("neg", "No financial visibility beyond top-line revenue figures"),
    ("neg", "NAP mismatch (ZIP 30329 vs 30309) is hurting local rankings"),
    ("pos", "4.7-star rating and 25+ years of reputation drive referrals"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Quijano Law",           "132 reviews", "4.9 stars"),
    ("Sharma Law / eLaw Immigration", "125 reviews", "4.9 stars"),
    ("Chavarro Law Firm",     "136 reviews", "4.9 Birdeye"),
]
...
SMB_MODEL_DESC = (
    "All four pillars must work together. Right now, coaching comes first — "
    "building the team and systems that let Rahim finally step back and "
    "work on the business, not just in it."
)

GOAL_HEADLINE = "$150K-$200K → $500K"
...
PRIORITIES = [
    (
        ...
        [
            "Fix NAP mismatch hurting local rankings",
            "Redo paid search — landing page, keywords",
            "Turn Google into a pipeline that runs itself",
            "Build pages for Marietta, Decatur, Sandy Springs",
            "Close the review gap with local competitors",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Move contact form out of the buried footer",
            "Remove newsletter form competing with the CTA",
            "Measure the answering service's real conversion",
            "Build a follow-up sequence for open leads",
    ...
    (
        ...
        [
            "Build a hiring process that doesn't repeat failures",
            "Coach delegation so cases don't all run through Rahim",
            "A team that performs without him in every case",
            "Start tracking margin, not just top-line revenue",
            "Build toward a firm that's actually sellable",
        ],
    ),
]
...
AVG_CASE_VALUE     = "$3,000-$6,000"
CONSERVATIVE_LABEL = "Conservative  (3-4 cases/mo):"
CONSERVATIVE_RESULT = "$13.5K-$18K revenue · ~5-6x ROAS"
AGGRESSIVE_LABEL   = "Aggressive  (24-25 cases/mo):"
AGGRESSIVE_RESULT  = "$108K-$112.5K revenue · ~9x ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Kick off Elite Coach — systems & hiring"),
    ("Day 14",  "Fix NAP mismatch across all directories"),
    ("Week 2",  "Move contact form, remove competing signup"),
    ("Week 3",  "Start basic profit and margin tracking"),
    ("Month 3", "Team ready — Phase 2 marketing evaluation"),
]

CLOSING_QUOTE = (
    '"A firm that runs without me, that I could be proud to hand off — '
