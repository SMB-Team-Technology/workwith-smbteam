...
Slide 1 — Where Kent Pincin Law Stands Today         (assessment overview)
Slide 2 — Your Growth Plan: 3 Priorities    (action plan)
Slide 3 — Your Investment & What's Next     (pricing + first 90 days)

Output: kent-pincin-law/Kent Pincin Law_July 22, 2026_Proposal.pptx
"""
...
# Report metadata
FIRM_NAME      = "Kent Pincin Law"
SALES_REP      = "Jacob Meissner"
OUTPUT_PATH    = "kent-pincin-law/Kent Pincin Law_July 22, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "RED",   "100% referral, no ads"),   # Lead Generation
    ("RED",   "RED",   "Turning away referrals"),  # Intake
    ("AMBER", "AMBER", "No delegation layer"),     # Team
    ("AMBER", "AMBER", "Overhead outpacing pay"),  # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Turning away referral volume due to intake capacity limits"),
    ("neg", "Zero paid marketing despite $4.8M-$6M in annual revenue"),
    ("neg", "Partners remain the single point of failure as team tripled"),
    ("pos", "26% close rate shows intake judgment is strong, not the issue"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Miracle Mile Law Group", "117 reviews", "4.4★ · 3-city pages"),
    ("The Simon Law Group",    "60 reviews",  "4.9★ · same PI market"),
    ("Conner & Associates",    "16 reviews",  "5.0★ · Redondo Beach"),
]
CLIENT_REVIEWS      = "8-11 reviews"
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $900K/Month"

SMB_MODEL_DESC = (
    "All four pillars — Lead Generation, Intake, Team, and Profit — must work "
    "together. When all four work, Michael and Emily can finally step back and "
    "become pure CEOs managing managers, instead of managing everything themselves."
)

GOAL_HEADLINE = "$400K-$500K → $900K/mo revenue"
GOAL_DBM      = "Stop being the bottleneck. Become the CEO."
...
PRIORITIES = [
    (
        ...
        [
            "Launch Google Ads + LSA for both practice areas",
            "Build geo-pages for SD, SF, Torrance, Downtown LA",
            "Close review gap vs. named competitors",
            "Consolidate two duplicate Yelp listings",
            "Turn 5-market claim into 5-market visibility",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Build a real case-assignment workflow",
            "Stop turning away referral live transfers",
            "Free Michael from handling 90% of intake",
            "Capture volume already being turned away",
    ...
    (
        ...
        [
            "Add a Fractional COO to own delegation",
            "Build structure to support 12-15 attorneys",
            "Get overhead growth under control",
            "Protect partner take-home as team scales",
            "Move toward $5M+/year per partner goal",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$15,997", "$18,997/mo",
        "Google Ads · LSA · Meta Ads · Geo-Pages · Local SEO",
        "1D4ED8",
    ),
    (
        "FRACTIONAL COO (FCOO) ADVISOR",
        "$3,297", "$3,797/mo",
        "Intake & case-assignment workflows · Group coaching",
        "6D28D9",
    ),
]

BUNDLE_TOTAL   = "$19,294 / mo"
BUNDLE_SAVINGS = "Save $3,500/mo by bundling"

AD_SPEND_NOTE = (
    "+ Recommended ad spend: $24,900–$150,000/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE     = "$5,885"
CONSERVATIVE_LABEL = "Conservative  (~20 cases/mo):"
CONSERVATIVE_RESULT = "$117,700 revenue · 4.7× ROAS"
AGGRESSIVE_LABEL   = "Aggressive  (~144 cases/mo):"
AGGRESSIVE_RESULT  = "$847,700 revenue · 5.6× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Launch Google Ads + LSA for both practice areas"),
    ("Day 14",  "Geo-pages live for SD, SF, Torrance, Downtown LA"),
    ("Week 2",  "Fractional COO kickoff begins workflow mapping"),
    ("Week 3",  "Yelp listings consolidated, review push begins"),
    ("Month 3", "Case-assignment delegation workflow in place"),
]

CLOSING_QUOTE = (
    '"Pure CEOs, managing managers — not the ones holding IT, case '
    'assignment, and intake together alone."'
)
