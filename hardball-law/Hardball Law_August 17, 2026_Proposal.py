...
# Report metadata
FIRM_NAME      = "Hardball Law"                      # FILL: full firm name
SALES_REP      = "Jacob Meissner"                    # FILL: rep's full name
OUTPUT_PATH    = "hardball-law/Hardball Law_August 17, 2026_Proposal.pptx"  # FILL: output path
...
PILLARS = [
    ("RED", "CRITICAL", "Zero paid ads ever"),      # FILL: Lead Generation
    ("RED", "CRITICAL", "No after-hours cover"),    # FILL: Intake
    ("RED", "CRITICAL", "Solo, no staff"),          # FILL: Team
    ("RED", "CRITICAL", "No revenue tracked"),      # FILL: Profit Plan
]
...
FINDINGS = [
    ("neg", "40 years of reputation wasted on zero-ROI SEO and radio spend"),
    ("neg", "NAP/brand split across directories confuses prospects and Google"),
    ("neg", "No intake team — after-hours calls often go unanswered"),
    ("pos", "40+ years of reputation, active video content, and a live blog"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
# List 3 competitors + the client row is added automatically.
COMPETITORS = [
    ("Plaxen Adler Muncy",   "Strong reviews", "Dominates PI search"),
    ("Silverman Thompson",   "Strong reviews", "Leads med-mal reputation"),
    ("Miller Stern Lawyers", "Strong reviews", "Visible for motorcycle cases"),
]
CLIENT_REVIEWS      = "Few reviews"      # FILL: client's actual review count
CLIENT_REVIEWS_NOTE = "← You are here"

# Stage strip (right panel, Slide 1)
STAGE_TEXT = "Stage 3: Solo Practitioner  →  Goal: Stage 6, Law Firm Owner"  # FILL

# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to a Selective, Predictable Caseload"  # FILL

SMB_MODEL_DESC = (
    "All four pillars — leads, intake, team, and profit — must work together. "
    "When they do, Susan can finally stay selective about her caseload."
)  # FILL: brief model description

GOAL_HEADLINE = "Referrals → Predictable Pipeline"       # FILL: e.g. "$1M → $1.8M revenue"
GOAL_DBM      = "Choose your cases — not take every one"  # FILL: DBM outcome phrase
...
PRIORITIES = [
    (
        ...
        [
            "Launch Google Search & LSA for target cases",
            "Fix the NAP/brand split across directories",
            "Activate Meta ads with existing video assets",
            "Target wrongful death, med-mal, motorcycle terms",
    ...
    (
        ...
        [
            "Add 24/7 intake coverage for PI emergencies",
            "Build a defined follow-up process for leads",
            "Confirm above-the-fold contact path on site",
            "Stop after-hours calls going to competitors",
            "Every lead captured is a case you get to choose",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "003A59",
        [
            "Start Elite Coach Plus systems & accountability",
            "Track revenue and case value for the first time",
            "Document a process a future team can run",
            "Build toward FCOO support as lead flow grows",
            "See your numbers clearly enough to plan freedom",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$4,997", "$5,697/mo",
        "Google Search · LSA · Meta Ads · Local SEO/GBP optimization",
        "0091C9",
    ),
    (
        "ELITE COACH PLUS",
...
BUNDLE_TOTAL   = "$8,197 / mo"                      # FILL: sum of bundled prices
BUNDLE_SAVINGS = "Save $997/mo by bundling"        # FILL

AD_SPEND_NOTE = (
    "+ Recommended ad spend: $7,500–$20,000/mo paid directly to Google/Meta"
)  # FILL

AVG_CASE_VALUE     = "$5,000–$8,000"                         # FILL
CONSERVATIVE_LABEL = "Conservative  (2 cases/mo):"   # FILL
CONSERVATIVE_RESULT = "$12,000 revenue · 1.6× ROAS"  # FILL
AGGRESSIVE_LABEL   = "Aggressive  (6 cases/mo):"     # FILL
AGGRESSIVE_RESULT  = "$39,000 revenue · 2.0× ROAS"   # FILL

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Launch Google Search & LSA campaigns"),
    ("Day 14",  "Fix NAP/brand split across directories"),
    ("Week 2",  "Kick off Elite Coach Plus coaching sessions"),
    ("Week 3",  "Confirm PageSpeed & mobile performance"),
    ("Month 3", "Launch Meta video & retargeting campaigns"),
]

CLOSING_QUOTE = (
    '"A steady flow of quality PI cases, allowing for selective case intake."'
)  # FILL
