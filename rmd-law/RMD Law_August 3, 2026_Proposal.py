...
OUTPUT_PATH    = "rmd-law/RMD Law_August 3, 2026_Proposal.pptx"
...
PILLARS = [
    ("AMBER", "AMBER", "$50K/mo on Yelp Ads"),      # Lead Generation
    ("AMBER", "AMBER", "28% conv., no funnel view"), # Intake
    ("AMBER", "AMBER", "Owner still decides mix"),   # Team
    ("RED",   "CRITICAL", "No revenue figure given"), # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Signings dropped from 50/mo to 30/mo despite steady $80K/mo spend"),
    ("neg", "Invisible in the local 3-pack in Sacramento, San Diego, San Bernardino, Austin"),
    ("neg", "LSAs underperforming — aged accounts, poor ranking, low-quality leads"),
    ("pos", "Irvine review footprint (251) already beats every named local competitor"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Demas Law Group",     "668 reviews",   "Dominates Sacramento pack"),
    ("Thomas J. Henry",     "3,083 reviews", "Dominant Austin brand"),
    ("The Dominguez Firm",  "28 reviews",    "Same building, San Bernardino"),
]
CLIENT_REVIEWS      = "251 reviews"
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach 50+ Signings/Month"

SMB_MODEL_DESC = (
    "All four pillars — marketing, intake, team, and profit — have to work "
    "together. When they do, Michael can finally step back from deciding "
    "channel mix and market entry himself, and let each new city run on a system."
)

GOAL_HEADLINE = "30 → 50+ cases/mo"
GOAL_DBM      = "Every new market runs itself, not through Michael"
...
PRIORITIES = [
    (
        ...
        [
            "Rebuild the aged, underperforming LSA account",
            "Launch coordinated Google + Meta ads, 5 markets",
            "Fix NAP consistency across every directory",
            "Win the Sacramento & San Bernardino local pack",
            "Turn ad spend into a system, not a guess",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Track leads before they become “qualified”",
            "Add after-hours coverage across 2 time zones",
            "Find why signings dropped from 50 to 30/mo",
            "Build a full-funnel view, not just 28% conv.",
            "Recover cases without spending another dollar",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Add a Fractional COO Director for multi-market ops",
            "Kick off Master's Circle group coaching",
            "Build profit-per-case-type visibility",
            "Resolve the employment law site decision",
            "Let new markets run without Michael's input",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$10,497", "$12,497/mo",
        "SEO · Google Ads · LSA rebuild · Meta Ads, 5 markets",
        "1D4ED8",
    ),
    (
        "MASTER'S CIRCLE + FCOO DIRECTOR",
        "$8,394", "$10,794/mo",
        "Group coaching · masterminds · dedicated Ops Director",
        "6D28D9",
    ),
]
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $19,500–$54,000/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE      = "$5,000–$8,000"
CONSERVATIVE_LABEL  = "Conservative  (~5 cases/mo):"
CONSERVATIVE_RESULT = "$32,500 revenue · 1.7× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (~18 cases/mo):"
AGGRESSIVE_RESULT   = "$117,000 revenue · 2.2× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Kick off LSA rebuild + coordinated ad launch"),
    ("Day 14",  "NAP consistency fixed across directories"),
    ("Week 2",  "Master's Circle coaching + FCOO Director begin"),
    ("Week 3",  "Full-funnel intake tracking stood up"),
    ("Month 3", "Mobile fixes live, 5-market SEO push underway"),
]

CLOSING_QUOTE = (
    '"Every city we enter should run itself — not go through me."'
)
