...
# Report metadata
FIRM_NAME      = "CourtLaw Injury Lawyers"
SALES_REP      = "Randy Gold"
OUTPUT_PATH    = "courtlaw-injury-lawyers/CourtLaw Injury Lawyers_July 28, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "RED",   "5/month, was 5/day"),   # Lead Generation
    ("RED",   "RED",   "No bilingual intake"),  # Intake
...
FINDINGS = [
    ("neg", "Intake collapsed from ~5/day at peak to ~5/month, with no system to reverse it"),
    ("neg", "Karim alone handles every financial decision — no profit visibility"),
    ("neg", "~90% of clients are Spanish-speaking with no bilingual intake staff"),
    ("pos", "~203 Google reviews at 4.6 stars — beats every named local competitor"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Lombardi & Lombardi",  "28-42 reviews", "3.1-4.1★ · 10 mi away"),
    ("Bramnick Rodriguez",   "1 review",      "Statewide brand name"),
    ("Team Law",             "No indexed reviews", "60+ yrs, same city"),
]
CLIENT_REVIEWS      = "~203 reviews"
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Rebuild to $5M/Year"

SMB_MODEL_DESC = (
    "All four pillars — leads, intake, team, and profit — have to work together, "
    "or growth stalls no matter how much you spend on ads. When all four are working, "
    "Karim can finally step back from carrying the firm alone."
)

GOAL_HEADLINE = "$5M peak → a predictable $5M/yr"
GOAL_DBM      = "A firm that runs itself, not just survives"
...
PRIORITIES = [
    (
        ...
        [
            "Launch Meta ads — no negative history here",
            "Add Local Service Ads, lead with your reviews",
            "Rebuild Google Ads around rideshare + Spanish",
            "Amplify geo-pages already built and ranking",
    ...
    (
        ...
        [
            "Hire bilingual intake for 90% of your clients",
            "Build a defined follow-up sequence for leads",
            "Add after-hours coverage for rideshare calls",
            "Convert more of the 5 leads you get today",
            "Stop losing cases you already paid to generate",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Build financial visibility Karim's never had",
            "Coach a delegation layer below Karim",
            "Track cost-per-case and true margins",
            "Plan for the next downturn before it hits",
            "Let the team carry weight, not just Karim",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$15,997", "$18,997/mo",
        "LSA · Meta Ads · Rebuilt Google Ads · Local SEO",
        "1D4ED8",
    ),
    (
        "ELITE COACH PLUS",
        "$3,200", "$3,497/mo",
        "1:1 coaching · financial visibility · delegation",
        "6D28D9",
    ),
]
...
CONSERVATIVE_RESULT = "$45,000 revenue · 4.5× ROAS"
AGGRESSIVE_LABEL   = "Aggressive  (16 cases/mo):"
AGGRESSIVE_RESULT  = "$120,000 revenue · 3.0× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Launch Meta retargeting + lead-gen campaigns"),
    ("Day 14",  "Local Service Ads live, reviews front and center"),
    ("Week 2",  "Rebuilt Google Ads: rideshare + Spanish launch"),
    ("Week 3",  "Geo-pages optimized, intake sequence in place"),
    ("Month 3", "Coaching builds Karim's first delegation layer"),
]

CLOSING_QUOTE = (
    '"Forty years built this firm\'s name. The next forty shouldn\'t rest on me alone."'
)
