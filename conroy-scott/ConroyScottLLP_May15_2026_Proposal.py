...
# Report metadata
FIRM_NAME      = "Conroy Scott LLP"
SALES_REP      = "Jacob Meissner"
OUTPUT_PATH    = "conroy-scott/ConroyScottLLP_May15_2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL",  "Ads on wrong practice area"),  # Lead Generation
    ("AMBER", "IN PROGRESS", "No tracking or process"),    # Intake
    ("AMBER", "IN PROGRESS", "Adam is the bottleneck"),    # Team
    ("AMBER", "IN PROGRESS", "No margin visibility"),      # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Ads driving W&E leads at full capacity — corporate & real estate targets get nothing"),
    ("neg", "Adam generates 50%+ of revenue personally, capping growth at his personal hours"),
    ("neg", "27 reviews for a 68-year-old firm limits local pack ranking across all channels"),
    ("pos", "4.6-star rating is the highest of any verified local competitor — ready to amplify"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Weaver Simmons LLP",        "~169 reviews", "dominant · 30+ lawyers"),
    ("B&B Law",                   "26 reviews",   "5.0 stars · direct competitor"),
    ("Miller Maki LLP",           "16 reviews",   "lower rating · real estate"),
]
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $2.5M Revenue"

SMB_MODEL_DESC = (
    "All four pillars must work together. "
    "Missing any one means growth stalls — and Adam stays the bottleneck."
)

GOAL_HEADLINE = "$1.25M → $2.5M revenue"
GOAL_DBM      = "Adam steps back from operations"
...
PRIORITIES = [
    (
        ...
        [
            "Redirect ads to corporate & real estate now",
            "Rebuild website with practice area pages",
            "Activate Google LSA — Google Screened",
            "Meta Ads targeting Sudbury business owners",
            "Review pipeline — target 75+ reviews fast",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Move consultation CTA above the fold",
            "Make contact form visible on homepage",
            "Implement lead source attribution tracking",
            "Train intake coordinator — respond in 1 hr",
            "Build follow-up for unconverted leads",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Build org chart — delegate first ops layer",
            "FCOO Advisor designs accountability system",
            "Document corporate GC intake & onboarding",
            "Track profit margin by practice area",
            "Price corporate retainer model correctly",
        ],
    ),
]
...
PACKAGES = [
    ...
    (
        "MASTER'S CIRCLE + FCOO ADVISOR",
        "$6,694", "$8,794/mo",
        "Coaching · FCOO ops support · Masterminds · Workshops",
        "6D28D9",
    ),
]

BUNDLE_TOTAL   = "$14,091 / mo"
BUNDLE_SAVINGS = "Save $3,700/mo by bundling"

AD_SPEND_NOTE = (
    "+ Recommended ad spend: $10,200–$22,000/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE      = "~$2,000"
CONSERVATIVE_LABEL  = "Conservative  (13 cases/mo):"
CONSERVATIVE_RESULT = "$26,000 revenue · 2.5x ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (33 cases/mo):"
AGGRESSIVE_RESULT   = "$66,000 revenue · 3.0x ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",    "Ads redirected — corporate & real estate live"),
    ("Week 2",   "Website rebuild + landing pages underway"),
    ("Week 4",   "Google Screened LSA activated"),
    ("Week 6",   "Coaching kickoff — org chart design begins"),
    ("Month 3",  "75+ reviews live; intake process documented"),
]

CLOSING_QUOTE = (
    '"The corporate and real estate clients Conroy Scott wants are searching online right now '
    '— and finding someone else, because the firm built for referrals has not yet built the engine to meet them."'
)
