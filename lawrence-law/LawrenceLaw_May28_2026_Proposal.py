...
Output: lawrence-law/LawrenceLaw_May28_2026_Proposal.pptx
"""
...
# Report metadata
FIRM_NAME   = "Lawrence Law LLC"
SALES_REP   = "Dan Bryant"
OUTPUT_PATH = "lawrence-law/LawrenceLaw_May28_2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL",  "No ads on any channel"),    # Lead Generation
    ("AMBER", "AMBER",     "Contact page 404 error"),   # Intake
    ("AMBER", "AMBER",     "Kate is sole rainmaker"),   # Team
...
FINDINGS = [
    ("neg", "Website contact page returns 404 — no inbound lead can reach the firm online"),
    ("neg", "No paid search, LSA, or Meta ads — invisible for every relevant Baltimore search"),
    ("neg", "No verified Google reviews — B2B litigation local pack is entirely unclaimed"),
    ("pos", "NAMWOLF & WBENC certifications open diversity channels competitors cannot access"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Silverman Thompson",         "113 reviews", "4.8★ · PI/family law"),
    ("Whiteford Taylor & Preston", "7 reviews",   "4.4★ · Tier 1 constr. law"),
    ("Kramon & Graham",            "7 reviews",   "4.4★ · Chambers litigation"),
]
CLIENT_REVIEWS      = "Not verified"
CLIENT_REVIEWS_NOTE = "← GBP unconfirmed"
...
SMB_MODEL_DESC = (
    ...
    "Missing any one stalls growth regardless of effort. "
    "When all four work for Lawrence Law, Kate can step back from chasing "
    "every new client relationship — and let the system do it."
)

GOAL_HEADLINE = "$3M → $6M revenue"
GOAL_DBM      = "A firm that grows — without you opening every door"
...
PRIORITIES = [
    (
        ...
        [
            "Fix the 404 contact page — every lead can reach you",
            "Launch Google Ads for construction + employment law",
            "Claim LSA top spot — pay per call, not per click",
            "Build 30+ GBP reviews — own the Baltimore map pack",
            "Turn NAMWOLF certs into a unique paid search channel",
        ],
    ),
    (
        ...
        [
            ...
            "Cover after-hours web leads before rivals get the call",
            "Track every inbound lead from first touch to retainer",
            "Convert warm website traffic before they go elsewhere",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Build BD system that runs without Kate's network",
            "Document NAMWOLF outreach as a repeatable process",
            "Track profit by practice area — know what earns most",
            "Set partner KPIs so all three partners own growth",
            "Coaching cadence that outlasts any partner's bandwidth",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$10,497", "$12,497/mo",
        "Website rebuild · Google Ads · LSA · Meta · SEO · GBP",
        "1D4ED8",
    ),
    (
        "MASTER'S CIRCLE",
        "$4,600", "$4,997/mo",
        "Weekly coaching · Masterminds · Workshops · Partner KPIs",
        "6D28D9",
    ),
]
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $10,200–$50,000/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE      = "$5,000+ (confirm on call)"
CONSERVATIVE_LABEL  = "Conservative  (~13 cases/mo):"
CONSERVATIVE_RESULT = "~$65,000 revenue · ~6.4× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (~76 cases/mo):"
AGGRESSIVE_RESULT   = "~$380,000 revenue · ~7.6× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",    "Google Ads live — construction + employment law"),
    ("Day 14",   "Contact page fixed + above-fold form live"),
    ("Week 2",   "GBP optimized + review acquisition campaign starts"),
    ("Week 3",   "LSA application + Google Screened submitted"),
    ("Month 3",  "20+ GBP reviews + first pipeline report delivered"),
]

CLOSING_QUOTE = (
    "“The credentials are built. The systems are what’s missing — "
    "and when we fix that, Lawrence Law grows without you chasing it.”"
)
