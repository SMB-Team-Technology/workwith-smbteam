"""
Audit PowerPoint — Charles Chase
...
# Report metadata
FIRM_NAME   = "Charles Chase"
SALES_REP   = "Randy Gold"
OUTPUT_PATH = "charles-chase/CharlesChase_June12_2026_Proposal.pptx"

# Optional images
WEBSITE_SCREENSHOT_PATH = None

# ── Slide 1 ──────────────────────────────────────────────────────
# Copied from section_05_growth_health.html urgency-score
URGENCY_SCORE = "8"

# Pillar status — (status, label, one-line detail)
# Copied from section_05 sc-cards: all four are RED
PILLARS = [
    ("RED", "CRITICAL", "No website, GBP, or reviews"),
    ("RED", "CRITICAL", "No intake process exists"),
    ("RED", "CRITICAL", "Solo — no staff of any kind"),
    ("RED", "CRITICAL", "Near-zero revenue, no plan"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — copied from section_executive_summary.html top priorities
FINDINGS = [
    ("neg", "No owned website, GBP, or directories — invisible in every digital channel"),
    ("neg", "200 daily ad visitors, zero leads — vendor targeted health insurance keywords"),
    ("neg", "3 referral cases in 2 months; zero inbound digital leads ever received"),
    ("pos", "Focused workers' comp practice = highly targetable, intent-driven ad strategy"),
]

# Competitor table — copied from section_06_lead_generation.html and research notes
COMPETITORS = [
    ("Masnikoff Associates",          "34 Avvo reviews", "27 yrs · multilingual · dominant SEO"),
    ("Winer Law Group",               "63 Avvo reviews", "4.7★ Google · 30 yrs licensed"),
    ("Benenfeld / Injury Law Service","85 Avvo reviews", "32 yrs · active PPC campaigns"),
]
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $250K in 12 Months"

SMB_MODEL_DESC = (
    "Lead generation, intake, team, and profit must all work together — "
    "none of them alone is enough. "
    "When all four pillars work, Charles Chase can build the firm income "
    "and the freedom that makes the life he is building toward actually possible."
)

GOAL_HEADLINE = "$250K revenue goal — Year 1"
GOAL_DBM      = "A firm that funds the life in Portugal"

# Priority columns — derived from quick wins and section 06–09 findings
PRIORITIES = [
    (
        "Build the", "Marketing Engine", "1D4ED8",
        [
            "Build a firm-owned workers' comp website",
            "Launch Google Ads on injured worker terms",
            "Resolve GBP — unlock local search + LSA",
            "Claim Avvo, Justia, FindLaw directories",
            "Turn Google into your client pipeline",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Build intake process before first paid lead",
            "Add contact forms + 24/7 lead capture",
            "Define qualifier script for injured workers",
            "Set up CRM lead tracking from day one",
            "Stop losing leads to after-hours voicemail",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Track cost per lead and cost per case",
            "Build profit plan toward $1M+ annual goal",
            "Hire intake coordinator at 5+ cases/month",
            "Free attorney time for casework, not calls",
            "Build the team layer that makes Portugal real",
        ],
    ),
]

# ── Slide 3 ──────────────────────────────────────────────────────
# Package cards — prices copied exactly from section_11 investment grid
PACKAGES = [
    (
        "FULL SERVICE MARKETING — STARTER",
        "$4,847", "$5,697/mo",
        "New website · Google Ads · GBP setup · local SEO · directories",
        "1D4ED8",
    ),
    (
        "ELITE COACH",
        "$1,600", "$3,497/mo",
        "Weekly coaching · practice masterminds · intake framework",
        "6D28D9",
    ),
]

# Copied from section_11 investment-total and investment-savings
BUNDLE_TOTAL   = "$6,447 / mo"
BUNDLE_SAVINGS = "Save $2,747/mo by bundling"

# Copied from section_11 Block 4 ad spend row
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $3,000–$10,000/mo paid directly to Google/LSA/Meta"
)

# Copied from section_11 Block 4 ROI table
AVG_CASE_VALUE      = "$2,500"
CONSERVATIVE_LABEL  = "Conservative  (3 cases/mo):"
CONSERVATIVE_RESULT = "$7,500 revenue · 2.5× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (15 cases/mo):"
AGGRESSIVE_RESULT   = "$37,500 revenue · 3.75× ROAS"

# Timeline — derived from section_11 Block 5 first 90 days
TIMELINE = [
    ("Day 1",    "Coaching kickoff + intake workflow defined"),
    ("Day 14",   "Google Ads live on workers' comp keywords"),
    ("Week 4",   "New website live with Fort Lauderdale pages"),
    ("Week 6",   "GBP verified — local search + LSA opened"),
    ("Month 3",  "Directories claimed + review profile building"),
]

# Closing quote — vivid DBM synthesis in the owner's voice
CLOSING_QUOTE = (
    '"A firm that replaces $400K in income and runs without me '
    'while I\'m in Portugal — that\'s what I\'m building."'
)
