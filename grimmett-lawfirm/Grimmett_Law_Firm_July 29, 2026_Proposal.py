...
# Report metadata
FIRM_NAME      = "Grimmett Law Firm"
SALES_REP      = "Jacob Meissner"
OUTPUT_PATH    = "grimmett-lawfirm/Grimmett_Law_Firm_July 29, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL", "~50% referral fee"),      # Lead Generation
    ("RED",   "CRITICAL", "20% of calls unanswered"),  # Intake
    ("AMBER", "AMBER",    "Sized for 5, not 10"),     # Team
    ("AMBER", "AMBER",    "Margin at risk per case"),  # Profit Plan
]
...
FINDINGS = [
    ("neg", "Nearly all new clients come from one referral source at a ~50% fee"),
    ("neg", "~52-60 of 260-300 weekly inbound calls go unanswered every week"),
    ("neg", "GBP and Yelp category errors suppress visibility for your core practice"),
    ("pos", "17 years of trial experience and a 4.4-star reputation to build on"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
# List 3 competitors + the client row is added automatically.
COMPETITORS = [
    ("Griffith Law",       "471 reviews", "Est. 2002 · ranks for PI Franklin"),
    ("The Williams Firm",  "81 reviews",  "5.0 stars · Nashville PI searches"),
    ("Puryear Newman & Morton", "66 reviews", "Est. 1997 · multi-practice depth"),
]
CLIENT_REVIEWS      = "27 reviews"
CLIENT_REVIEWS_NOTE = "← You are here"

# Stage strip (right panel, Slide 1)
STAGE_TEXT = "Stage: Skilled Lawyer, Owner-Carried  →  Goal: $10M, 10-Lawyer Firm Owner"
...
SMB_MODEL_DESC = (
    ...
    "Missing any one caps growth no matter how hard the others work. "
    "When all four pillars run without you carrying every case yourself, "
    "David can finally lead a firm instead of personally carrying it."
)

GOAL_HEADLINE = "$900K → $10M revenue"
GOAL_DBM      = "A firm that runs on systems, not just on David"
...
PRIORITIES = [
    (
        ...
        [
            "Fix GBP category: General Law → Personal Injury",
            "Resolve Yelp category and Franklin/Nashville NAP",
            "Launch Google Ads for PI, Bad Faith, Custody terms",
            "Turn ads into a pipeline that runs without a referral",
            "Revive the stalled Bad Faith blog content",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Close the 20% unanswered weekly call gap",
            "Move the contact form above the fold",
            "Confirm mobile site speed isn't costing leads",
            "Stand up an intake process before ad leads arrive",
            "Capture every caller you've already paid to reach",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Begin Elite Coach Plus leadership coaching",
            "Build a case-manager layer ahead of 10 lawyers",
            "Build a profit plan around real case economics",
            "Protect the 15% margin goal as revenue grows",
            "Build a team David can lead instead of carry",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$4,847", "$5,697/mo",
        "Google Ads · Local SEO · GBP fix · directory cleanup · content",
        "1D4ED8",
    ),
    (
        "ELITE COACH PLUS",
...
AVG_CASE_VALUE     = "$2,500–$5,000"
CONSERVATIVE_LABEL = "Conservative  (2-3 cases/mo):"
CONSERVATIVE_RESULT = "$6,000-$12,000 revenue · 0.8-1.6× ROAS"
AGGRESSIVE_LABEL   = "Aggressive  (9-10 cases/mo):"
AGGRESSIVE_RESULT  = "$24,000-$48,000 revenue · 1.0-1.9× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Launch Google Ads for PI, Bad Faith, Custody"),
    ("Day 14",  "Fix GBP, Yelp, and Franklin/Nashville NAP"),
    ("Week 2",  "Move contact form above the fold, check mobile"),
    ("Week 3",  "Stand up intake process to close call gap"),
    ("Month 3", "Elite Coach Plus builds your leadership plan"),
]

CLOSING_QUOTE = (
    '"Every referral proved you can win the case. Now build the firm '
    'that wins the client on its own terms."'
)
