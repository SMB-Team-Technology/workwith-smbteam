"""
Audit PowerPoint — Santoni, Vocci & Ortega LLC
...
# ═══════════════════════════════════════════════════════════════════
# FILL — All placeholders filled below
# ═══════════════════════════════════════════════════════════════════

FIRM_NAME   = "Santoni, Vocci & Ortega LLC"
SALES_REP   = "Jacob Meissner"
OUTPUT_PATH = "santoni-vocci-ortega/Santoni_Vocci_Ortega_LLC_July_6_2026_Proposal.pptx"
...
PILLARS = [
    ...
    ("RED",   "CRITICAL", "No after-hours coverage"),
    ("AMBER", "AMBER",    "Partners absorb all ops"),
    ("AMBER", "AMBER",    "No profit framework"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

FINDINGS = [
    ("neg", "No paid ads or LSA — PI leads route to Leppler and Portner & Shure daily"),
    ("neg", "No after-hours intake system — ad leads lost before morning response"),
    ("neg", "NAP address gap across directories suppresses both office profiles"),
    ("pos", "102 Google reviews at 4.6 stars + Super Lawyers credentials for all 3 partners"),
]

COMPETITORS = [
    ("Leppler Injury Law",    "407 reviews", "5.0 stars · PI 3-pack leader"),
    ("Portner & Shure, P.A.", "133+ reviews", "Multi-office · active PPC"),
    ("Whitney, LLP",          "15 Yelp reviews", "Consumer rights · est. 1992"),
]
CLIENT_REVIEWS      = "102 reviews"
CLIENT_REVIEWS_NOTE = "← You are here"

STAGE_TEXT = "Stage 4: Small Business Manager  →  Goal: Stage 6, Law Firm Owner"

# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $200K/Month"

SMB_MODEL_DESC = (
    "Lead Generation, Intake, Team, and Profit Plan must all work "
    "together or growth stalls. When all four run in sync, "
    "Santoni, Vocci & Ortega reaches more of the clients who need "
    "them — without Jane, Matthew, or Chelsea personally driving "
    "every new client relationship."
)

GOAL_HEADLINE = "$1.2M  →  $2.4M revenue"
GOAL_DBM      = "Reach more clients — without more hours from the partners"

PRIORITIES = [
    (
        "Build the", "Marketing Engine", "1D4ED8",
        [
            "Turn Google into an inbound client pipeline",
            "Launch LSA — pay only per verified PI call",
            "First-mover paid ads in consumer rights niche",
            "Close the 305-review gap with Leppler",
            "Build city pages for the Baltimore metro area",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Automated after-hours response for web leads",
            "Speed-to-lead: respond in under 5 minutes",
            "Define intake script and follow-up sequence",
            "Hire or designate dedicated intake coordinator",
            "Track lead-to-client rate by marketing channel",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Separate partner roles from operational work",
            "Set revenue targets and profit distributions",
            "Build team accountability with KPI scorecards",
            "Track cost per case by practice area",
            "Scale firm capacity without scaling partner hours",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$7,397", "$7,997/mo",
        "Website rebuild · Google Ads · LSA · Meta Ads · GBP optimization",
        "1D4ED8",
    ),
    (
        "ELITE COACH PLUS",
        "$3,200", "$3,497/mo",
        "Weekly coaching · Intake framework · Profit plan · Team structure",
        "6D28D9",
    ),
]
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $10,000–$15,000/mo paid directly to Google/Meta"
)
...
AGGRESSIVE_RESULT   = "~$275,000 revenue · 18× ROAS"

TIMELINE = [
    ("Day 1",    "Launch consumer rights & PI Google Ads"),
    ("Day 14",   "Enroll in LSA for PI and consumer rights"),
    ("Week 2",   "Website rebuild: homepage CTA + city pages"),
    ("Week 3",   "NAP audit: fix address inconsistency across dirs"),
    ("Month 3",  "Coaching kickoff: intake protocol + profit targets"),
]

CLOSING_QUOTE = (
    "“Ten years of fighting for clients who had no one else to call. "
    "The only thing that changes now is how many of them can find you.”"
)
