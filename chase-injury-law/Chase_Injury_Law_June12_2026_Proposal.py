...
# ═══════════════════════════════════════════════════════════════════
# FILL — Chase Injury Law  |  June 12, 2026  |  Rep: Randy Gold
# ═══════════════════════════════════════════════════════════════════

# Report metadata
FIRM_NAME      = "Chase Injury Law"
SALES_REP      = "Randy Gold"
OUTPUT_PATH    = "chase-injury-law/Chase_Injury_Law_June12_2026_Proposal.pptx"

# Optional images
WEBSITE_SCREENSHOT_PATH = None

# ── Slide 1 ──────────────────────────────────────────────────────
URGENCY_SCORE = "9"  # copied from section_05 urgency-score

# Pillars — copied from section_05 sc-card traffic lights
PILLARS = [
    ("RED",   "CRITICAL", "No live GBP"),         # Lead Generation
    ("RED",   "CRITICAL", "No intake process"),   # Intake
    ("RED",   "CRITICAL", "Solo — zero staff"),   # Team
    ("RED",   "CRITICAL", "Pre-revenue startup"), # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Findings — 3 neg from exec-summary Top Priorities; 1 pos from section_05 Positive
FINDINGS = [
    ("neg", "GBP stuck since launch — invisible in every local search"),
    ("neg", "Google Ads: 200 daily visitors and zero qualified leads"),
    ("neg", "Solo op from Portugal with no intake system or team"),
    ("pos", "Strong brand: 10+ yrs, former prosecutor, compelling story"),
]

# Competitors — copied from section_06 competitor research
COMPETITORS = [
    ("Anidjar & Levine",       "1,696 reviews", "Fort Lauderdale dominant"),
    ("Rosenberg & Rosenberg",  "957 reviews",   "Broward County coverage"),
    ("Lyle B. Masnikoff",      "5.0 stars",     "Workers' comp PPC specialist"),
]
CLIENT_REVIEWS      = "0 reviews"
CLIENT_REVIEWS_NOTE = "← You are here"

# Stage strip — copied from section_03
STAGE_TEXT = "Stage 3: Solo Practitioner  →  Goal: Stage 6, Law Firm Owner"

# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $250K in Year One"

SMB_MODEL_DESC = (
    "Lead generation, intake, team, and profit plan — "
    "all four must work together. "
    "When they do, Michelle can generate real income from Portugal "
    "without managing the firm hour by hour."
)

GOAL_HEADLINE = "$0 → $250K — Year One Goal"
GOAL_DBM      = "Income that works from Portugal"

# Priority columns — firm-specific bullets ≤55 chars each
PRIORITIES = [
    (
        "Build the", "Marketing Engine", "1D4ED8",
        [
            "Fix Google Ads — workers' comp keywords only",
            "Resolve GBP: unlock 3-pack and LSA together",
            "Launch LSA: top of Google, pay per real call",
            "Turn Google into a lead pipeline from Portugal",
            "Build reviews — close the gap with competitors",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Embed contact form above fold on homepage",
            "Auto-confirm every inquiry — no gaps at night",
            "Intake script: qualify leads without you",
            "Follow-up sequence: no lead falls through",
            "Close the Portugal time zone gap for good",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Fix NAP: one city, one signal to Google",
            "Hire intake coordinator — remove the bottleneck",
            "Break-even model: know your monthly target",
            "Track cost-per-case by channel every month",
            "Build the plan that sustains life in Portugal",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$3,397", "N/A",
        "Google Ads · LSA · Local SEO · Website optimization",
        "1D4ED8",
    ),
    (
        "ELITE COACH",
        "$2,600", "$3,497/mo",
        "Weekly coaching · Intake system · Break-even modeling",
        "6D28D9",
    ),
]
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $3,000–$7,500/mo paid directly to Google/LSA"
)

AVG_CASE_VALUE      = "$25,000"
CONSERVATIVE_LABEL  = "Conservative  (1 case/mo):"
CONSERVATIVE_RESULT = "$25,000 revenue · 8× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (3 cases/mo):"
AGGRESSIVE_RESULT   = "$75,000 revenue · 10× ROAS"

# Timeline — first 90 days from section_11 Block 5
TIMELINE = [
    ("Day 1",    "Google Ads launch — workers' comp keywords"),
    ("Week 2",   "GBP verification resolved; LSA eligibility unlocked"),
    ("Week 3",   "NAP fixed; homepage contact form moved above fold"),
    ("Month 2",  "Intake system + after-hours coverage protocol built"),
    ("Month 3",  "Break-even model complete; case targets set"),
]

CLOSING_QUOTE = (
    '"A firm that rings your phone in Portugal — that\'s exactly what we\'re building."'
)
