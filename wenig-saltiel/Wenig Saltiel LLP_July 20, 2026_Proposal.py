...
# Report metadata
FIRM_NAME      = "Wenig Saltiel LLP"
SALES_REP      = "Jacob Meissner"
OUTPUT_PATH    = "wenig-saltiel/Wenig Saltiel LLP_July 20, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL", "Referrals only, no ads"),
    ("AMBER", "AMBER",    "Phone-first, no chat"),
    ("RED",   "CRITICAL", "Owner hand-reviews billing"),
    ("AMBER", "AMBER",    "Manual legacy billing"),
]
...
FINDINGS = [
    ("neg", "Jeffrey personally hand-reviews a 3-inch stack of paper pre-bills, 3 hours every two weeks, with zero automation"),
    ("neg", "Document assembly and billing inside the firm's legacy ProLaw system remain fully manual across every matter"),
    ("neg", "Rosenberg & Estis and Belkin Burden Goldman both outrank the firm in Google reviews and ratings in the same Manhattan practice areas"),
    ("pos", "25+ years of reputation built entirely on referrals — zero paid marketing has ever been needed to keep the firm busy"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
# List 3 competitors + the client row is added automatically.
COMPETITORS = [
    ("Rosenberg & Estis, P.C.",      "57 reviews",    "4.6★ · same practice areas"),
    ("Belkin Burden Goldman, LLP",   "52-53 reviews", "4.7★ · ~60-attorney firm"),
    ("Adam Leitman Bailey, P.C.",    "0 reviews",     "Aggressive content marketing"),
]
CLIENT_REVIEWS      = "39 reviews"
CLIENT_REVIEWS_NOTE = "← You are here"

# Stage strip (right panel, Slide 1)
STAGE_TEXT = "Stage 3: Referral-Built Reputation  →  Goal: Time back, not a bigger firm"

# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Plan: 3 Priorities to Get Your Time Back"

SMB_MODEL_DESC = (
    "SMB Team pairs a dedicated Fractional CTO with Legal AI Workforce software "
    "to automate the manual work still running through your own hands. When "
    "pre-bill review, document assembly, and ProLaw billing all run through AI "
    "with human oversight, Jeffrey can finally step back from the paperwork "
    "without stepping back from the firm."
)

GOAL_HEADLINE = "3 hrs → ~1 hr per pre-bill cycle"
GOAL_DBM      = "Reclaim the hours pre-bill review has cost you"

# Each priority: (line1, line2, accent_color_hex, [5 bullet strings])
PRIORITIES = [
    (
        "Automate the", "Pre-Bill Review", "1D4ED8",
        [
            "AI flags pre-bill issues before your own review",
            "Kickoff with the Fractional CTO to scope it",
            "Build and test the AI pre-bill workflow",
            "All AI output drafts for human review first",
            "Get two hours back every billing cycle",
        ],
    ),
    (
        "Modernize", "Document & Billing", "0F766E",
        [
            "Automate document assembly for common filings",
            "Export and map ProLaw data for integration",
            "Layer a modern workflow onto legacy ProLaw",
            "Reduce billing error risk with an AI first pass",
            "Free attorneys from repetitive drafting work",
        ],
    ),
    (
        "Build In", "Trust & Oversight", "6D28D9",
        [
            "Every AI draft goes to you before it's sent",
            "Stay in full control of the final work product",
            "Review results with Jeffrey at 30/60/90 days",
            "Identify the next task worth automating",
            "Trust the system without losing your voice",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$4,997", "$5,797/mo",
        "Fractional CTO · Pre-bill AI · Doc assembly · ProLaw integration",
        "1D4ED8",
    ),
]

BUNDLE_TOTAL   = "$4,997 / mo"
BUNDLE_SAVINGS = "Save $800/mo vs. stand-alone pricing"

AD_SPEND_NOTE = (
    "+ One-time onboarding: $14,997 (Law Firm AI Foundation Sprint)"
)

AVG_CASE_VALUE      = "3 hrs → ~1 hr"
CONSERVATIVE_LABEL  = "Hours saved / cycle:"
CONSERVATIVE_RESULT = "~2 hrs back every 2 weeks"
AGGRESSIVE_LABEL    = "Hours saved / year:"
AGGRESSIVE_RESULT   = "~52 hrs back annually"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Kickoff call with the Fractional CTO"),
    ("Day 14",  "ProLaw data exported and mapped"),
    ("Week 2",  "First pre-bill AI workflow built & tested"),
    ("Week 3",  "Document assembly automation deployed"),
    ("Month 3", "30/60/90 review; next task identified"),
]

CLOSING_QUOTE = (
    '"This was never about growing the firm or cutting costs — '
    'I just want my time back from that 3-inch stack of paper."'
)
...
def build_slide3(prs):
    ...
    add_text(slide, "PROJECTED TIME SAVINGS",
             5.14, 1.18, 4.52, 0.24, 8, ROI_GREEN, bold=True)
    add_text(slide, "Time per pre-bill cycle:", 5.14, 1.46, 2.10, 0.28, 9, rgb("1E293B"), bold=True)
