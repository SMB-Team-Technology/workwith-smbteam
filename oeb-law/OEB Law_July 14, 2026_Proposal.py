...
OUTPUT_PATH    = "oeb-law/OEB Law_July 14, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "RED",    "Invisible in 3-pack"),   # Lead Generation
    ("AMBER", "AMBER",  "Video greeting off"),     # Intake
    ("AMBER", "AMBER",  "Tim hard to reach"),      # Team
    ("RED",   "RED",    "No revenue data"),        # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Invisible in local 3-pack/organic for every core PI search in Knoxville"),
    ("neg", "No revenue, case value, or close-rate data — profitability unknown"),
    ("neg", "Tim remains the bottleneck — hard to reach between court commitments"),
    ("pos", "~105 reviews at 4.5 stars — already ahead of Hodges, Doughty & Carson"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Garza Law Firm",              "1,418 reviews", "Tops all 3 core PI searches"),
    ("NST Law",                     "126 reviews",   "30+ attorneys, outranks OEB"),
    ("Hodges, Doughty & Carson",    "78 reviews",    "Fewer reviews, still outranks OEB"),
]
CLIENT_REVIEWS      = "~105 reviews"
...
SMB_MODEL_DESC = (
    ...
    "together. Missing any one means growth stalls regardless of ad spend. "
    "When all four work, Tim can finally step back and trust the firm runs without him."
)

GOAL_HEADLINE = "Referrals Only → Predictable Cases"
GOAL_DBM      = "Tim steps back and trusts it works"
...
PRIORITIES = [
    (
        ...
        [
            "Fix local SEO to win the 3-pack again",
            "Launch Google Ads on core PI searches",
            "Enroll in Local Service Ads (pay-per-lead)",
            "Turn Google into a pipeline that runs itself",
            "Build city pages for Maryville, Oak Ridge, Sevierville",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Turn the Intaker video greeting back on",
            "Move the contact form above the fold",
            "Build a defined follow-up sequence",
            "Add after-hours coverage so no lead goes cold",
            "Convert leads already generated, no new ad spend",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Start Elite Coach Plus for Tim and DJ",
            "Build a leadership layer below Tim",
            "Track case value and close rate for the first time",
            "Turn ad spend into a measurable investment",
            "Free Tim to be in court without the firm stalling",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$4,847", "$5,697/mo",
        "Local SEO · Google Ads · LSA · Meta Ads · Website",
        "1D4ED8",
    ),
    (
        "ELITE COACH PLUS",
...
CONSERVATIVE_RESULT = "~$16,800 revenue · ~2.2× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (8–9 cases/mo):"
AGGRESSIVE_RESULT   = "~$59,000 revenue · ~2.4× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Launch Google Ads, LSA, and Meta retargeting"),
    ("Day 14",  "Turn on Intaker video greeting; fix contact form"),
    ("Week 2",  "Local SEO rebuild begins; GBP optimization"),
    ("Week 3",  "Coaching kickoff for Tim and DJ"),
    ("Month 3", "Baseline case value & close-rate tracking live"),
]

CLOSING_QUOTE = (
    '"The OEB name already means something in Knoxville. '
    'It\'s time our systems matched it."'
)
