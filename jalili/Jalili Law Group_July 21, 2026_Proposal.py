...
# Report metadata
FIRM_NAME      = "Jalili Law Group"
SALES_REP      = "Jacob Meissner"
OUTPUT_PATH    = "jalili/Jalili Law Group_July 21, 2026_Proposal.pptx"
...
PILLARS = [
    ("AMBER", "AMBER",    "Inconsistent quality"),   # Lead Generation
    ("RED",   "CRITICAL", "No lead screening"),      # Intake
    ("AMBER", "AMBER",    "Owner still screens"),    # Team
    ("AMBER", "AMBER",    "No margin data"),          # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Attorney personally screens every free consult, wasting time on unqualified leads"),
    ("neg", "Three mismatched phone numbers fragment tracking of the $10K/month ad spend"),
    ("neg", "No profit-margin visibility exists behind the $2M revenue goal"),
    ("pos", "103 leads and $4.2K spent in July alone — real demand already exists"),
]

# Competitor table — list of (name, badge, "brief note")
# Live Google review counts could not be confirmed this cycle — using qualitative
# differentiators instead of fabricated review numbers, per data-integrity rule.
COMPETITORS = [
    ("Daniel Brown Law", "Top 10 Boutique", "Nat'l recognition"),
    ("Kazandji Law",     "Dual practice",   "Toronto + Thornhill"),
    ("Donich Law",       "Nat'l media",     "High-risk cases"),
]
CLIENT_REVIEWS      = "Not yet confirmed"
...
SMB_MODEL_DESC = (
    ...
    "together. When they do, Iman and Nasim can step back from every consultation "
    "and let the firm fund the U.S. expansion on its own."
)

GOAL_HEADLINE = "$950K → $2M revenue"
GOAL_DBM      = "Build the firm that funds your next one"
...
PRIORITIES = [
    (
        ...
        [
            "Launch high-intent Google Ads for criminal defense",
            "Turn ad spend into a pipeline, not just call volume",
            "Add city pages for Vaughan, Scarborough, North York",
            "Claim Avvo, Justia, and FindLaw directory listings",
            "Fix 3 mismatched phone numbers fragmenting tracking",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Screen consults before they hit the attorney's calendar",
            "Separate Legal Aid leads from private-pay prospects",
            "Add a follow-up sequence for leads who don't convert",
            "Free the attorney's time for higher-value cases",
            "Turn 103 monthly leads into clients, not just calls",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Give your COO ownership of client screening",
            "Track case profitability by practice area",
            "Build the leadership bench the U.S. firm needs",
            "See real margin behind the $2M revenue goal",
...
PACKAGES = [
    (
        ...
        "$4,847", "$5,697/mo",
        "Google Ads · Meta Ads · Local SEO · Website",
        "1D4ED8",
    ),
    (
        "ELITE COACH PLUS",
        "$3,200", "$3,497/mo",
        "Weekly coaching · Masterminds · Intake systems",
        "6D28D9",
    ),
]
...
AGGRESSIVE_LABEL   = "Aggressive  (29 cases/mo):"
AGGRESSIVE_RESULT  = "$116,000 revenue · 5.3× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Launch refined Google Ads for criminal defense"),
    ("Day 14",  "Consolidate call tracking to one trackable number"),
    ("Week 2",  "City landing pages and directory listings go live"),
    ("Week 3",  "Elite Coach Plus kickoff — build intake screening"),
    ("Month 3", "Case profitability tracking live by practice area"),
]

CLOSING_QUOTE = (
    '"Prove the demand, fix the systems, then build the firm that comes next."'
)
