...
# Report metadata
FIRM_NAME      = "Rebel Brown Law Group"
SALES_REP      = "Jacob Meissner"
OUTPUT_PATH    = "rebel-brown/RebelBrownLawGroup_July21_2026_Proposal.pptx"
...
PILLARS = [
    ...
    ("RED",   "CRITICAL", "Only 1 in 4 answered"),
    ("RED",   "CRITICAL", "Owner is bottleneck"),
    ("AMBER", "AMBER",    "No P&L by practice"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "60 of 80 monthly leads go unanswered — ~$200K/year lost"),
    ("neg", "No city/county SEO pages despite 8-county SSD claim"),
    ("neg", "Owner capped at 2-3 billable hours/day by admin work"),
    ("pos", "SSD site is modern, professional, 10.0 Avvo rating"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Hoffman DiMuzio", "84 reviews", "4.9★ · multi-office"),
    ("Bross & Frankel", "10 reviews", "SSD-only, outranks organically"),
    ("Stopper Lopez",   "8 reviews",  "City-specific landing pages"),
]
CLIENT_REVIEWS      = "48 reviews"
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach 1,000 SSD Clients"

SMB_MODEL_DESC = (
    "All four pillars — lead generation, intake, team, and profit — must work "
    "together. When all four work, Marianne can finally step back from the 70% "
    "of her day spent on non-billable work."
)

GOAL_HEADLINE = "$400K → 1,000 SSD Clients"
GOAL_DBM      = "Step back and let SSD run itself"
...
PRIORITIES = [
    (
        ...
        [
            "Launch SSD Google Ads & LSA campaigns",
            "Build landing pages for all 8 counties",
            "Fix GBP category and NAP inconsistencies",
            "Turn Google into a pipeline that runs without you",
            "Add retargeting and testimonial Meta ads",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Answer all 80 monthly leads, not just 20",
            "Stand up a defined follow-up sequence",
            "Add after-hours and overflow coverage",
            "Recover the $200K/year being lost",
            "Turn your 80% close rate into revenue",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Build delegation for the lean team",
            "Elite Coach Plus builds accountability",
            "Track profit by practice, not just revenue",
            "Free Marianne from 70% non-billable work",
            "Build a team that runs without her",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$4,847", "$5,697/mo",
        "Local SEO · Google Ads · LSA · GBP · Landing Pages",
        "1D4ED8",
    ),
    (
        "ELITE COACH PLUS",
...
AGGRESSIVE_RESULT  = "$100,000 revenue · 7.1× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Launch SSD Google Ads & LSA campaigns"),
    ("Day 14",  "GBP category fix and NAP cleanup complete"),
    ("Week 2",  "City/county landing pages published"),
    ("Week 3",  "Intake follow-up process fully live"),
    ("Month 3", "Elite Coach Plus builds team delegation"),
]

CLOSING_QUOTE = (
    '"I want the SSD practice to run without me being the one '
    'babysitting every single case."'
)
