...
Output: krause-law/Krause_Law_May_28_2026_Proposal.pptx
"""
...
# Report metadata
FIRM_NAME      = "Krause Law, PLLC"
SALES_REP      = "Randy Gold"
OUTPUT_PATH    = "krause-law/Krause_Law_May_28_2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL", "Zero paid ads running"),   # Lead Generation
    ("AMBER", "CAUTION",  "No AI intake automation"), # Intake
    ("AMBER", "CAUTION",  "Benjamin is the ceiling"), # Team
    ("AMBER", "CAUTION",  "No CPA tracking in place"),# Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Zero paid advertising — CCK Law & Berry Law capture every veteran ready to hire right now"),
    ("neg", "DisabledVeterans.org has no tracked bridge to law firm intake — the most powerful asset generates no measurable leads"),
    ("neg", "No AI intake automation — veterans who don't hear back within the hour call a competitor before morning"),
    ("pos", "2,000+ articles, national media, VA-accredited credentials — a decade of authority competitors cannot replicate"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("CCK Law",     "512+ reviews", "$1B+ recovered · 91% CAVC wins"),
    ("Berry Law",   "paid ads only", "$300M+ backpay · all 50 states"),
    ("Woods & Woods", "active ads", "National paid ads · VA keywords"),
]
CLIENT_REVIEWS      = "not confirmed"
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $1.5M Revenue"

SMB_MODEL_DESC = (
    "The SMB Team model aligns four pillars — Lead Generation, Intake, "
    "Self-Managing Team, and Profit Plan — so every part of the firm "
    "works together. When all four pillars run, Benjamin can focus on "
    "the veterans who need him most and step away from everything else."
)

GOAL_HEADLINE = "$750K → $1.5M revenue"
GOAL_DBM      = "Build the most trusted veterans law brand — and let the firm run"
...
PRIORITIES = [
    (
        ...
        [
            "Turn paid ads into a 24/7 veteran consultation pipeline",
            "Google Screened LSA — above CCK Law in every vet search",
            "Bridge DisabledVeterans.org traffic into the intake pipeline",
            "Meta retargeting converts the audience Benjamin already built",
            "Branded keyword protection stops competitors poaching his name",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "AI instant response to every case evaluation — same minute",
            "24/7 intake coverage across all time zones, no staff needed",
            "Automated follow-up nurtures leads who aren't ready day one",
            "Calendar booking from inquiry to consultation in one click",
            "Veterans feel heard immediately — before any competitor calls",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "AI Accelerator L1 — Fractional CTO builds AI workflows now",
            "Automate case research and client updates with AI tools",
            "Track cost-per-acquisition before the first ad dollar ships",
            "AI Avatar runs the content pipeline while Benjamin leads",
            "Free Benjamin from every case — firm runs without him",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$4,847", "$5,697/mo",
        "Website · Google Ads · LSA · Meta Ads · SEO · GBP",
        "1D4ED8",
    ),
    (
        "ELITE COACH",
        "$2,600", "$3,497/mo",
        "Weekly 1:1 coaching · KPI framework · Stage 4→6 roadmap",
        "6D28D9",
    ),
]

BUNDLE_TOTAL   = "$12,694 / mo"
BUNDLE_SAVINGS = "Save $2,247/mo by bundling all four packages"

AD_SPEND_NOTE = (
    "+ Recommended ad spend: $5,000–$9,000/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE     = "$3,500"
CONSERVATIVE_LABEL = "Conservative  (6 cases/mo):"
CONSERVATIVE_RESULT = "$21,000 revenue · 4.2× ROAS"
AGGRESSIVE_LABEL   = "Aggressive  (14 cases/mo):"
AGGRESSIVE_RESULT  = "$49,000 revenue · 5.4× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",    "New website build begins — VA appeals consultation capture"),
    ("Day 14",   "Google Screened LSA application submitted — pay-per-lead"),
    ("Week 6",   "Google Ads launch — VA disability + VR&E ad groups live"),
    ("Month 2",  "AI Accelerator L1 — intake AI and case research workflows"),
    ("Month 3",  "AI Avatar — first 30 short-form videos published & live"),
]

CLOSING_QUOTE = (
    '"Benjamin has built the credibility — now it is time to build the engine '
    'that converts it into the practice he has earned."'
)
