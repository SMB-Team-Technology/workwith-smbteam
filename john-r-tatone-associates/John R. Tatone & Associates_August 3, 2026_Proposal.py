...
# Report metadata
FIRM_NAME      = "John R. Tatone & Associates"
SALES_REP      = "Nick Holderman"
OUTPUT_PATH    = "john-r-tatone-associates/John R. Tatone & Associates_August 3, 2026_Proposal.pptx"
...
PILLARS = [
    ("AMBER", "AMBER", "Only LSA converting"),
    ("AMBER", "AMBER", "No dedicated intake"),
    ("RED",   "CRITICAL", "Just 3, no manager"),
    ("AMBER", "AMBER", "No overall margin"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Only Google LSA is actually converting — other channels aren't paying off"),
    ("neg", "No above-the-fold form or live chat — visitors leave before they connect"),
    ("neg", "3-person team, no manager — the practice can't run without John"),
    ("pos", "Avvo rating of 9.4 \"Superb\" — among the strongest in this market"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("SSR Law Offices",     "129 reviews", "4.9★ · outranks on estate planning"),
    ("Rutkowski Law Firm",  "166 reviews", "4.8★ · 166-1,100 organic visits/mo"),
    ("Hubbs Law Group PLC", "N/A reviews", "Direct Utica-based competitor"),
]
CLIENT_REVIEWS      = "126 reviews"
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $500K in 2026"

SMB_MODEL_DESC = (
    "All four pillars — marketing, intake, team, and profit — must work together. "
    "When they do, John can finally step back from the practice without it stalling."
)

GOAL_HEADLINE = "$270K → $500K revenue"
GOAL_DBM      = "Step away and trust the practice runs"
...
PRIORITIES = [
    (
        ...
        [
            "Expand Google LSA — the channel already proven",
            "Add local SEO landing pages for nearby suburbs",
            "Turn Facebook spend into real lead generation",
            "Fix the Yelp category and NAP inconsistencies",
            "Turn LSA into a pipeline that runs without you",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Add a visible way to start a conversation online",
            "Build a fast follow-up sequence for every lead",
            "Add clear calls-to-action on every page",
            "Track which leads convert and which don't",
            "Turn every website visit into a real conversation",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Add a management layer beneath John",
            "Build a documented profit plan",
            "Track true case value and close rate",
            "Create accountability without more headcount",
            "Free John from being the only safety net",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$3,397", "—",
        "Managed Google LSA · GBP · core SEO · local landing pages",
        "1D4ED8",
    ),
    (
        "ELITE COACH",
        "$2,600", "$3,497/mo",
        "Weekly coaching · practice area masterminds · growth curriculum",
        "6D28D9",
    ),
]
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $3,500–$7,500/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE     = "$2,500 (est.)"
CONSERVATIVE_LABEL = "Conservative  (3-4 cases/mo):"
CONSERVATIVE_RESULT = "$9,000 revenue · 2.6× ROAS"
AGGRESSIVE_LABEL   = "Aggressive  (9-10 cases/mo):"
AGGRESSIVE_RESULT  = "$23,000 revenue · 3.1× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Kick off Elite Coach onboarding & KPI targets"),
    ("Day 14",  "Launch expanded Google LSA campaign coverage"),
    ("Week 2",  "Add above-the-fold contact forms & CTAs"),
    ("Week 3",  "Fix Yelp category & Novi, MI NAP inconsistency"),
    ("Month 3", "Live landing pages for Sterling Hts, Warren, Macomb"),
]

CLOSING_QUOTE = (
    '"After everything it took to get here, I want a practice that keeps '
    'moving even when I\'m not the one moving it."'
)
