...
# Report metadata
FIRM_NAME      = "O'Connor Family Law"
SALES_REP      = "Dan Bryant"
OUTPUT_PATH    = "oconnor-family-law/OConnorFamilyLaw_05112026_Proposal.pptx"
...
PILLARS = [
    ("AMBER", "NEEDS WORK",  "LSA: ~50% spam leads"),    # Lead Generation
    ("GREEN", "STRONG",      "96.7% show rate"),          # Intake
    ("AMBER", "NEEDS WORK",  "DOO auditing agency"),      # Team
    ("AMBER", "NEEDS WORK",  "True CPL unknown"),         # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "$10k/mo LSA: ~50% spam — agency unchanged after 8–10 months of feedback."),
    ("neg", "GBP category 'Family Law' misses high-intent 'Divorce Lawyer' searches."),
    ("neg", "Avery's DOO bandwidth lost to agency auditing instead of firm growth."),
    ("pos", "96.7% consult show rate — intake is the strongest in the market."),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Callahan | Inman",       "21 attorneys",   "dominant LSA/PPC · Worcester Co."),
    ("Burke Levy, P.C.",       "4.9★ / 52 rev.", "LSA ranking edge · Westborough"),
    ("S. Shore Divorce & FL",  "S. Shore brand", "Hanover/South Shore specialist"),
]
CLIENT_REVIEWS      = "136 reviews · 4.6★"
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $7M"

SMB_MODEL_DESC = (
    "SMB Team builds four pillars — Lead Generation, Intake, Team, and Profit — "
    "until your firm runs without you managing it. "
    "When all four work, Heather can step back and trust the firm runs exactly as she built it."
)

GOAL_HEADLINE = "$4M → $7M revenue"
GOAL_DBM      = "Step back — the firm runs as you built it"
...
PRIORITIES = [
    (
        ...
        [
            "Replace LSA spam with signed-case attribution",
            "Launch PPC with income demographic layering",
            "Fix GBP: 'Divorce Lawyer' on both offices",
            "Dispute every LSA junk lead to recover credit",
    ...
    (
        ...
        [
            "Replace Answering Legal for 11am–2pm gap",
            "Deploy pre-consult nurture in Lawmatics",
            "Activate Hanover GBP — close South Shore gap",
            "Build review response cadence on both profiles",
            "Intake closes at capacity with qualified volume",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Free Avery from agency auditing entirely",
            "Build signed-case attribution by channel",
            "Add fractional CFO at $5M for profit planning",
            "Phase 2: FCOO Advisor for operational scale",
            "Own your numbers — profit planned, not found",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$7,397", "$8,997/mo",
        "Website · PPC · LSA · Meta · GBP · signed-case attribution",
        "1D4ED8",
    ),
    (
        "RECOMMENDED COACHING & ADVISORY",
        "$4,600", "$4,997/mo",
        "Elite Coach · High Level Mastermind · Team Training",
        "6D28D9",
    ),
]
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $15,000–$25,000/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE      = "$15,000"
CONSERVATIVE_LABEL  = "Conservative  (10–12 cases/mo):"
CONSERVATIVE_RESULT = "$150K–$180K revenue · 10x–12x ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (16–20 cases/mo):"
AGGRESSIVE_RESULT   = "$240K–$300K revenue · 9.6x–12x ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "GBP: switch both profiles to 'Divorce Lawyer'"),
    ("Day 14",  "Begin LSA junk lead dispute process"),
    ("Week 2",  "Launch PPC with income demographic targeting"),
    ("Week 3",  "Activate Hanover GBP — posts + review responses"),
    ("Month 3", "Deploy pre-consult nurture sequence in Lawmatics"),
]

CLOSING_QUOTE = (
    '"The intake engine is built, the team is ready, and the leads are there — '
    'what is missing is a marketing partner who optimizes to what gets signed, '
