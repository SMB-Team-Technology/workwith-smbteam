...
# Report metadata
FIRM_NAME      = "Oakwood Legal Group"
SALES_REP      = "Dan Bryant"
OUTPUT_PATH    = "oakwood-legal-group/Oakwood Legal Group_September 14, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL", "Invisible in local search"),
    ("AMBER", "AMBER",    "Contact form buried below"),
    ("RED",   "CRITICAL", "Owner fixes GBP himself"),
    ("AMBER", "AMBER",    "No CPA tracking in place"),
]
...
FINDINGS = [
    ("neg", "Oakwood does not appear in a single local search tested for Personal Injury or Employment Law"),
    ("neg", "A buried contact form risks losing the leads the firm is about to pay to generate"),
    ("neg", "Mike is still the one fixing Google Business Profile categories himself"),
    ("pos", "Already listed on FindLaw, Super Lawyers, and BBB — real directory credibility to build on"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
# List 3 competitors + the client row is added automatically.
COMPETITORS = [
    ("West Coast Trial Lawyers",       "Larger review base",     "Downtown LA · dominates PI searches"),
    ("Law Offices of Jacob Emrani",    "Larger review base",     "Aggressive PI brand presence"),
    ("Setyan Law",                     "Sharper category focus", "Keyword-matched employment pages"),
]
CLIENT_REVIEWS      = "Building review base"
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Dominate Your Market"

SMB_MODEL_DESC = (
    "All four pillars — leads, intake, team, and profit — must work "
    "together for growth to compound. When they do, Oakwood becomes "
    "the firm people find first, in every market it serves."
)

GOAL_HEADLINE = "$3M → $6M revenue"
GOAL_DBM      = "Be the firm clients find first, every time"
...
PRIORITIES = [
    (
        ...
        [
            "Launch Employment Law paid search & LSA campaigns",
            "Fix GBP: DBA, categories, and business name",
            "Clean up conflicting Yelp/directory addresses",
            "Build Employment Law practice-area content",
            "Turn Google into a pipeline that finds you first",
        ],
    ),
    (
        ...
        [
            ...
            "Evaluate adding a live chat option",
            "Build a follow-up sequence for new leads",
            "Add after-hours coverage for calls",
            "Never lose a lead you already paid to generate",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "003A59",
        [
            "Delegate GBP & vendor management off Mike",
            "Track cost-per-lead from campaign day one",
            "Scope coaching/ops support on the next call",
            "Build a profit baseline before scaling spend",
            "Free Mike from fixing categories mid-call",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$5,697", "",
        "Website · SEO · GBP · paid ads focused on Employment Law",
        "0091C9",
    ),
]

BUNDLE_TOTAL   = "$5,697 / mo"
BUNDLE_SAVINGS = "Priced standalone this round — no bundle discount"
...
CONSERVATIVE_RESULT = "$96,000 revenue · 9.6× ROAS"
AGGRESSIVE_LABEL   = "Aggressive  (36 cases/mo):"
AGGRESSIVE_RESULT  = "$288,000 revenue · 11.5× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Launch Employment Law paid campaigns"),
    ("Day 14",  "Fix GBP compliance & begin directory cleanup"),
    ("Week 2",  "Reposition contact form above the fold"),
    ("Week 3",  "Publish Employment Law practice-area content"),
    ("Month 3", "Schedule Solution Call to scope coaching/ops"),
]

CLOSING_QUOTE = (
    '"When someone in this city searches for a lawyer, it should be '
    'my name they find first."'
)
