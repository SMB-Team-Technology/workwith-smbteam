...
# Report metadata
FIRM_NAME      = "Erkan & Sullivan, P.C."
SALES_REP      = "Michael Kopp"
OUTPUT_PATH    = "erkan-sullivan/Erkan_Sullivan_August 12, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL", "212 visits vs. 1,900"),
    ("AMBER", "AMBER",    "CTA buried mid-page"),
    ("AMBER", "AMBER",    "Hinges on 1 new hire"),
    ("AMBER", "AMBER",    "$800/hr target only"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "6-9x organic visibility gap vs. Neyman and Urbelis despite better reviews"),
    ("neg", "Zero paid ads running — the 27,000-case OUI ruling sits in one unseen blog post"),
    ("neg", "No after-hours lead capture risks losing high-urgency criminal defense inquiries"),
    ("pos", "154 five-star Google reviews — already ahead of 2 of 3 named competitors"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Stephen Neyman Law Offices", "96 reviews",  "868 keywords · dominates SEO"),
    ("Gilman Law, P.C.",           "470 reviews", "OUI specialist · 3x reviews"),
    ("Urbelis Law",                "37 reviews",  "Out-ranks despite fewer reviews"),
]
CLIENT_REVIEWS      = "154 reviews"
...
SMB_MODEL_DESC = (
    ...
    "regardless of ad spend. When all four work, Murat and Ryan can finally step "
    "back from personally running every hearing."
)

GOAL_HEADLINE = "$500K → $1M revenue"
GOAL_DBM      = "Step back — and the firm still wins cases"
...
PRIORITIES = [
    (
        ...
        [
            "Launch Google PPC on OUI/DUI terms",
            "Add Meta retargeting for lost visitors",
            "Rebuild SEO to close the Neyman gap",
            "Campaign around the 27K-case OUI ruling",
            "Turn visibility into premium case flow",
        ],
    ),
    (
        ...
        [
            ...
            "Add live capture for after-hours inquiries",
            "Build a defined intake follow-up process",
            "Track which leads convert to signed cases",
            "Stop losing paid-for leads to slow follow-up",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "003A59",
        [
            "Add leadership layer before the new hire",
            "Build a system for delegating high-margin work",
            "Track profitability across every case type",
            "Confirm the $800/hr partner target is real",
            "Make the associate hire capacity, not rescue",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$4,997", "$5,697/mo",
        "SEO rebuild · Google PPC · Meta retargeting",
        "0091C9",
    ),
    (
        "ELITE COACH PLUS",
        "$3,200", "$3,497/mo",
        "1:1 coaching · group sessions · masterminds",
        "003A59",
    ),
]
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $6,700-$16,500/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE      = "$15,000"
CONSERVATIVE_LABEL  = "Conservative  (~5 cases/mo):"
CONSERVATIVE_RESULT = "$75,000 revenue · 11.2x ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (~16 cases/mo):"
AGGRESSIVE_RESULT   = "$240,000 revenue · 14.5x ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Launch Google PPC & Meta retargeting"),
    ("Day 14",  "Elite Coach Plus kickoff with Murat & Ryan"),
    ("Week 2",  "Move consultation CTA above the fold"),
    ("Week 3",  "Begin local SEO rebuild & OUI campaign"),
    ("Month 3", "Add leadership layer ahead of associate hire"),
]

CLOSING_QUOTE = (
    "\"We've built the trial record. Now it's time our marketing, "
    "and our team, caught up to it.\""
)
