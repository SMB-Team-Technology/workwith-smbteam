...
# Report metadata
FIRM_NAME      = "Law Offices of David Bliven"
SALES_REP      = "Dan Bryant"
OUTPUT_PATH    = "law-offices-of-david-bliven/Law Offices of David Bliven_June 12, 2026_Proposal.pptx"
...
PILLARS = [
    ("AMBER", "AMBER",    "NAP inconsistency"),       # Lead Generation
    ("RED",   "CRITICAL", "40–73% no-show rate"),     # Intake
    ("AMBER", "AMBER",    "No KPIs / underutilized"), # Team
    ("AMBER", "AMBER",    "$100K+ AR, no system"),    # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings
FINDINGS = [
    ("neg", "40–73% no-show rate — $500K+ in uncaptured annual revenue"),
    ("neg", "Paralegals billing 10 hrs/wk — $50K+/mo unused billing capacity"),
    ("neg", "$100K+ outstanding AR with no collections workflow in place"),
    ("pos", "147 Google reviews — 3–9x ahead of every White Plains competitor"),
]

# Competitor table
COMPETITORS = [
    ("Nolletti Law Group PLLC",   "16 reviews",   "4 Super Lawyers attys · premium"),
    ("Douglas Family Law Group",  "47 reviews",   "5-star boutique · mediation"),
    ("Johnson & Cohen LLP",       "Not indexed",  "Multi-atty · full digital"),
]
CLIENT_REVIEWS      = "147 reviews"
CLIENT_REVIEWS_NOTE = "← You · 4.8★"

# Stage strip (right panel, Slide 1)
STAGE_TEXT = "Stage 4: Small Business Manager  →  Stage 6: Law Firm Owner"

# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $2M by 2027"

SMB_MODEL_DESC = (
    "Lead generation, intake, team, and profit must all "
    "work together — missing any one means growth stalls "
    "regardless of ad spend. When all four pillars are "
    "aligned, David can build the firm he envisioned: "
    "one that generates revenue without him in every role."
)

GOAL_HEADLINE = "$750K → $2M+ revenue"
GOAL_DBM      = "Pay yourself what you've earned — step back"
...
PRIORITIES = [
    (
        ...
        [
            "Fix NAP inconsistency — turn 147 reviews into rankings",
            "Activate Google Screened LSA for both offices",
            "Expand ads to all 4 practice areas + urgency terms",
            "Monthly GBP posts from Divorce Wars video series",
    ...
    (
        ...
        [
            "Clio Grow reminders live — 24hr before every consult",
            "Set show-rate KPI — track weekly, intervene monthly",
            "No-show follow-up sequence — recover lost leads",
            "Define response time SLA — no lead unanswered",
            "Conversion rises 15% → 25%+ without extra ad spend",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Paralegal utilization targets — 25+ hrs/wk each",
            "Associate onboarding framework before August hire",
            "AR collections workflow inside Clio Manage",
            "KPI dashboard — profit, utilization, AR weekly",
            "David practices law, not ops — systems run the firm",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$4,847", "$5,697/mo",
        "Google Ads · LSA · Video Growth · SEO · NAP cleanup · GBP",
        "1D4ED8",
    ),
    (
        "ELITE COACH PLUS",
        "$3,200", "$3,497/mo",
        "Weekly coaching · Intake KPIs · Team framework · AR system",
        "6D28D9",
    ),
]
...
CONSERVATIVE_RESULT = "$62,500 revenue · ~18× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (10–12 cases/mo):"
AGGRESSIVE_RESULT   = "$262,500 revenue · ~19× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Ads audit + NAP directory corrections begin"),
    ("Day 14",  "Clio Grow reminders live + intake KPIs set"),
    ("Week 2",  "AR collections workflow built in Clio Manage"),
    ("Week 3",  "Associate onboarding framework complete"),
    ("Month 3", "Show rate 75%+, paralegal billing at target"),
]

CLOSING_QUOTE = (
    '"Build a firm that runs without me as the firefighter — '
    'so I can grow without burning out and finally pay myself what I\'m worth."'
)
