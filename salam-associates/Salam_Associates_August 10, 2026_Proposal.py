...
# Report metadata
FIRM_NAME      = "Salam & Associates, P.C."
SALES_REP      = "Jacob Meissner"
OUTPUT_PATH    = "salam-associates/Salam_Associates_August 10, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED", "CRITICAL", "Invisible in 6/6 searches"),
    ("RED", "CRITICAL", "~$600K/yr lost to slow follow-up"),
    ("RED", "CRITICAL", "Owner does 75% of the work"),
    ("RED", "CRITICAL", "Revenue $4M → ~$2M this year"),
]
...
FINDINGS = [
    ("neg", "Slow, inconsistent intake follow-up is losing ~5 cases a month — about $600,000 a year"),
    ("neg", "Fatima personally handles ~75% of the firm's work with no accountability structure behind her"),
    ("neg", "Revenue is projected to fall from $4M to ~$2M this year as case volume drops from ~400 to ~175/year"),
    ("pos", "21 years in business with an A+ BBB rating and attorney bios that build real trust once found"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
# List 3 competitors + the client row is added automatically.
COMPETITORS = [
    ("Fielding Law",      "128+ reviews", "5.0★ · Richardson-based"),
    ("MAS Law",           "500+ reviews", "4.5★ · Richardson-based"),
    ("Mullen & Mullen",   "500+ reviews", "4.9★ · 8x Top Choice"),
]
CLIENT_REVIEWS      = "90 reviews"
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities Before Marketing Restarts"

SMB_MODEL_DESC = (
    "All four pillars must work together. Adding marketing spend on top of "
    "a broken follow-up process would only waste it. Fix coaching, AI "
    "automation, and intake first — then Fatima can finally step back "
    "and trust the firm runs without her."
)

GOAL_HEADLINE = "~$2M → ~$2.4M revenue"
GOAL_DBM      = "Step back and trust the firm runs itself"

# Each priority: (line1, line2, accent_color_hex, [5 bullet strings])
PRIORITIES = [
    (
        "Coach the", "Team to Self-Manage", "003A59",
        [
            "Weekly coaching led by Peter Schultz begins",
            "Build the accountability structure missing today",
            "Reduce how much depends on Fatima catching it",
            "Practice area masterminds + quarterly workshops",
            "A team that runs without her in every file",
        ],
    ),
    (
        "Automate the", "Back Office with AI", "0091C9",
        [
            "Fractional CTO L2 leads monthly custom AI builds",
            "Build a custom Fatima GPT for firm SOPs",
            "Automate case notes and document tracking",
            "Catch gaps like missing radiology bills",
            "Launch an automated client-retention campaign",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "69CD2B",
        [
            "Complete the transition to Alert Communications",
            "Add AI-based instant lead routing 24/7",
            "Recover the ~$600K/year lost to slow follow-up",
            "Fix the GBP photo and grow reviews past 200",
            "Prove intake works before marketing restarts",
        ],
    ),
]
...
PACKAGES = [
    (
        "ELITE COACH PLUS",
        "$3,200", "$3,497/mo",
        "Weekly group coaching (Peter Schultz) · masterminds · workshops",
        "003A59",
    ),
    (
        "LEGAL AI WORKFORCE — FRACTIONAL CTO L2",
        "$4,997", "$5,797/mo",
        "Fatima GPT · case/doc tracking automation · retention drip campaign",
        "0091C9",
    ),
]

BUNDLE_TOTAL   = "$8,197 / mo"
BUNDLE_SAVINGS = "Save $1,097/mo by bundling"

AD_SPEND_NOTE = (
    "+ One-time onboarding: $14,997 (Law Firm AI Foundation Sprint, saves $5,000) — no ad spend in Phase 1"
)

AVG_CASE_VALUE      = "$7,500 (default est.)"
CONSERVATIVE_LABEL  = "Phase 2 planning, conservative:"
CONSERVATIVE_RESULT = "$13.3K revenue · 1.33× return"
AGGRESSIVE_LABEL    = "Phase 2 planning, aggressive:"
AGGRESSIVE_RESULT   = "$124.9K revenue · 2.50× return"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Coaching kickoff with Peter Schultz"),
    ("Day 14",  "Fractional CTO L2 scopes the Fatima GPT build"),
    ("Week 2",  "Alert Communications intake transition completes"),
    ("Week 3",  "GBP photo fixed, review campaign launches"),
    ("Month 3", "Review the $600K/year leak — confirm it's closed"),
]

CLOSING_QUOTE = (
    '"Make the firm self-managing so I can retire from daily operations '
    'and run a new NGO."'
)
