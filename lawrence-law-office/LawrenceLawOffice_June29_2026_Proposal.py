"""
Audit PowerPoint — Lawrence Law Office
...
FIRM_NAME   = "Lawrence Law Office"
SALES_REP   = "Dan Bryant"
OUTPUT_PATH = "lawrence-law-office/LawrenceLawOffice_June29_2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL",   "No paid ads anywhere"),    # Lead Generation
    ("RED",   "CRITICAL",   "No after-hours cover"),    # Intake
    ("AMBER", "NEEDS WORK", "No delegation layer"),     # Team
    ("AMBER", "NEEDS WORK", "No financial metrics"),    # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

FINDINGS = [
    ("neg", "Absent from all paid search while Joslyn, Gavvl, and Petroff advertise actively"),
    ("neg", "56 Google reviews at 4.0★ — Gavvl (90+, 4.8★) and Joslyn (78+, 4.8★) hold the 3-pack"),
    ("neg", "No intake system or after-hours coverage — first-to-respond competitors win those cases"),
    ("pos", "9.7 Avvo rating + Board Certified since 1990 + geo-targeting pages already in place"),
]

COMPETITORS = [
    ("Gavvl Law, LLC",      "90+ reviews \xb7 4.8★", "Best Lawyers 2026 \xb7 ads active"),
    ("Joslyn Law Firm",     "78+ reviews \xb7 4.8★", "Google Ads + LSA \xb7 6 attorneys"),
    ("Wood & Long LLC",     "Limited Google",              "Exclusively family law \xb7 modern brand"),
]
CLIENT_REVIEWS      = "~56 reviews \xb7 4.0★"
CLIENT_REVIEWS_NOTE = "← You are here"

STAGE_TEXT = "Stage 3: Solo Practitioner  →  Goal: Stage 6, Law Firm Owner"

# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $125K/Month"

SMB_MODEL_DESC = (
    "Lead Generation, Intake, Team, and Profit must all work together — "
    "fixing one without the others creates temporary wins, not lasting growth. "
    "When all four pillars are in place, Linda can step back and trust the firm "
    "runs without her in the middle of everything."
)

GOAL_HEADLINE = "$750K → $1.5M annual revenue"
GOAL_DBM      = "Step back — the firm runs without you"
...
PRIORITIES = [
    (
        ...
        [
            ...
            "Launch LSA — pay only when prospects call you",
            "Close the review gap: 56 → 75+ Google reviews",
            "Meta retargeting — re-capture site visitors",
            "Website rebuilt to convert mobile visitors",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "After-hours protocol: no lead goes unanswered",
            "Document intake script — make it delegatable",
            "Follow-up sequence for leads who don’t sign fast",
            "Track close rate from first contact forward",
            "Free Linda from intake calls entirely",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Hire intake coordinator — remove founders from queue",
            "Track cost-per-client by marketing source",
            "Define profit goals — not just revenue targets",
            "Build org structure that runs without you daily",
            "Weekly coaching keeps all KPIs on track",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$4,847", "$5,697/mo",
        "Website \xb7 Google Ads \xb7 LSA \xb7 Meta \xb7 SEO \xb7 GBP management",
        "1D4ED8",
    ),
    (
        "ELITE COACH PLUS",
        "$3,200", "$3,497/mo",
        "Weekly coaching \xb7 masterminds \xb7 quarterly workshops",
        "6D28D9",
    ),
]
...
CONSERVATIVE_RESULT = "$20K–$25K revenue  \xb7  ~6\xd7 ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (18–22 cases/mo):"
AGGRESSIVE_RESULT   = "$90K–$110K revenue  \xb7  ~6–7\xd7 ROAS"

TIMELINE = [
    ("Day 1",   "Coaching kickoff — baselines + 90-day goals set"),
    ("Day 14",  "Google Ads live — Columbus divorce + custody"),
    ("Week 2",  "Website optimization + NAP consistency audit"),
    ("Week 3",  "GBP review campaign live — targeting 75+ reviews"),
    ("Month 3", "Ads optimized, KPIs tracked, after-hours protocol live"),
]

CLOSING_QUOTE = (
    "“Imagine checking in on the firm — not because it needs you, "
    "but because you choose to. That is the firm Linda Lawrence has earned.”"
)
...
def add_footer(slide, page, total, logo_path):
    ...
    add_text(slide, f"CONFIDENTIAL  \xb7  Prepared by {SALES_REP} | SMB Team",
...
def build_slide1(prs):
    ...
    add_text(slide, f"LAW FIRM GROWTH AUDIT  \xb7  {FIRM_NAME.upper()}",
