...
# Report metadata
FIRM_NAME      = "Reneau Bays"
SALES_REP      = "Dan Bryant"
OUTPUT_PATH    = "reneau-bays/Reneau_Bays_09082026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL", "No paid marketing"),
    ("RED",   "CRITICAL", "1 in 4 calls missed"),
    ("AMBER", "AMBER",    "No associates yet"),
    ("AMBER", "AMBER",    "No profit plan"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "82 of the last 356 inbound calls went to voicemail unanswered, with no after-hours coverage"),
    ("neg", "Two unmerged Google Business Profiles split reviews — one still miscategorized as Bankruptcy Attorney"),
    ("neg", "No sustained paid marketing has ever existed, leaving the firm invisible in searches Lai & Turner and Cannon & Associates dominate"),
    ("pos", "Two respected, referral-worthy practices already merged into one firm with real reputation and depth"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Lai & Turner Law Firm",  "Higher volume", "existing SMB Team client"),
    ("Cannon & Associates",    "Higher volume", "OKC + Edmond footprint"),
    ("Rahill Law Firm PLLC",   "Strong brand",  "family-law-only focus"),
]
CLIENT_REVIEWS      = "Fragmented (2 GBPs)"
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $1.6M in Revenue"

SMB_MODEL_DESC = (
    "All four pillars — leads, intake, team, and profit — must work together. "
    "When they do, Danny can finally step back from litigating and Roger can bring on the associates he's been planning."
)

GOAL_HEADLINE = "$800K → $1.6M revenue"
GOAL_DBM      = "Step away from casework and trust the firm runs"
...
PRIORITIES = [
    (
        ...
        [
            "Merge & fix the two fragmented Google profiles",
            "Launch Local Services Ads — family law & probate",
            "Rebuild page titles around real search terms",
            "Add landing pages for 5 surrounding cities",
    ...
    (
        ...
        [
            ...
            "Launch the already-planned Smith.ai + Clio setup",
            "Build a defined lead follow-up sequence",
            "Stop losing calls that go straight to voicemail",
            "Convert leads you're already generating today",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "003A59",
        [
            "Coach Danny's shift from litigating to managing",
            "Build Roger's associate hiring framework",
            "Connect rate increases to real take-home income",
            "Add an FCOO Advisor once the first hire lands",
            "Build a firm that runs without either partner",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$4,997", "$5,697/mo",
        "GBP merge · LSA · geo-optimized pages · PPC",
        "0091C9",
    ),
    (
        "ELITE COACH PLUS",
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $5,500–$20,000/mo paid directly to Google/LSA"
)

AVG_CASE_VALUE      = "$10,000"
CONSERVATIVE_LABEL  = "Conservative  (~7 cases/mo):"
CONSERVATIVE_RESULT = "$70,000 revenue · 12.7× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (~32 cases/mo):"
AGGRESSIVE_RESULT   = "$320,000 revenue · 16× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Kick off Elite Coach Plus sessions"),
    ("Day 14",  "Launch Google Local Services Ads"),
    ("Week 2",  "Merge & re-categorize both GBPs"),
    ("Week 3",  "Relaunch geo-optimized practice pages"),
    ("Month 3", "Add Google Search PPC alongside LSA"),
]

CLOSING_QUOTE = (
    '"Imagine handing off the casework, watching your kids\' games, '
    'and knowing the firm still runs exactly the way you built it."'
)
