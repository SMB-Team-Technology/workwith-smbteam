...
# Report metadata
FIRM_NAME      = "Polchinski & Smith LLP"
SALES_REP      = "Randy Gold"
OUTPUT_PATH    = "polchinski-smith/Polchinski & Smith LLP_July 24, 2026_Proposal.pptx"
...
PILLARS = [
    ("AMBER", "AMBER", "No truck/WC ranking"),   # Lead Generation
    ("RED",   "RED",   "3 partners do it all"),  # Intake
    ("AMBER", "AMBER", "7 staff, no delegate"),  # Team
    ("AMBER", "AMBER", "5x growth, no P&L"),      # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "All intake and settlements run through 3 partners personally"),
    ("neg", "Zero visibility for truck accident and workers' comp searches"),
    ("neg", "No paid ads anywhere — 100% referral and organic dependent"),
    ("pos", "Grew revenue roughly 5x in a year with zero paid marketing"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("McIntyre Law",   "292 reviews", "Ranks for trucking"),
    ("Ylla | Gosney",  "343 reviews", "4.9★ · Moore overlap"),
    ("Dan Davis Law",  "175 reviews", "Ranks for work comp"),
]
CLIENT_REVIEWS      = "47 reviews"
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $2M in Revenue"

SMB_MODEL_DESC = (
    "All four pillars — leads, intake, team, and profit — have to work "
    "together. When they do, the partners can finally step away from "
    "personally running every intake and settlement."
)
...
PRIORITIES = [
    (
        ...
        [
            "Launch Local Service Ads within 2 weeks",
            "Launch Google Ads for car, truck & WC cases",
            "Close SEO gap on truck & workers' comp",
            "Fix the NAP mismatch hurting local rank",
    ...
    (
        ...
        [
            "Build a trained intake process, not partners",
            "Add after-hours coverage for missed calls",
            "Define a follow-up sequence for every lead",
            "Automate Case Peer tasking — no more oversight",
            "Free you from answering every single call",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Kick off Elite Coach for delegation systems",
            "Build a leadership layer under the partners",
            "Track case value and profit by practice area",
            "Plan the next attorney hire the right way",
            "Turn 5x growth into wealth you keep",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$4,847", "$5,697/mo",
        "Local SEO · Google Ads · LSA · Website CRO · Reporting",
        "1D4ED8",
    ),
    (
        "ELITE COACH",
...
CONSERVATIVE_RESULT = "$14K-$21K revenue · 1.4-2.1× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (7-9 cases/mo):"
AGGRESSIVE_RESULT   = "$49K-$63K revenue · 2.0-2.5× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Local Service Ads go live"),
    ("Day 14",  "Google Ads launch across all 3 practice areas"),
    ("Week 2",  "Elite Coach kickoff — intake plan begins"),
    ("Week 3",  "NAP citations fixed, SEO build starts"),
    ("Month 3", "Truck & WC visibility live, intake delegated"),
]

CLOSING_QUOTE = (
    '"We built this to $1M without a system — imagine what happens '
    'once we\'re not the ones answering every call."'
)
