...
# Report metadata
FIRM_NAME      = "Shield Law"
SALES_REP      = "Jonathan Farace"
OUTPUT_PATH    = "shield-law/Shield Law_July 31, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL", "SEO down 91%"),
    ("RED",   "CRITICAL", "Owner-run intake"),
    ("AMBER", "AMBER",    "No delegation yet"),
    ("AMBER", "AMBER",    "Revenue volatile"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "SEO collapse cut new clients from 30/mo to 10-15/mo after the office move"),
    ("neg", "Every directory checked still lists the old Brockton address and phone number"),
    ("neg", "The only paid channel is $1,500/mo — far below what it takes to compete"),
    ("pos", "122 five-star reviews already earned — ready to be reclaimed, not rebuilt"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Levin and Levin, LLP",  "10 reviews",  "4.9★ · Est. 1933"),
    ("Bellotti Law Group",    "Superb rtg.", "10.0 Avvo · PI"),
    ("Ullian & Associates",   "Award winner","40+ yrs · Bankruptcy"),
]
CLIENT_REVIEWS      = "122 reviews"
CLIENT_REVIEWS_NOTE = "← Not yet on Quincy listing"
...
SMB_MODEL_DESC = (
    ...
    "together. When they do, Nick can finally step away and trust the firm runs."
)

GOAL_HEADLINE = "$45K → $200K/month"
...
PRIORITIES = [
    (
        ...
        [
            "Reclaim the Quincy Google Business Profile",
            "Launch Estate Planning & Family Law ads",
            "Consolidate 122 reviews onto the new listing",
            "Add Local Service Ads — pay only per lead",
    ...
    (
        ...
        [
            "Build a real follow-up process for every lead",
            "Add after-hours coverage for missed calls",
            "Track every lead so none slip through",
            "Fix the buried contact form on the website",
            "Convert the leads you're already paying for",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Delegate admin to Matt and Hunter — not just Nick",
            "Set up baseline profit tracking by practice area",
            "Start Elite Coach Plus delegation sessions",
            "Build toward dropping the second job for good",
            "Turn $200K/month into a floor, not a stretch goal",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$4,997", "$5,697/mo",
        "SEO · Google Ads · LSA · Meta Ads · GBP rebuild",
        "1D4ED8",
    ),
    (
        "ELITE COACH PLUS",
        "$3,200", "$3,497/mo",
        "Weekly coaching · masterminds · delegation systems",
        "6D28D9",
    ),
]
...
AVG_CASE_VALUE      = "~$4,300 (blended)"
CONSERVATIVE_LABEL  = "Conservative  (~10 cases/mo):"
CONSERVATIVE_RESULT = "~$44,600 revenue · ~6.0× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (~33 cases/mo):"
AGGRESSIVE_RESULT   = "~$143,000 revenue · ~7.0× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Launch Google Ads & LSA across all practice areas"),
    ("Day 14",  "Claim & optimize the Quincy Google Business Profile"),
    ("Week 2",  "Begin consolidating 122 Brockton reviews"),
    ("Week 3",  "Kick off Elite Coach Plus delegation sessions"),
    ("Month 3", "Profit tracking live across every practice area"),
]

CLOSING_QUOTE = (
    '"A firm that runs whether I\'m on a call or on a plane — '
    'and income I don\'t need a second job to protect."'
)
