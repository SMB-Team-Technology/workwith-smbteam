...
# Report metadata
FIRM_NAME      = "Swift Law North"
SALES_REP      = "Jacob Meissner"
OUTPUT_PATH    = "swift-law-north/Swift Law North_August 27, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "RED",   "100% from referrals"),      # Lead Generation
    ("AMBER", "AMBER", "Overseas team, no phone#"),  # Intake
    ("AMBER", "AMBER", "4 partners, no ops layer"),  # Team
    ("RED",   "RED",   "No revenue tracked"),        # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Invisible across all 3 target case types — competitors own the search intent"),
    ("neg", "No homepage phone number and unconfirmed after-hours coverage risk cold leads"),
    ("neg", "All decisions run through 4 partners — no dedicated ops layer for 100+/month"),
    ("pos", "Real team already in place: 4 partners, ~10 underwriting staff, dedicated intake"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Consumer Attorneys",        "Ranks in all 3 areas", "Owns this search intent"),
    ("Francis Mailman Soumilas",  "FCRA market leader",   "Dominates credit-report search"),
    ("Sulaiman Law Group",        "Multi-area reach",     "Covers ID theft + FDCPA"),
]
CLIENT_REVIEWS      = "No listing yet"
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach 100+ Clients a Month"

SMB_MODEL_DESC = (
    "Leads, intake, team, and profit all have to work together, or growth stalls "
    "no matter how much you spend on ads. When they do, Swift Law North can turn "
    "its national reach and bilingual advantage into the dominant brand in this market."
)

GOAL_HEADLINE = "30-40 → 100+ clients/mo"
GOAL_DBM      = "Become the national brand people already trust"
...
PRIORITIES = [
    (
        ...
        [
            "Launch Google + Meta across all 3 case types",
            "Turn ads into a pipeline that runs without you",
            "Activate a dedicated Spanish-language track",
            "Build Avvo, Justia, FindLaw citations for SLN",
            "Fix homepage: add phone number, confirm mobile",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "69CD2B",
        [
            "Audit outsourced team's response times",
            "Close after-hours coverage gaps",
            "Add a visible phone number for urgent callers",
            "Build a defined lead follow-up sequence",
    ...
    (
        ...
        [
            "Add a dedicated ops layer above underwriting",
            "Build accountability across all 14+ staff",
            "Track case profitability by practice area",
            "Set a real revenue baseline to measure against",
            "Free the partners from managing everything",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$7,497", "$8,997/mo",
        "Google Ads · Meta Ads · Spanish campaigns · Directory buildout",
        "0091C9",
    ),
    (
        "MASTER'S CIRCLE",
...
CONSERVATIVE_RESULT = "$100,000 revenue · ~33× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (~19 cases/mo):"
AGGRESSIVE_RESULT   = "$475,000 revenue · ~40× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Launch Google + Meta campaigns nationally"),
    ("Day 14",  "Spanish-language campaign track goes live"),
    ("Week 2",  "Homepage phone number + mobile fix shipped"),
    ("Week 3",  "Directory citations built under SLN brand"),
    ("Month 3", "Master's Circle ops framework in place"),
]

CLOSING_QUOTE = (
    '"Every person out there who needs us finds us — that\'s what 500 clients a month actually looks like."'
)
