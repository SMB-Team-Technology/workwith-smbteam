...
# ═══════════════════════════════════════════════════════════════════
# FILL — All values sourced from completed audit sections
# ═══════════════════════════════════════════════════════════════════

# Report metadata
FIRM_NAME   = "Irvine Legal"
SALES_REP   = "Dan Bryant"
OUTPUT_PATH = "irvine-legal/Irvine Legal_June 2, 2026_Proposal.pptx"
...
PILLARS = [
    ...
    ("AMBER", "AMBER",     "No digital intake"),
    ("AMBER", "AMBER",     "Stalls in 1-2 weeks"),
    ("AMBER", "AMBER",     "No margin tracking"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — 3 neg + 1 pos
FINDINGS = [
    ("neg", "Zero Google reviews — invisible in every local construction law search while competitors hold all map pack positions."),
    ("neg", "100% referral-dependent — no digital engine exists to reach the $100K/month revenue goal."),
    ("neg", "Firm stalls within 1–2 weeks without Joshua — litigation bottleneck blocks any path to real time freedom."),
    ("pos", "Sales team closes subscription clients independently — a genuine intake foundation competitors at this stage rarely have."),
]

# Competitor table — 3 competitors
COMPETITORS = [
    ("SSHK Lawyers",                 "22 reviews", "Davis/Weber County map pack"),
    ("Fetzer Booth Mountain West Law", "8 reviews",  "Construction + business law"),
    ("Winder & Counsel, P.C.",        "AV Preeminent", "Construction law since 1983"),
]
...
SMB_MODEL_DESC = (
    ...
    "Missing any one means growth stalls no matter how hard you work. "
    "When all four work, Joshua can take that 6-week trip "
    "and trust the firm keeps running."
)

GOAL_HEADLINE = "$780K → $1.2M+ revenue"
...
PRIORITIES = [
    (
        ...
        [
            "Turn Google into a subscription client pipeline",
            "Own the Davis County construction law map pack",
            "Launch Meta ads targeting construction owners",
            "Rebuild site to capture leads, not just look good",
            "Own 'outsourced general counsel' before others do",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Add homepage form — capture leads in the moment",
            "Build follow-up sequence for cold ad-generated leads",
            "Define after-hours response for evening inquiries",
            "Train sales team on ad lead vs. referral approach",
            "Track every lead from first touch to signed client",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Document contract review process — free Joshua's time",
            "Track margin by line: subscription, litigation, RA",
            "Build plan to hire litigation attorney at $200K+",
            "Extend safe-absence window from 2 weeks toward 6",
            "Create profit plan that funds the nonprofit mission",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$4,847", "$5,697/mo",
        "Website rebuild · Google Ads · Local SEO · Meta Ads · GBP",
        "1D4ED8",
    ),
    (
        "ELITE COACH PLUS",
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $5,000–$14,500/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE      = "$5,000 (default)"
CONSERVATIVE_LABEL  = "Conservative  (6 cases/mo):"
CONSERVATIVE_RESULT = "$30,000 revenue · 6.0× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (22 cases/mo):"
AGGRESSIVE_RESULT   = "$110,000 revenue · 7-8× ROAS"

TIMELINE = [
    ("Day 1",    "Launch Google Ads for construction law keywords"),
    ("Day 14",   "Website rebuild begins; homepage form goes live"),
    ("Week 6",   "Review campaign hits 10+ Google reviews"),
    ("Week 8",   "NAP citations unified; GBP category corrected"),
    ("Month 3",  "Coaching: profit tracking + intake process live"),
]

CLOSING_QUOTE = (
    '"The firm runs itself, the mission is funded — '
    'and I take that 6-week Himalaya trip knowing nothing burns down while I\'m gone."'
)
