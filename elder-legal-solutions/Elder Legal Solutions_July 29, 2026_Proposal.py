...
# Report metadata
FIRM_NAME      = "Elder Legal Solutions"
SALES_REP      = "Randy Gold"
OUTPUT_PATH    = "elder-legal-solutions/Elder Legal Solutions_July 29, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "RED",   "100% referral-only"),
    ("RED",   "RED",   "10K+ unread emails"),
    ("RED",   "RED",   "Tamr's the only one"),
    ("AMBER", "AMBER", "Annual books only"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Intake buried under 10,000+ unread emails"),
    ("neg", "Tamr is the firm's only point of failure"),
    ("neg", "Absent from every major elder-law directory"),
    ("pos", "Revenue growing $100K-$120K/year on referrals alone"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("The Siegel Law Group",  "87+ reviews",   "geo-page network"),
    ("Elder Needs Law",       "21-30 reviews", "multi-platform presence"),
    ("Elder Law, P.A.",       "unconfirmed",   "active FB presence"),
]
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Fund Tamr's Raise"

SMB_MODEL_DESC = (
    "All four pillars — leads, intake, team, and profit — work together. "
    "When they do, Neal can step back from the inbox and Tamr can stop "
    "carrying the firm's core practice alone."
)

GOAL_HEADLINE = "$740K → $940K revenue"
GOAL_DBM      = "Fund Tamr's raise, not his overtime"
...
PRIORITIES = [
    (
        ...
        [
            "Launch Google, LSA & Meta ad campaigns",
            "Claim Avvo, Justia & Yelp directory spots",
            "Build geo-targeted pages like competitors",
            "Add attorney bios to build trust online",
    ...
    (
        ...
        [
            "Replace the 10,000-email inbox with a process",
            "Add fast follow-up to lift close rate to 50-75%",
            "Cover after-hours leads before they go cold",
            "Track every lead from first contact to close",
            "Free Neal from personally working every intake",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Hire the paralegal already identified",
            "Build a second person on core Medicaid/VA work",
            "Move to monthly financial reporting",
            "Track margin by practice area, not just revenue",
            "Turn $200K more revenue into Tamr's $50K raise",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$4,847", "$5,697/mo",
        "Local SEO · Google Ads · LSA · Meta Ads · Reporting",
        "1D4ED8",
    ),
    (
        "ELITE COACH PLUS",
        "$3,200", "$3,497/mo",
        "Weekly coaching · Masterminds · 1:1 sessions",
        "6D28D9",
    ),
]
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $9,000–$13,500/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE      = "$5,000"
CONSERVATIVE_LABEL  = "Conservative  (13 cases/mo):"
CONSERVATIVE_RESULT = "$65,000 revenue · 7.2× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (31 cases/mo):"
AGGRESSIVE_RESULT   = "$155,000 revenue · 11.5× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Launch Google, LSA & Meta ad campaigns"),
    ("Day 14",  "Local SEO & GBP live across all 5 offices"),
    ("Week 2",  "Homepage revamp adds attorney bios & CTAs"),
    ("Week 3",  "Elite Coach Plus coaching kickoff begins"),
    ("Month 3", "Baseline reporting tracks leads & close rate"),
]

CLOSING_QUOTE = (
    '"Tamr gets the raise he\'s earned — not from working harder, but '
    'because the firm finally runs without either of us carrying it alone."'
)
