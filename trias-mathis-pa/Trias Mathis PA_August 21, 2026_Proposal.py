...
# Report metadata
FIRM_NAME      = "Trias Mathis PA"
SALES_REP      = "Nick Holderman"
OUTPUT_PATH    = "trias-mathis-pa/Trias Mathis PA_August 21, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL", "0% digital presence"),
    ("AMBER", "AMBER",    "No defined process"),
    ("AMBER", "AMBER",    "Owner-run decisions"),
    ("RED",   "CRITICAL", "No cost tracking"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Zero digital presence across all 5 practice areas in Fort Myers"),
    ("neg", "Conflicting directories (Avvo, FindLaw, BBB) dilute local authority"),
    ("neg", "$17K-$19K/mo spent with no cost-per-case tracking"),
    ("pos", "19-city landing page infrastructure already built and ready"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Viles & Beckman",       "761 reviews", "40+ yrs · PI/WC"),
    ("The Dellutri Law Group","740 reviews", "Ft. Myers brand"),
    ("Farah & Farah",         "578 reviews", "Statewide · PPC"),
]
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $2.5M/Year"

SMB_MODEL_DESC = (
    "All four pillars — marketing, intake, team, and profit — must work "
    "together, or growth stalls no matter the ad spend. When all four work, "
    "Larry can finally build a firm that grows on his own terms, not a "
    "referral partner's."
)

GOAL_HEADLINE = "$1M → $2.5M revenue"
GOAL_DBM      = "Growth you control, not favors you depend on"
...
PRIORITIES = [
    (
        ...
        [
            "Launch Google Ads & LSA for all 5 practice areas",
            "Build a pipeline that runs without referral favors",
            "Fix directory listings diluting Fort Myers authority",
            "Launch Spanish campaigns for an untapped market",
            "Add video content, confirm site speed",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "69CD2B",
        [
            "Define an intake process for all 4 offices",
            "Add after-hours coverage — leads can't wait",
            "Track every lead from click to signed case",
            "Protect new ad spend from silent lead leaks",
            "Keep the Free Case Evaluation form converting",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "003A59",
        [
            "Start Elite Coach Plus for delegation & accountability",
            "Free Larry from vetting every new vendor",
            "Track cost-per-case across all 5 practice areas",
            "Know which practice area is worth growing",
            "Turn revenue growth into real take-home profit",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$7,497", "$8,997/mo",
        "Website · SEO · Google Ads · LSA · Social",
        "0091C9",
    ),
    (
        "ELITE COACH PLUS",
...
CONSERVATIVE_RESULT = "$30,000 revenue · 3.0× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (17 cases/mo):"
AGGRESSIVE_RESULT   = "$170,000 revenue · 3.4× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Launch Google Ads & LSA campaigns"),
    ("Day 14",  "Fix directory & NAP listings across offices"),
    ("Week 2",  "Reinforce website — video, confirm page speed"),
    ("Week 3",  "Kick off Elite Coach Plus coaching sessions"),
    ("Month 3", "Cost-per-case tracking live firm-wide"),
]

CLOSING_QUOTE = (
    '"I want a firm that future-proofs itself — one I control, not one '
    'I depend on referral partners for."'
)
