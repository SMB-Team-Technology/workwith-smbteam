...
# Report metadata
FIRM_NAME      = "Gilley Law"
SALES_REP      = "Nick Holderman"
OUTPUT_PATH    = "gilley-law/GilleyLaw_June18_2026_Proposal.pptx"
...
PILLARS = [
    ("AMBER", "AMBER",    "Single paid channel"),    # Lead Generation
    ("RED",   "CRITICAL", "No staff, no process"),   # Intake
    ("RED",   "CRITICAL", "Solo — no delegation"),   # Team
    ("AMBER", "AMBER",    "No P&L or tracking"),     # Profit Plan
]
...
FINDINGS = [
    ("neg", "Landing page only — zero organic search capability, every client today arrived through paid ads"),
    ("neg", "Solo intake — every missed call while Chris is in court loses a paid LSA lead"),
    ("neg", "Not in local 3-pack while Zaki Ali Trial Lawyers holds 462 reviews and dominates every search"),
    ("pos", "39 reviews at 4.8★ carried from prior work — strong GBP foundation for a new firm"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
# List 3 competitors + the client row is added automatically.
COMPETITORS = [
    ("Zaki Ali Trial Lawyers",    "462 reviews · 4.9★", "Dominates criminal 3-pack"),
    ("Eskew Law (Indianapolis)",  "Anderson landing page", "75+ yrs exp · bilingual"),
    ("Beeman Heifner Benge P.A.", "Local multi-attorney",  "Super Lawyers 2022–2026"),
]
CLIENT_REVIEWS      = "~20–39 reviews · 4.8★"
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $730,000 in 2026"

SMB_MODEL_DESC = (
    "The SMB Team model builds all four pillars together — "
    "Lead Gen, Intake, Team, and Profit — so every part of "
    "the firm works as a system. When each pillar is running, "
    "Chris can finally step away from the 10 PM nights and "
    "trust the firm runs itself."
)

GOAL_HEADLINE = "$730K 2026 Revenue Goal"
GOAL_DBM      = "No more 10 PM nights — firm runs itself"
...
PRIORITIES = [
    (
        ...
        [
            "Launch Google PPC for criminal + family law",
            "Optimize LSA spend and build review velocity",
            "Build SEO website — organic cases without more ads",
            "Counter Indianapolis firms bidding your keywords",
            "Turn Google into a pipeline that runs itself",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Install after-hours answering coverage",
            "Design intake process — not just Chris on calls",
            "Track every lead so no paid case goes unseen",
            "Add follow-up for leads who don't sign same day",
            "Free Chris from intake — one hire changes everything",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Hire intake coordinator to own consult calls",
            "Fix 7 directory errors — restore GBP trust",
            "Track cost per case across both practice areas",
            "Build monthly P&L — profit planned, not found",
            "Install systems that run without Chris in the room",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$4,847", "$5,697/mo",
        "Website · Google Ads · LSA · Meta Ads · SEO",
        "1D4ED8",
    ),
    (
        "ELITE COACH PLUS",
        "$3,200", "$3,497/mo",
        "1:1 coaching · intake design · financial tracking",
        "6D28D9",
    ),
]
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $5,500–$8,000/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE      = "$2,200"
CONSERVATIVE_LABEL  = "Conservative  (6 cases/mo):"
CONSERVATIVE_RESULT = "$13,200 revenue · 2.4× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (11 cases/mo):"
AGGRESSIVE_RESULT   = "$24,200 revenue · 3.0× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Ad campaigns launch — PPC + LSA optimization"),
    ("Day 14",  "Website rebuild underway, 7 NAP errors fixed"),
    ("Week 2",  "Meta retargeting activated, review asks begin"),
    ("Week 3",  "Coaching kickoff — intake design, KPIs set"),
    ("Month 3", "Full reporting: cost per case, channel ROI"),
]

CLOSING_QUOTE = (
    '"When the system works, the phone rings — whether Chris is '
    'in court, at home, or nowhere near his desk."'
)
