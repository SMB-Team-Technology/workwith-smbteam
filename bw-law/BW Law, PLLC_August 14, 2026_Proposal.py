...
Output: bw-law/BW Law, PLLC_August 14, 2026_Proposal.pptx
"""
...
# Report metadata
FIRM_NAME      = "BW Law, PLLC"
SALES_REP      = "Randy Gold"
OUTPUT_PATH    = "bw-law/BW Law, PLLC_August 14, 2026_Proposal.pptx"
...
PILLARS = [
    ("AMBER", "AMBER", "GBP/referrals work"),      # Lead Generation
    ("AMBER", "AMBER", "No homepage contact form"), # Intake
    ("RED",   "CRITICAL", "Owner reviews all work"), # Team
    ("AMBER", "AMBER", "No hiring/capacity plan"),   # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Owner personally reviews all contractor work — no delegated team"),
    ("neg", "$500K goal has no hiring or capacity plan behind it yet"),
    ("neg", "100% of leads come from referrals, GBP, and networking — no paid channel"),
    ("pos", "Highest star rating (5.0) of any firm in its local comparison set"),
]
...
SMB_MODEL_DESC = (
    ...
    "When hiring, intake, and team systems are in place, Sarah can finally step away "
    "from being \"chief everything officer\" and spend her time on business development."
)

GOAL_HEADLINE = "$200K → $500K revenue"
GOAL_DBM      = "Step back and trust the team handles it"
...
PRIORITIES = [
    ...
    (
        ...
        [
            "Add a homepage contact form for ready prospects",
            "Define a follow-up process beyond referrals",
            "Build after-hours coverage for missed leads",
            "Confirm PageSpeed and mobile experience",
            "Track intake-to-signed-client conversion",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "003A59",
        [
            "Build a hiring system so Sarah stops reviewing everything",
...
CLOSING_QUOTE = (
    '"Build a trusted team to delegate work and reduce personal stress — '
    'and finally have more time for business development and networking."'
)
