"""
Audit PowerPoint — Hargrave Family Law
Generates a 3-slide proposal PPTX.
"""
...
FIRM_NAME   = "Hargrave Family Law"
SALES_REP   = "Dan Bryant"
OUTPUT_PATH = "hargrave-family-law/Hargrave_Family_Law_07022026_Proposal.pptx"
...
PILLARS = [
    ("AMBER", "AMBER", "No paid ads observed"),     # Lead Generation
    ("AMBER", "AMBER", "Form buried; no chat"),      # Intake
    ("AMBER", "AMBER", "Structure unverified"),      # Team
    ("AMBER", "AMBER", "Revenue unconfirmed"),       # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

FINDINGS = [
    ("neg", "No Google Ads, LSA, or Meta ads observed — competitors hold every paid position while Hargrave relies solely on organic traffic"),
    ("neg", "FindLaw lists a different phone number than the website — this NAP mismatch suppresses local 3-pack rankings"),
    ("neg", "Contact form buried below the fold with no after-hours coverage — evening and weekend leads have no way to convert"),
    ("pos", "211 Google reviews at 4.9 stars — more than double the count of both named competitors (McClure 99, Goranson Bain 88)"),
]

COMPETITORS = [
    ("Goranson Bain Ausley", "88 reviews", "#1 Dallas · 26+ attorneys"),
    ("McClure Law Group",    "99 reviews", "Collaborative divorce SEO"),
    ("Powell Law Offices",   "unknown",    "Active Google Ads now"),
]
CLIENT_REVIEWS      = "211 reviews · 4.9★"
CLIENT_REVIEWS_NOTE = "← You are here"

STAGE_TEXT = "Stage 4: Small Business Manager  →  Stage 6: Law Firm Owner"

# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Build a Predictable Revenue Engine"

SMB_MODEL_DESC = (
    "Lead Generation. Intake. Self-Managing Team. Profit Plan. "
    "All four must work together — and right now, none of the four are operating at full strength. "
    "When they do, Jennifer can lead the firm she has built rather than be consumed by it."
)

GOAL_HEADLINE = "Organic only  →  Full Paid + Organic Pipeline"
GOAL_DBM      = "Lead the firm — and let it run without you"

PRIORITIES = [
    (
        "Build the", "Marketing Engine", "1D4ED8",
        [
            "Turn 211 reviews into top-of-page LSA visibility",
            "Launch collaborative divorce Google Ads first",
            "Fix NAP mismatch; activate weekly GBP posts",
            "Target McKinney/Collin County as a separate market",
            "Run Google, LSA, and Meta simultaneously from day one",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Move contact form above the fold on homepage",
            "Install after-hours lead capture across both offices",
            "Document intake process from first call to signed retainer",
            "Define follow-up sequence for leads who don't sign same day",
            "Set a response time standard for every inquiry channel",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Build a management layer across Dallas and McKinney",
            "Define accountability rhythms and KPIs by office",
            "Map cost per case by practice area and case type",
            "Set profit targets — not just revenue targets",
            "Coaching framework so Jennifer leads, not manages",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$4,847", "$5,697/mo",
        "Website · Google Ads · LSA · Meta Ads · Local SEO",
        "1D4ED8",
    ),
    (
        "ELITE COACH PLUS",
        "$3,200", "$3,497/mo",
        "Weekly coaching · Masterminds · Intake audit · Workshops",
        "6D28D9",
    ),
]
...
CONSERVATIVE_RESULT = "$24,000 revenue · 8× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (20 cases/mo):"
AGGRESSIVE_RESULT   = "$120,000 revenue · 10× ROAS"

TIMELINE = [
    ("Day 1",   "Google Search Ads & LSA launch"),
    ("Day 14",  "Website form & mobile speed fixes live"),
    ("Week 2",  "FindLaw NAP corrected; GBP posts active"),
    ("Week 3",  "Coaching kickoff & intake audit complete"),
    ("Month 3", "Meta video ads live; accountability systems set"),
]

CLOSING_QUOTE = (
    '"The reputation is built — the system to turn it into a predictable client pipeline is not, yet."'
)
