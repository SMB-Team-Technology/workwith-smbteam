...
# Report metadata
FIRM_NAME      = "Caress Law"
SALES_REP      = "Jacob Meissner"
OUTPUT_PATH    = "caress-law/Caress Law_August 5, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "RED",   "Organic/referral only"),
    ("AMBER", "AMBER", "No defined process"),
    ("RED",   "RED",   "Owner is the fallback"),
    ("AMBER", "AMBER", "Revenue known, margins not"),
]
...
FINDINGS = [
    ("neg", "Tammi remains the operational fallback for every system, blocking strategic growth"),
    ("neg", "Current fractional COO lacks law-firm experience, stalling attorney KPI work"),
    ("neg", "Manual probate, billing, and mail workflows block the move to flat-fee pricing"),
    ("pos", "4.4-star rating and a 2025 Best of the Rose City award show real market credibility"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
# List 3 competitors + the client row is added automatically.
COMPETITORS = [
    ("Fitzwater Law",         "85 reviews",  "4.8 stars · since 1931"),
    ("Nay & Friedenberg LLC", "76 reviews",  "4.7 stars · since 1984"),
    ("Chenoweth Law Group",   "127 reviews", "3.8 stars"),
]
CLIENT_REVIEWS      = "27 reviews"
CLIENT_REVIEWS_NOTE = "← You are here"

# Stage strip (right panel, Slide 1)
STAGE_TEXT = "Stage 4: Small Business Manager  →  Goal: Firm Runs Without Owner"

# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $7.6M"

SMB_MODEL_DESC = (
    "Automation and qualified leadership must work together — one without "
    "the other still leaves Tammi as the fallback. When both are in place, "
    "Tammi can finally step back into strategy instead of daily operations."
)

GOAL_HEADLINE = "$2.2M → $7.6M revenue"
GOAL_DBM      = "Step away from daily operations and trust the firm runs"

# Each priority: (line1, line2, accent_color_hex, [5 bullet strings])
PRIORITIES = [
    (
        "Automate the", "Operational Core", "1D4ED8",
        [
            "Cut the probate workflow from 39 steps to 27",
            "Automate the 3-hour monthly bill review",
            "Automate mail processing end to end",
            "Automate home office expense compilation",
            "Turn recovered hours into strategic time",
        ],
    ),
    (
        "Fix Leadership &", "Delegate for Real", "0F766E",
        [
            "Replace an underqualified COO with law-firm expertise",
            "Build attorney KPIs and accountability structure",
            "Give Tammi a true operational partner, not a title",
            "Establish weekly coaching and mastermind support",
            "Move ownership of daily systems off Tammi's desk",
        ],
    ),
    (
        "Build Toward", "Predictable Profit", "6D28D9",
        [
            "Move probate and trust admin from hourly to flat fees",
            "Use automation efficiency to make flat fees viable",
            "Track progress toward the $7.6M 3-year goal",
            "Close the local-search visibility gap over time",
            "Turn revenue growth into real, plannable profit",
        ],
    ),
]
...
PACKAGES = [
    (
        "LEGAL AI WORKFORCE — FRACTIONAL CTO L2",
        "$4,997", "$5,797/mo",
        "Monthly custom automations · Clio/Lawmatics/Outlook integration",
        "1D4ED8",
    ),
    (
        "FCOO ADVISOR",
        "$3,297", "$3,797/mo",
        "Law-firm ops leadership · weekly coaching · masterminds",
        "6D28D9",
    ),
]

BUNDLE_TOTAL   = "$8,294 / mo"
BUNDLE_SAVINGS = "Save $1,300/mo by bundling"

# No ad spend is recommended for this engagement — the discovery call was
# a Fractional CTO / AI-automation engagement, not a marketing discussion.
AD_SPEND_NOTE = (
    "No ad spend recommended — this engagement is automation & operations, not marketing"
)

ROI_CARD_TITLE      = "WHAT THIS INVESTMENT INCLUDES"
AVG_CASE_VALUE       = "$14,997"
CONSERVATIVE_LABEL  = "Foundation Sprint (one-time):"
CONSERVATIVE_RESULT = "Standalone $19,997 · save $5,000"
AGGRESSIVE_LABEL    = "Recurring total (bundled):"
AGGRESSIVE_RESULT   = "$8,294/mo · save $1,300/mo"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Kick off AI Foundation Sprint + CTO onboarding"),
    ("Day 14",  "Begin FCOO Advisor transition from current COO"),
    ("Week 2",  "Scope first automations: probate workflow priority"),
    ("Week 3",  "Begin attorney KPI and accountability build"),
    ("Month 3", "First custom automations live; coaching underway"),
]

CLOSING_QUOTE = (
    '"Step away from being the fallback for every system — '
    'and finally have the time to lead Caress Law, not just run it."'
)
...
def build_slide3(prs):
    ...
    # Investment summary card (relabeled from ROI-on-ad-spend: no ad spend
    # is recommended for this engagement — see PACKAGE INSTRUCTION override)
    add_rect(slide, 4.96, 1.12, 4.82, 1.44, fill=ROI_BG)
    add_text(slide, ROI_CARD_TITLE,
             5.14, 1.18, 4.52, 0.24, 8, ROI_GREEN, bold=True)
    add_text(slide, "One-time onboarding fee:", 5.14, 1.46, 2.10, 0.28, 9, rgb("1E293B"), bold=True)
