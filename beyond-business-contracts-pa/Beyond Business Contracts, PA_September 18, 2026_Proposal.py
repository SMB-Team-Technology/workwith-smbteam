...
# Report metadata
FIRM_NAME      = "Beyond Business Contracts, PA"
SALES_REP      = "Michael Kopp"
OUTPUT_PATH    = "beyond-business-contracts-pa/Beyond Business Contracts, PA_September 18, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL", "100% networking, no system"),
    ("AMBER", "AMBER",    "No documented process"),
    ("RED",   "CRITICAL", "Just owner + 1 part-time VA"),
    ("AMBER", "AMBER",    "No margin visibility"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

FINDINGS = [
    ("neg", "You are the entire team, carrying litigation and the transactional pivot alone."),
    ("neg", "New business runs entirely on personal networking, with no digital system to scale beyond your own hours."),
    ("neg", "Intake runs through one part-time VA with no documented process or after-hours coverage."),
    ("pos", "A 2026 Avvo Client's Choice award and Martindale Client Champion award show real credibility already."),
]

COMPETITORS = [
    ("Dean & Dean, LLP",        "Established reviews", "Real estate/closings leader"),
    ("Glover Law Firm, PLLC",   "Established reviews", "Dominant local legal brand"),
    ("Michael J. Cooper, P.A.", "Few reviews found",   "Business law competitor"),
]
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach a Million-Dollar Firm"

SMB_MODEL_DESC = (
    "All four pillars — lead gen, intake, team, and profit — have to work "
    "together for sustainable growth. When they do, Tania can finally step "
    "back from carrying every case herself."
)

GOAL_HEADLINE = "$250K → $1M revenue"
GOAL_DBM      = "Step back and lead as CEO, not caseworker"

PRIORITIES = [
    (
        "Deploy", "AI Systems", "0091C9",
        [
            "Stand up AI workspace + first Skill",
            "Deploy AI agent for document collection",
            "Automate deadline tracking for closings",
            "Add AI-assisted closing coordination",
            "Fractional CTO leads the AI rollout",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "69CD2B",
        [
            "Document a real intake workflow",
            "Add after-hours coverage for inquiries",
            "Track every lead from call to signed",
            "Standardize follow-up for slow responses",
            "Turn networking contacts into a pipeline",
        ],
    ),
    (
        "Build Team &", "Profit Clarity", "003A59",
        [
            "Weekly coaching + 90-day game plan",
            "Map path to hiring first associate",
            "Track margin, not just gross revenue",
            "Build org structure beyond just 2 people",
            "Plan next phase: CFO once past $400K",
        ],
    ),
]
...
PACKAGES = [
    (
        "AI ACCELERATOR L1 — FRACTIONAL CTO",
        "$3,297", "$3,797/mo",
        "AI workspace · document + deadline agents · closings",
        "0091C9",
    ),
    (
        "ELITE COACH",
        "$2,600", "$3,497/mo",
        "Weekly coaching · 90-day plan · practice masterminds",
        "003A59",
    ),
]

BUNDLE_TOTAL   = "$5,897 / mo"
BUNDLE_SAVINGS = "Save $1,397/mo by bundling"

AD_SPEND_NOTE = None  # no marketing/ads package is part of this recommendation

AVG_CASE_VALUE     = "$X,XXX"
CONSERVATIVE_LABEL = "Conservative  (X cases/mo):"
CONSERVATIVE_RESULT = "$XX,XXX revenue · X.X× ROAS"
AGGRESSIVE_LABEL   = "Aggressive  (X cases/mo):"
AGGRESSIVE_RESULT  = "$XX,XXX revenue · X.X× ROAS"

TIMELINE = [
    ("Day 1",   "AI workspace setup begins"),
    ("Day 14",  "First AI Skill deployed for documents"),
    ("Week 2",  "Fractional CTO workflow-mapping session"),
    ("Week 3",  "Coaching kickoff + 90-day game plan set"),
    ("Month 3", "Deadline tracking & closing coordination live"),
]

CLOSING_QUOTE = (
    '"A scalable, AI-driven firm that lets me finally step back from '
    'litigation — and become the top title attorney in Ocala."'
)
