...
# Report metadata
FIRM_NAME      = "BGN Attorneys"                     # FILL: full firm name
SALES_REP      = "Dan Bryant"                        # FILL: rep's full name
OUTPUT_PATH    = "bgn-attorneys/BGN_Attorneys_September 2, 2026_Proposal.pptx"  # FILL: output path

# Optional images
WEBSITE_SCREENSHOT_PATH = None   # Set to a local PNG path to show a screenshot; None = skip entirely

# ── Slide 1 ──────────────────────────────────────────────────────
URGENCY_SCORE = "8"                          # FILL: copy from section_05 urgency score

# Pillar status — each is ("RED" | "AMBER" | "GREEN", label, one-line detail)
PILLARS = [
    ("AMBER", "AMBER",    "Mostly referrals"),      # FILL: Lead Generation
    ("AMBER", "AMBER",    "Partner runs intake"),   # FILL: Intake
    ("AMBER", "AMBER",    "No manager layer"),      # FILL: Team
    ("RED",   "CRITICAL", "No profit plan"),        # FILL: Profit Plan
]
...
FINDINGS = [
    ("neg", "Case growth is outpacing hiring and capacity"),
    ("neg", "No profit plan connects growth to comp targets"),
    ("neg", "Site & directory issues erode trust at zero fix cost"),
    ("pos", "100% employee retention — zero churn to date"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
# List 3 competitors + the client row is added automatically.
COMPETITORS = [
    ("Khan Injury Law",   "Many reviews", "Established since 2007"),
    ("Brumley Law Firm",  "Many reviews", '"#1 in Western WA" positioning'),
    ("Herschensohn Law",  "Many reviews", "35+ yrs serving Kent market"),
]
CLIENT_REVIEWS      = "Very few reviews"      # FILL: client's actual review count
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach 40-50 Cases a Month"  # FILL

SMB_MODEL_DESC = (
    "All four pillars — leads, intake, team, and profit — must work together. "
    "When they do, Scott, Tyson, and Dan can finally step away and trust the firm runs."
)  # FILL: brief model description

GOAL_HEADLINE = "30 → 40-50 cases/mo"   # FILL: e.g. "$1M → $1.8M revenue"
GOAL_DBM      = "Step away and trust the firm runs"  # FILL: DBM outcome phrase

# Each priority: (line1, line2, accent_color_hex, [5 bullet strings])
PRIORITIES = [
    (
        "Fix Free,", "Quick Wins", "0091C9",
        [
            "Fix the broken www redirect (no cost)",
            "Correct Spokane/Pasco listings to Kent",
            "Start a simple review-request process",
            "Turn referral trust into local visibility",
            "Set the stage for marketing in 2027",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "69CD2B",
        [
            "Define after-hours follow-up coverage",
            "Document a lead-to-case handoff process",
            "Give the new intake specialist a clear process",
            "Capture leads already coming in",
            "Free up partner time from daily intake",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "003A59",
        [
            "Build a hiring sequence for 40-50 cases/mo",
            "Create a manager layer under the partners",
            "Build a profit plan tied to comp targets",
            "Turn $15K avg. case value into a target",
            "Guard against over-hiring eating take-home",
        ],
    ),
]
...
PACKAGES = [
    (
        "ELITE COACH PLUS (STANDALONE)",
        "$3,497", "$3,497/mo",
        "Weekly group coaching · Hiring sequence · Profit plan",
        "003A59",
    ),
]

BUNDLE_TOTAL   = "$3,497 / mo"                      # FILL: sum of bundled prices
BUNDLE_SAVINGS = "Standalone pricing — no bundle discount applies"        # FILL

AD_SPEND_NOTE = (
    "No ad spend included — marketing deferred to January 2027"
)  # FILL

AVG_CASE_VALUE     = "$15,000"                        # FILL
CONSERVATIVE_LABEL = "This engagement:"              # FILL
CONSERVATIVE_RESULT = "No ad spend included"         # FILL
AGGRESSIVE_LABEL   = "Marketing revisit:"            # FILL
AGGRESSIVE_RESULT  = "Planned for January 2027"      # FILL

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Kick off Elite Coach Plus onboarding"),
    ("Day 14",  "Build hiring sequence for 40-50 cases/mo"),
    ("Week 2",  "Draft org chart & partner role clarity"),
    ("Week 3",  "Fix www redirect & NAP listings (no cost)"),
    ("Month 3", "Profit plan tied to $500K comp target"),
]

CLOSING_QUOTE = (
    '"We built this firm to run itself — not to run us into the ground."'
)  # FILL
