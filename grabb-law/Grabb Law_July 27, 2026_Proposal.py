...
OUTPUT_PATH    = "grabb-law/Grabb Law_July 27, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "RED",   "$25K/mo, zero visibility"),   # Lead Generation
    ("AMBER", "AMBER", "No CRM/lead tracking"),        # Intake
    ("AMBER", "AMBER", "No manager layer"),             # Team
    ("RED",   "RED",   "Revenue unconfirmed"),          # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Spending ~$25K/mo on marketing with zero visibility in 6 local search tests"),
    ("neg", "NAP inconsistency and ~46 reviews vs. 92-348 for the top 3 competitors"),
    ("neg", "Revenue and marketing ROI unconfirmed — no baseline to measure against"),
    ("pos", "24/7 answering service with immediate follow-up already in place"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Karnas Law Firm",     "348 reviews", "Dedicated MVA/moto pages"),
    ("Goldberg & Osborne",  "243 reviews", "Multi-market ad spend"),
    ("Zanes Law",           "92 reviews",  "Active PPC landing pages"),
]
CLIENT_REVIEWS      = "~46 reviews"
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Add 5-10 Cases a Month"

SMB_MODEL_DESC = (
    "All four pillars — Lead Gen, Intake, Team, and Profit — have to work together. "
    "Miss one and the $25,000 Grabb Law already spends keeps circulating without becoming "
    "more cases. When all four work, Robert can finally step away from day-to-day oversight."
)

GOAL_HEADLINE = "+5-10 cases / month"
GOAL_DBM      = "Step away from day-to-day oversight"
...
PRIORITIES = [
    (
        ...
        [
            "Redirect part of the $25K/mo into Google & LSA",
            "Fix NAP inconsistency across all directories",
            "Turn Google into a pipeline that runs without Robert",
            "Launch Meta retargeting for website visitors",
            "Rebuild attorney bio & site performance",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Track where leads go after the phone rings",
            "Build a follow-up sequence for non-signers",
            "Confirm web forms route into 24/7 intake",
            "Speed up decisions on borderline case criteria",
            "Turn the answering service into a real funnel",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Name a dedicated intake/ops point of contact",
            "Confirm revenue and true marketing ROI",
            "Track cost-per-case across every channel",
            "Coach Robert's team toward self-management",
            "Turn the case goal into a tracked number",
        ],
    ),
]
...
CONSERVATIVE_RESULT = "$13K-$15K revenue · 1.3x ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (6-7 cases/mo):"
AGGRESSIVE_RESULT   = "$48K-$52K revenue · 2.5x ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Launch Google Ads & LSA for car accident searches"),
    ("Day 14",  "Redirect existing budget into digital channels"),
    ("Week 2",  "Fix NAP across Avvo, FindLaw, Yelp, BBB"),
    ("Week 3",  "Begin website optimization & attorney bio"),
    ("Month 3", "Elite Coach Plus kickoff builds accountability"),
]

CLOSING_QUOTE = (
    '"The budget is already there. What\'s missing is the system — '
    'and the freedom that comes with never managing it alone."'
)
