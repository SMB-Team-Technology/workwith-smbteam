"""
Audit PowerPoint — Danton Legal
SMB Team Proposal Deck — 3 slides
"""
...
FIRM_NAME   = "Danton Legal"
SALES_REP   = "Jacob Meissner"
OUTPUT_PATH = "danton-legal/Danton_Legal_June_4_2026_Proposal.pptx"

WEBSITE_SCREENSHOT_PATH = None

# ── Slide 1 ──────────────────────────────────────────────────────
# Urgency score from section_05
URGENCY_SCORE = "7"

# Pillars from section_05 — (status, label, one-line detail ≤20 chars)
PILLARS = [
    ("RED",   "CRITICAL", "No web, GBP, or ads"),   # Lead Generation
    ("RED",   "CRITICAL", "No website or forms"),    # Intake
    ("RED",   "CRITICAL", "Solo, no staff"),          # Team
    ("AMBER", "AMBER",    "Referral-dependent"),      # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings from section_executive_summary — consequence-first
FINDINGS = [
    ("neg", "dantonlegal.com is blank — no client can find or contact the firm"),
    ("neg", "No GBP or reviews = zero local search visibility in Pasadena"),
    ("neg", "Competing IP firms run ads; Danton Legal has no presence at all"),
    ("pos", "39 years of IP excellence: HP, Sony Records & Microsoft"),
]

# Competitors from section_06 — (name, reviews, brief note)
COMPETITORS = [
    ("The Soni Law Firm",      "11+ reviews",  "Same ZIP · IP/entertainment"),
    ("Cotman IP Law Group",    "9+ reviews",   "Pasadena #1 IP firm"),
    ("Mandour & Associates",   "3+ reviews",   "Running Pasadena IP ads"),
]
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Build the Danton Legal Brand"

SMB_MODEL_DESC = (
    "Lead Generation, Intake, Team, and Profit must all "
    "work together. When each pillar is built, Danton Legal "
    "generates clients and income under your own name — "
    "independently of any referral or prior employer."
)

GOAL_HEADLINE = "Build Danton Legal's Client Pipeline"
GOAL_DBM      = "A practice that carries your name — and runs on its own"

PRIORITIES = [
    (
        "Build the", "Marketing Engine", "1D4ED8",
        [
            "Launch website with IP & entertainment law pages",
            "Claim and optimize Google Business Profile",
            "Fix address conflicts across Avvo, Justia, State Bar",
            "Run Pasadena IP & trademark Google Ads campaigns",
    ...
    (
        ...
        [
            "Add consultation request form to the new website",
            "Build 48-hr follow-up for every new inquiry",
            "Create after-hours contact and callback process",
            "Set up a system to track every lead and inquiry",
            "Every IP inquiry becomes a scheduled consultation",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Track revenue by case type — find what pays best",
            "Set monthly revenue and profit targets quarterly",
            "Elite Coach Plus kickoff: intake + business plan",
            "Plan first hire: paralegal or intake coordinator",
            "Own your numbers before you scale the team",
        ],
    ),
]

# ── Slide 3 ──────────────────────────────────────────────────────
# Packages from section_11 — (label, bundled_price, retail_price, services_line, accent_hex)
PACKAGES = [
    (
        "FULL SERVICE MARKETING — STARTER",
        "$4,847", "$5,697/mo",
        "Website · Google Ads · LSA · Local SEO · GBP optimization",
        "1D4ED8",
    ),
    (
        "ELITE COACH PLUS",
        "$3,200", "$3,497/mo",
        "Weekly coaching · 1:1 calls · Masterminds · Workshops",
        "6D28D9",
    ),
]
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $6,000–$6,500/mo paid directly to Google/Meta"
)

# ROI from section_11 Block 4
AVG_CASE_VALUE      = "$7,500"
CONSERVATIVE_LABEL  = "Conservative  (~7 cases/mo):"
CONSERVATIVE_RESULT = "~$52,500 revenue · ~8.8× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (~10 cases/mo):"
AGGRESSIVE_RESULT   = "~$75,000 revenue · ~11.5× ROAS"

# Timeline — from section_11 first 90 days (≤55 chars each)
TIMELINE = [
    ("Day 1",   "Launch Google Ads for Pasadena IP/trademark terms"),
    ("Day 14",  "Build + launch Danton Legal website with IP pages"),
    ("Week 2",  "Create and fully optimize Google Business Profile"),
    ("Week 3",  "Audit + correct NAP across Avvo, Justia, Lawyers.com"),
    ("Month 3", "Elite Coach Plus kickoff: intake + financial plan"),
]

# Closing quote — from section_executive_summary, translated to owner's voice
CLOSING_QUOTE = (
    '"After 39 years of building other firms\' reputations, '
    'Danton Legal is the name clients search for, find, and hire — starting now."'
)
