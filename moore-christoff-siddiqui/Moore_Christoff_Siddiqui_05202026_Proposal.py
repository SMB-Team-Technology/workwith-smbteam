...
# Report metadata
FIRM_NAME      = "Moore Christoff & Siddiqui"
SALES_REP      = "Dan Bryant"
OUTPUT_PATH    = "moore-christoff-siddiqui/Moore_Christoff_Siddiqui_05202026_Proposal.pptx"
...
PILLARS = [
    ...
    ("AMBER", "AMBER",     "Speed-to-lead gaps"),
    ("AMBER", "AMBER",     "No managing partner"),
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $3M by End of 2027"

SMB_MODEL_DESC = (
    "Lead Generation, Intake, Team, and Profit must all work together — "
    "not one at a time. When all four are in place, Sam can step back from "
    "the day-to-day and run Moore Christoff & Siddiqui as CEO, with cases "
    "coming in from Fairfax and Alexandria whether or not he is in the building."
)

GOAL_HEADLINE = "$2M → $3M by end of 2027"
GOAL_DBM      = "Lead the firm as CEO, not its hardest-working attorney"
...
PRIORITIES = [
    (
        ...
        [
            "Fix GBP to 'Criminal Defense Lawyer' — today",
            "Activate LSA: earn the top Google position",
            "Launch Google Ads for DUI & reckless in Fairfax",
            "Build city pages: Alexandria + Fairfax criminal",
            "Turn Google into a case pipeline that runs itself",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "After-hours protocol: no call goes unanswered",
            "Speed-to-lead: first response within 15 minutes",
            "Follow-up sequence for every unconverted inquiry",
            "Your 80-90% close rate gets a chance on every lead",
    ...
    (
        ...
        [
            "Define managing partner role — free Sam as CEO",
            "Build firm playbook: operations without partners",
            "Track profit by case type: DUI vs reckless driving",
            "Weekly coaching builds the leadership structure",
            "Make $3M a plan — profit by design, not discovery",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$7,397", "$8,997/mo",
        "Google Ads · LSA · SEO · Meta · GBP optimization · City pages",
        "1D4ED8",
    ),
    (
        "MASTER'S CIRCLE",
        "$4,600", "$4,997/mo",
        "Weekly coaching · Leadership systems · Intake protocol · Masterminds",
        "6D28D9",
    ),
]
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $13,200–$25,000/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE      = "~$4,500"
CONSERVATIVE_LABEL  = "Conservative  (13 cases/mo):"
CONSERVATIVE_RESULT = "$58,500 revenue · 4.4× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (30 cases/mo):"
AGGRESSIVE_RESULT   = "$135,000 revenue · 5.4× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Google Ads launch — Fairfax & Alexandria criminal"),
    ("Day 14",  "LSA verification submitted for all 8 attorneys"),
    ("Week 2",  "GBP category fixed; cover photo updated"),
    ("Week 3",  "Coaching kickoff: leadership + intake protocol"),
    ("Month 3", "City landing pages live — SEO foundation built"),
]

CLOSING_QUOTE = (
    '"The firm I want runs whether I\'m in trial or not — '
    'and the cases keep coming in from Fairfax and Alexandria."'
)
