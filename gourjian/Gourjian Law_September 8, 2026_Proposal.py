...
Slide 1 — Where Gourjian Law Stands Today         (assessment overview)
Slide 2 — Your Growth Plan: 3 Priorities    (action plan)
Slide 3 — Your Investment & What's Next     (pricing + first 90 days)

Output: gourjian/Gourjian Law_September 8, 2026_Proposal.pptx
"""
...
# Report metadata
FIRM_NAME      = "Gourjian Law"
SALES_REP      = "Nick Holderman"
OUTPUT_PATH    = "gourjian/Gourjian Law_September 8, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL", "100% referrals only"),
    ("RED",   "CRITICAL", "1 untracked inbox"),
    ("AMBER", "AMBER",    "No ops layer yet"),
    ("AMBER", "AMBER",    "No reinvest plan"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Babachanian already outranks Gourjian Law in GC search"),
    ("neg", "Every inquiry funnels through one untracked email inbox"),
    ("neg", "Growth depends entirely on referrals and personal hiring effort"),
    ("pos", "60% profit margin, 20%+ annual growth — exceptional clarity"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
# Review counts are unconfirmed via a direct Google listing — qualitative
# framing used instead of specific numbers, per audit data-integrity rules.
COMPETITORS = [
    ("Babachanian | Legal Counsel", "Outranks in search", "Dedicated GC service pages"),
    ("KAASS Law",                   "Highest visibility", "Active digital marketing"),
    ("Lurie & Associates / Macias", "Ranks ahead",         "Basic SEO Gourjian lacks"),
]
CLIENT_REVIEWS      = "No search presence"
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Scale Beyond Referrals"

SMB_MODEL_DESC = (
    "All four pillars — leads, intake, team, and profit — have to work "
    "together for growth to compound. When they do, Gourjian Law's reputation can "
    "finally reach clients beyond Varand's personal network."
)

GOAL_HEADLINE = "$3M+ → More Attorneys, More Reach"
GOAL_DBM      = "The firm grows without depending only on referrals"
...
PRIORITIES = [
    (
        ...
        [
            "Fix directory listings across every platform",
            "Build local SEO for GC, real estate, litigation",
            "Launch firm-authored content, not syndicated news",
            "Test targeted paid visibility at low cost",
            "Own search terms competitors haven't claimed yet",
        ],
    ),
    (
        "Fix Intake &", "Protect Every Inquiry", "69CD2B",
        [
            "Add a real contact form — not just email",
            "Track every inquiry so none get missed",
            "Build a fast-response process for referrals",
            "Protect the reputation referrals depend on",
            "Turn inquiries into a process, not luck",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "003A59",
        [
            "Build a repeatable attorney hiring system",
            "Add ops leadership so hiring isn't all on you",
            "Turn the 20% reinvestment idea into a real budget",
            "Free up billable hours spent managing growth",
            "A firm that grows without your constant oversight",
        ],
    ),
]
...
# Single-product engagement — only one package recommended for this account.
PACKAGES = [
    (
        "FULL SERVICE MARKETING — DOMINATE",
        "$12,497", "$12,497/mo",
        "Content, local SEO, directory cleanup, targeted ads",
        "0091C9",
    ),
]

BUNDLE_TOTAL   = "$12,497 / mo"
BUNDLE_SAVINGS = "Single-service engagement — no bundling savings apply"

AD_SPEND_NOTE = (
    "+ Recommended ad spend: $4,500–$18,000/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE     = "$5,000"
CONSERVATIVE_LABEL = "Conservative (5–6 inquiries/mo):"
CONSERVATIVE_RESULT = "$25,500 revenue · 5.7× ROAS"
AGGRESSIVE_LABEL   = "Aggressive ($18K/mo spend):"
AGGRESSIVE_RESULT  = "$45,000 revenue · 2.5× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Kick off directory & NAP cleanup"),
    ("Day 14",  "Local SEO & GBP work underway"),
    ("Week 2",  "First firm-authored content published"),
    ("Week 3",  "Contact form & inquiry tracking live"),
    ("Month 3", "Targeted paid presence testing visibility"),
]

CLOSING_QUOTE = (
    '"My reputation built this firm — now it\'s time to let it be found '
    'by more than the people who already know me."'
)
