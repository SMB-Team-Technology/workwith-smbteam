...
# Report metadata
FIRM_NAME      = "Aero Law Center"
SALES_REP      = "Dan Bryant"
OUTPUT_PATH    = "aero-law-center/Aero_Law_Center_July 24, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "RED",   "Ads paused, SEO ended"),
    ("AMBER", "AMBER", "9-11% lead-to-retained"),
    ("AMBER", "AMBER", "Runs through one attorney"),
    ("AMBER", "AMBER", "No channel profit view"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Google Ads paused on a payment issue — zero paid search visibility"),
    ("neg", "Paid SEO retainer ended; most recent blog post is over a year old"),
    ("neg", "Orlando and Atlanta, both named priority cities, have no page at all"),
    ("pos", "190 Google reviews at 4.9 stars — a real, verifiable reputation asset"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Tripp Scott",            "243 reviews", "39-lawyer firm, Martindale"),
    ("Aviation Legal Group",   "not found",   "AV Preeminent since 1997"),
    ("Lapayowker Jet Counsel", "not found",   "Chambers HNW ranked"),
]
CLIENT_REVIEWS      = "190 reviews"
CLIENT_REVIEWS_NOTE = "<- You are here (4.9 stars)"
...
SMB_MODEL_DESC = (
    ...
    "When they do, Aero Law Center can finally step away from routing "
    "every decision through one attorney and become the national platform "
    "it already brands itself as."
)

GOAL_HEADLINE = "$3M -> $5-7M revenue"
GOAL_DBM      = "Build the largest aviation law firm in the US"
...
PRIORITIES = [
    (
        ...
        [
            "Reactivate Google Ads — the account is just paused",
            "Turn Google into a client pipeline that runs itself",
            "Build the missing Aircraft Lien page",
            "Launch Orlando and Atlanta pages, your named targets",
            "Clean up GBP so your 4.9-star reputation gets found",
        ],
    ),
    (
        ...
        [
            ...
            "Add after-hours coverage for national inquiries",
            "Fix the gap behind the 9-11% lead-to-retained rate",
            "Convert more of the leads you already paid for",
            "Stop losing clients you already earned the hard way",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "FCOO Director builds ops so John isn't the bottleneck",
            "Firm-owned GBP access, not John's personal account",
            "Track profit by channel, not by one bad month",
            "Free John to do only the $700/hr work he must do",
            "Build a firm that runs without him in every room",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$15,997", "$18,997/mo",
        "Website rebuild · Google Ads · LSA · Meta Ads · Local SEO",
        "1D4ED8",
    ),
    (
        "MASTER'S CIRCLE + FCOO DIRECTOR",
        "$8,394", "$10,794/mo",
        "Group coaching · Masterminds · Dedicated FCOO Director",
        "6D28D9",
    ),
]
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $6,700-$105,000/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE      = "~$9,000 (blended estimate)"
CONSERVATIVE_LABEL  = "Conservative  (~8 cases/mo):"
CONSERVATIVE_RESULT = "$70,236 revenue - 10.5x ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (~147 cases/mo):"
AGGRESSIVE_RESULT   = "$1,320,939 revenue - 12.6x ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Resolve Google Ads payment issue, relaunch"),
    ("Day 14",  "Launch conservative paid channels at $6,700/mo"),
    ("Week 2",  "Build Aircraft Lien and Orlando/Atlanta pages"),
    ("Week 3",  "Clean up GBP ownership and NAP consistency"),
    ("Month 3", "Master's Circle and FCOO Director kickoff"),
]

CLOSING_QUOTE = (
    '"You already built the largest aviation law firm brand in the '
    'country. Now let\'s build the systems to make it true."'
)
