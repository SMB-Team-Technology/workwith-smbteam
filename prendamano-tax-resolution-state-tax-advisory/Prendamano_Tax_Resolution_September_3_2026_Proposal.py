...
# Report metadata
FIRM_NAME      = "Prendamano Tax Resolution & State Tax Advisory"
SALES_REP      = "Nick Holderman"
OUTPUT_PATH    = "prendamano-tax-resolution-state-tax-advisory/Prendamano_Tax_Resolution_September_3_2026_Proposal.pptx"
...
PILLARS = [
    ("RED", "CRITICAL", "$4.2K/mo, 1 client"),   # Lead Generation
    ("RED", "CRITICAL", "No delegation"),        # Intake
    ("RED", "CRITICAL", "Solo, no staff"),       # Team
    ("RED", "CRITICAL", "Barely breaks even"),   # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "$4,200/mo in marketing has produced one signed client since launch"),
    ("neg", "Zero Google Ads, GBP, or LSA presence despite high-intent IRS searches"),
    ("neg", "No team — Jennifer personally handles every intake call herself"),
    ("pos", "26+ years of experience and an active weekly blog build real credibility"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
# Review counts are unconfirmed for this market — shown qualitatively rather
# than as exact figures, per the review-data verification rule.
COMPETITORS = [
    ("Tenenbaum Law, P.C.",              "Established", "Same city · 25+ yrs"),
    ("Long Island Tax Resolution Svcs.", "Established", "Direct niche match"),
    ("J. David Tax Law",                 "Established", "Multi-state brand"),
]
CLIENT_REVIEWS      = "New firm"
...
SMB_MODEL_DESC = (
    ...
    "together. Right now marketing spend is going out faster than it comes "
    "back. When all four pillars work, Jennifer can finally step back and "
    "build the firm she's working toward."
)

GOAL_HEADLINE = "Today → $50,000/mo"
GOAL_DBM      = "A firm that runs itself and pays for college"
...
PRIORITIES = [
    (
        ...
        [
            "Launch Google Ads for IRS tax searches",
            "Claim & optimize Google Business Profile",
            "Set up Local Service Ads, Google Screened",
            "Fix Facebook targeting once Google is live",
    ...
    (
        ...
        [
            "Move contact form onto the homepage",
            "Fix two wrong phone numbers on site",
            "Build a follow-up sequence for every lead",
            "Add after-hours coverage for urgent calls",
    ...
    (
        ...
        [
            "Track cost per client on every dollar spent",
            "Build a delegated intake process",
            "Set up conversion tracking on all ad spend",
            "Plan margins instead of discovering them later",
            "Free your time for casework, not marketing",
        ],
    ),
]
...
# Only one package recommended — Full Service Marketing, Essentials tier,
# quoted standalone at full retail (no coaching, no bundle discount) per
# locked seller override in package_decision.json.
PACKAGES = [
    (
        "FULL SERVICE MARKETING — ESSENTIALS",
        "$3,797", "$3,797/mo",
        "Google Ads · GBP · LSA · Website Conversion Fixes",
        "0091C9",
    ),
]

BUNDLE_TOTAL   = "$3,797 / mo"
BUNDLE_SAVINGS = "Quoted standalone at full retail — no bundle discount"
...
CONSERVATIVE_RESULT = "$10,000 revenue · 3.3× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (2 cases/mo):"
AGGRESSIVE_RESULT   = "$20,000 revenue · 4.0× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Google Ads campaigns go live for IRS searches"),
    ("Day 14",  "Google Business Profile claimed & optimized"),
    ("Week 2",  "Contact form fixed, phone numbers corrected"),
    ("Week 3",  "Conversion tracking live on all ad spend"),
    ("Month 3", "Local Service Ads & Google Screened status"),
]

CLOSING_QUOTE = (
    '"Financial autonomy and time freedom — a firm worth selling, '
    'and a future funded for my kids."'
)
