...
# Report metadata
FIRM_NAME      = "Gaydos Law, P.C."
SALES_REP      = "Dan Bryant"
OUTPUT_PATH    = "gaydos-law/Gaydos Law_September 15, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "RED",   "Waning referrals only"),   # Lead Generation
    ("AMBER", "AMBER", "No CRM / lead tracking"),  # Intake
    ("AMBER", "AMBER", "No formal leadership"),    # Team
...
FINDINGS = [
    ("neg", "A waning referral flywheel is the firm's only real client source, with no scalable system replacing it."),
    ("neg", "Near-zero organic visibility outside personal injury is losing estate planning and probate cases to competitors."),
    ("neg", "With no CRM or lead tracking, referrals and inquiries can be lost with no way to see it happening."),
    ("pos", "An 8-person team and a 30-year office manager already provide real, informal delegation to build on."),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
# List 3 competitors + the client row is added automatically.
COMPETITORS = [
    ("Pribanic & Pribanic, LLC", "Est. PI specialist", "Same street, White Oak"),
    ("TopDog Law",               "Regional presence",  "Pittsburgh-wide reach"),
    ("Gaydos Law referral base", "Waning over time",   "35+ yr family legacy"),
]
CLIENT_REVIEWS      = "Review data unconfirmed"
CLIENT_REVIEWS_NOTE = "← You are here"

# Stage strip (right panel, Slide 1)
STAGE_TEXT = "Solo Practitioner  →  Goal: Self-Managing, Multi-Generational Firm"

# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Build a Firm That Outlasts You"

SMB_MODEL_DESC = (
    "All four pillars — Lead Generation, Intake, Team, and Profit — must work "
    "together. Missing any one means growth stalls no matter how strong the "
    "others are. When all four work together, Joseph can finally build a firm "
    "that runs on its own systems, not on him."
)

GOAL_HEADLINE = "Referral-Only → Multi-Channel Growth"
GOAL_DBM      = "Build something built to outlast you"
...
PRIORITIES = [
    ...
    (
        ...
        [
            "Track every referral so none go unseen",
            "Stand up a CRM to capture every inbound lead",
            "Add after-hours coverage so no call goes unanswered",
            "Protect the referral flow you already depend on",
            "Match every call to what the Gaydos name promises",
        ],
    ),
    (
...
CLOSING_QUOTE = (
    '"Your father built a name that has carried this firm for 35 years — '
    'now build the system that lets it carry the next generation, too."'
)
