...
# Report metadata
FIRM_NAME      = "Supernus Business & Law Center"
SALES_REP      = "Dan Bryant"
OUTPUT_PATH    = "supernus-business-law-center/Supernus_Business_and_Law_Center_May_28_2026_Proposal.pptx"
...
PILLARS = [
    ...
    ("RED",   "CRITICAL", "Solo handles all intake"),
    ("RED",   "CRITICAL", "No staff, zero team"),
    ("AMBER", "MODERATE", "Revenue unconfirmed"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings
FINDINGS = [
    ("neg", "No paid ads — regional firms capture DeKalb/Kane County searches daily"),
    ("neg", "Contact form buried — prospects who can't find it call a competitor"),
    ("neg", "Solo operation — firm cannot scale without owner doing every task"),
    ("pos", "239 five-star Birdeye reviews — strongest reputation in the local market"),
]

# Competitor table
COMPETITORS = [
    ("Foster & Buick Law Group",        "~30 Google reviews", "8 attorneys · 50+ yr history"),
    ("Klein Stoddard Buck & Lewis",     "3 Avvo reviews",     "5 attorneys · multigenerational"),
    ("Regional IL estate planning firms","558+ statewide",    "run paid campaigns into DeKalb/Kane"),
]
CLIENT_REVIEWS      = "239 Birdeye · 5.0★"
CLIENT_REVIEWS_NOTE = "← Strongest local reputation"

# Stage strip
STAGE_TEXT = "Stage 3: Solo Practitioner  →  Goal: Stage 6, Law Firm Owner"

# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Become DeKalb County's Go-To Attorney"

SMB_MODEL_DESC = (
    "Lead generation brings clients in. Intake converts them. "
    "A self-managing team scales operations. A profit plan keeps more of what grows. "
    "When all four work together, Jedediah can protect more families — "
    "and own the time he helps his clients build."
)

GOAL_HEADLINE = "Stage 3 → Stage 6 firm"
GOAL_DBM      = "Protect clients. Own your time."
...
PRIORITIES = [
    (
        ...
        [
            "Turn Google into a client pipeline — first mover wins",
            "Launch LSA: pay per lead, not per click",
            "Claim top search position before regional firms move in",
            "Build Kane County pages for untapped service area",
            "Fix NAP inconsistency — fastest free ranking boost",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Book strategy sessions above the fold — zero extra clicks",
            "Auto-respond to every inquiry within minutes, 24/7",
            "Stop after-hours leads from reaching competitors first",
            "Define intake script before the first team member joins",
            "Track every lead — know where every client comes from",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Document intake process — ready for first team hire",
            "Establish revenue baseline — know your numbers now",
            "Track profit by practice area — find highest-margin work",
            "Build a team that runs when you step away",
            "Plan the firm Jedediah advises his own clients to build",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$3,397", "N/A stand-alone",
        "Google Ads · Google Screened LSA · Meta Ads · local SEO · GBP",
        "1D4ED8",
    ),
    (
        "ELITE COACH",
        "$2,600", "$3,497/mo",
        "Group coaching · practice masterminds · workshops · annual event",
        "6D28D9",
    ),
]
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $6,500–$7,000/mo paid directly to Google / LSA / Meta"
)

AVG_CASE_VALUE      = "$5,000"
CONSERVATIVE_LABEL  = "Conservative  (7 cases/mo):"
CONSERVATIVE_RESULT = "$35,000 revenue · 5.4× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (10 cases/mo):"
AGGRESSIVE_RESULT   = "$50,000 revenue · 7.1× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Google Ads + LSA campaigns go live"),
    ("Day 14",  "Homepage booking CTA moved above the fold"),
    ("Week 2",  "NAP inconsistency corrected across directories"),
    ("Week 3",  "Coaching kickoff — revenue baseline established"),
    ("Month 3", "Intake protocol documented for first team hire"),
]

CLOSING_QUOTE = (
    '"The window to dominate estate planning and asset protection search in DeKalb County '
    'is open right now — no competitor is running paid ads, no one has claimed the LSA slot, '
    'and the first firm to activate will be very difficult to displace."'
)
