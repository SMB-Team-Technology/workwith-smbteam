...
# Report metadata
FIRM_NAME      = "Quinn Law Group"
SALES_REP      = "Nick Holderman"
OUTPUT_PATH    = "quinn/Quinn_Law_Group_June_24_2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL", "No Google/LSA/Meta"),    # Lead Generation
    ("RED",   "CRITICAL", "After-hours drop-off"),   # Intake
    ("AMBER", "AMBER",    "No ops layer built"),      # Team
    ("AMBER", "AMBER",    "No financial reports"),    # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "After-hours manual intake drops 2–3 cases/mo — $30K–$45K/mo in preventable revenue loss"),
    ("neg", "No Google Ads, LSA, or Meta — competitors capture every high-intent PI search Quinn misses"),
    ("neg", "At 168 reviews Quinn trails top competitors — local pack rank suffers across all 7 markets"),
    ("pos", "4.9★ across 168 reviews — strongest quality rating in the entire competitive set"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("MyPhillyLawyer",    "304 reviews", "dominant · local pack"),
    ("The Levin Firm",    "~227 reviews", "5.0★ · active growth"),
    ("Zavodnick & Lasky", "181 reviews",  "PA/NJ presence"),
]
CLIENT_REVIEWS      = "168 reviews"
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach 50–75 New Cases"

SMB_MODEL_DESC = (
    "Lead gen, intake, team, and profit must all work together — "
    "one weak pillar limits every other. When all four systems run "
    "in sync, Sean can step back from daily ops and focus on growth "
    "strategy and trying cases."
)

GOAL_HEADLINE = "50–75 New Cases → $1M+ Revenue"
GOAL_DBM      = "Capture Philadelphia PI — before competitors do"
...
PRIORITIES = [
    (
        ...
        [
            "Launch Google Ads for Philadelphia PI — now",
            "Get into LSA and pay per lead, not per click",
            "Turn Google into a pipeline that runs itself",
            "Multi-location SEO coverage across 7 offices",
            "Stop competitors from owning every PI search",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Automate after-hours intake — stop the bleed",
            "Replace manual follow-up with AI sequences",
            "Sign cases at 11pm without picking up phone",
            "Close review gap with post-case requests",
            "Convert more of the leads already generated",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Elite Coach Plus accountability — 7 offices",
            "Monthly KPI scorecard — know what's working",
            "Build an ops layer Sean can delegate to",
            "Track cost-per-case and ROI by channel",
            "Know what every dollar and every case earns",
        ],
    ),
]
...
PACKAGES = [
    ...
    (
        "ELITE COACH PLUS",
        "$3,200", "$3,497/mo",
        "Weekly coaching · PI mastermind · monthly accountability",
        "6D28D9",
    ),
]
...
CONSERVATIVE_RESULT = "$30K–$45K revenue · 3.5×–4× ROAS"
AGGRESSIVE_LABEL   = "Aggressive  (8–9 cases/mo):"
AGGRESSIVE_RESULT  = "$120K–$135K revenue · ~5× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Kickoff: account access, goals confirmed"),
    ("Week 2",  "Google Ads + LSA campaigns go live in Philly"),
    ("Week 4",  "After-hours intake automation deployed"),
    ("Month 2", "Elite Coach Plus program and coaching begun"),
    ("Month 3", "Full paid pipeline + automated intake running"),
]

CLOSING_QUOTE = (
    '"Quinn Law Group has the reputation to win this market — what it needs now is the marketing engine '
    'and intake automation to convert that reputation into 50–75 new cases before a competitor builds it first."'
)
