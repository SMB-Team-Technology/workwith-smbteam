...
# Report metadata
FIRM_NAME      = "Cole Reynolds Law"
SALES_REP      = "Nick Holderman"
OUTPUT_PATH    = "cole-reynolds-law/ColeReynoldsLaw_July24_2026_Proposal.pptx"
...
PILLARS = [
    ("AMBER", "AMBER", "One SEO channel only"),
    ("RED",   "RED",   "No support staff"),
    ("RED",   "RED",   "0 non-attorney staff"),
    ("GREEN", "GREEN", "10-20% overhead"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "100% of leads come from one organic SEO channel"),
    ("neg", "Zero support staff — every inquiry lands on a partner"),
    ("neg", "Review count (~5-7) trails competitors by 5x or more"),
    ("pos", "Runs at 10-20% overhead vs. industry's 40-60%"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Kinetic Law",   "36 reviews", "4.7 rating"),
    ("Manley Burke",  "43 reviews", "5 Best Lawyers attys"),
    ("Strauss Troy",  "7 reviews",  "47 lawyers, est. 1953"),
]
CLIENT_REVIEWS      = "~5-7 reviews"
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $2M in Annual Revenue"

SMB_MODEL_DESC = (
    "All four pillars — lead generation, intake, team, and profit — must work "
    "together. When each one works, Cole Reynolds Law can grow deal flow "
    "without adding another hour to the partners' week."
)

GOAL_HEADLINE = "$1M → $2M revenue"
GOAL_DBM      = "Step away — and the firm keeps growing"
...
PRIORITIES = [
    (
        ...
        [
            "Launch Google Ads for business law & M&A terms",
            "Bid on Kinetic Law & Manley Burke by name",
            "Claim Local Service Ads before competitors do",
            "Revive the blog — stale for 20+ months",
    ...
    (
        ...
        [
            "Build a defined intake process, start to finish",
            "Add after-hours coverage for inbound inquiries",
            "Move the contact form to the homepage",
            "Track response time on every inquiry",
    ...
    (
        ...
        [
            "Build delegation so partners aren't doing it all",
            "Prepare the firm to absorb the planned acquisition",
            "Track case value & cost by practice area",
            "Set a real revenue goal beyond guesswork",
            "Free up partner time for deal work — and life",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$7,397", "$8,997/mo",
        "Google Ads · Local SEO · Conquesting · Website fixes",
        "1D4ED8",
    ),
    (
        "ELITE COACH PLUS",
...
TIMELINE = [
    ("Day 1",   "Launch Google Ads on business law & M&A terms"),
    ("Day 14",  "Launch competitor-conquesting ad campaigns"),
    ("Week 2",  "Move contact form above the fold"),
    ("Week 3",  "Kick off Elite Coach Plus coaching sessions"),
    ("Month 3", "Confirm PageSpeed & reviews, optimize ongoing"),
]

CLOSING_QUOTE = (
    '"I want to add the next firm to ours — without adding '
    'another hour to my own week."'
)
