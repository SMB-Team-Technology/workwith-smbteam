...
# Report metadata
FIRM_NAME      = "The Bledsoe Firm"
SALES_REP      = "Jacob Meissner"
OUTPUT_PATH    = "the-bledsoe/The Bledsoe Firm_August 12, 2026_Proposal.pptx"
...
PILLARS = [
    ("AMBER", "AMBER", "4 vendors, no view"),     # Lead Generation
    ("AMBER", "AMBER", "17% close rate"),         # Intake
    ("AMBER", "AMBER", "New delegation to Kate"), # Team
    ("AMBER", "AMBER", "No ROI visibility"),      # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "17% close rate wastes most of the $36K/mo already spent"),
    ("neg", "4 disconnected vendors, no single view of what's working"),
    ("neg", "NAP mismatch and split Yelp listings dilute local SEO signal"),
    ("pos", "201 reviews at 4.8 stars — a strong trust foundation to build on"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Minyard Morris LLP", "246 reviews",      "4.9★ · out-paces on both"),
    ("Jos Family Law",     "175+ reviews",     "4.7★ · direct OC rival"),
    ("Cianci Law PC",      "No local office",  "Geo-conquesting Lake Forest"),
]
CLIENT_REVIEWS      = "201 reviews"
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Double Your Client Volume"

SMB_MODEL_DESC = (
    "All four pillars — marketing, intake, team, and profit — must work "
    "together. When they do, John can open new offices without carrying "
    "the marketing and intake load himself."
)

GOAL_HEADLINE = "17 → 20+ clients/mo"
GOAL_DBM      = "John steps back while the firm keeps growing"
...
PRIORITIES = [
    (
        ...
        [
            "One partner replaces four disconnected vendors",
            "Fix the NAP mismatch, merge split Yelp listings",
            "Activate Local Service Ads on existing reviews",
            "Close the review gap with Minyard Morris",
            "Turn ads into a pipeline that runs without you",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Deploy an AI Intake Qualifier to grade every call",
            "Double the 17% close rate on leads already paid for",
            "Move the contact form above the fold",
            "Add a defined follow-up sequence for cold leads",
            "Protect every lead Kate is already paying for",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Build one dashboard across all marketing channels",
            "Formalize Kate's authority over marketing and intake",
            "Track case value and cost per client by channel",
            "Get real data before opening the San Diego office",
            "Free John for strategy, not vendor management",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$10,497", "$12,497/mo",
        "Google Ads · LSA · Meta Ads · Local SEO · Reporting",
        "1D4ED8",
    ),
    (
        "LEGAL AI WORKFORCE — FRACTIONAL CTO L2",
        "$4,997", "$5,797/mo",
        "Custom Intake Qualifier · Claude Enterprise · CTO calls",
        "6D28D9",
    ),
]

BUNDLE_TOTAL   = "$15,494 / mo"
BUNDLE_SAVINGS = "Save $2,800/mo by bundling"

AD_SPEND_NOTE = (
    "+ Recommended ad spend: $5,500–$70,000/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE     = "$4,000 (practice default)"
CONSERVATIVE_LABEL = "Conservative  (9 cases/mo):"
CONSERVATIVE_RESULT = "$36,000 revenue · 6.5× ROAS"
AGGRESSIVE_LABEL   = "Aggressive  (140 cases/mo):"
AGGRESSIVE_RESULT  = "$560,000 revenue · 8.0× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Launch consolidated Google Ads + LSA campaigns"),
    ("Day 14",  "Fix NAP mismatch, merge split Yelp listings"),
    ("Week 2",  "Move the contact form above the fold"),
    ("Week 3",  "Fractional CTO kickoff: build Intake Qualifier"),
    ("Month 3", "One dashboard live, close rate climbing toward 20+/mo"),
]

CLOSING_QUOTE = (
    '"Kate runs marketing and intake — and I finally get to just '
    'practice law and open the next office."'
)
