"""
Audit PowerPoint — Kreiner Burns LLP
...
# ═══════════════════════════════════════════════════════════════════
# FILL — All placeholders completed below
# ═══════════════════════════════════════════════════════════════════

FIRM_NAME   = "Kreiner Burns LLP"
SALES_REP   = "Randy Gold"
OUTPUT_PATH = "kreiner-burns/KreinerBurns_June11_2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL",    "100% referrals, 0 ads"),
    ("RED",   "CRITICAL",    "Form leads not received"),
    ("AMBER", "IN PROGRESS", "Partners handle it all"),
    ("AMBER", "IN PROGRESS", "No financial roadmap"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

FINDINGS = [
    ("neg", "Invisible online: no GBP, no ads, no local presence in any market"),
    ("neg", "Form leads route to 3rd party — cases lost with no awareness"),
    ("neg", "60-70% revenue in 1-2 clients — a single departure is catastrophic"),
    ("pos", "$900K revenue and 75-80% close rate prove the firm can convert"),
]

COMPETITORS = [
    ("The Epstein Law Firm",  "169 reviews", "Comml. litigation NJ · 4.8★"),
    ("Smith Eibeler LLC",     "117 reviews", "Employment law NJ · 4.8★"),
    ("Frank P. Marciano",     "61 reviews",  "Family law Hoboken · 4.1★"),
]
CLIENT_REVIEWS      = "~0 confirmed"
...
SMB_MODEL_DESC = (
    ...
    "When all four pillars are aligned, Tom, Seth, and Peter can step back "
    "and trust that the firm generates $2M without them in every conversation."
)

GOAL_HEADLINE = "$900K → $2M revenue"
GOAL_DBM      = "Build a firm that scales beyond the founders"
...
PRIORITIES = [
    (
        ...
        [
            ...
            "Launch LSA in NJ and PA across 3 practice areas",
            "Fix NAP across directories — end the SEO penalty",
            "Build GBP into a competitive asset in all 4 markets",
            "Meta ads for family law in Hoboken and Hudson County",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Fix form routing — stop losing cases you don't know about",
            "Move contact form above the fold + add booking widget",
            "Define who handles leads and by what deadline",
            "Build a 48-hour follow-up for all marketing leads",
            "Convert the 8-10 monthly targets into signed clients",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Map $900K-to-$2M roadmap by practice area + market",
            "Define partner accountability for each practice area",
            "Build ops structure to support new associate hires",
            "Track revenue by case type to find your real margin",
            "Eliminate the 60-70% client concentration risk",
        ],
    ),
]
...
CONSERVATIVE_RESULT = "~$17,500 revenue · ~5× ROAS"
...
TIMELINE = [
    ...
    ("Month 3", "Full pipeline reporting by practice area"),
]

CLOSING_QUOTE = (
    '"The goal is $2 million — not because we worked harder, '
    'but because we built a system that generates clients whether we show up or not."'
)
