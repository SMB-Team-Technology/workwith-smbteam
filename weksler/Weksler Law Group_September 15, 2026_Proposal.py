...
# Report metadata
FIRM_NAME      = "Weksler Law Group"
SALES_REP      = "Dan Bryant"
OUTPUT_PATH    = "weksler/Weksler Law Group_September 15, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL", "Zero paid lead gen"),
    ("AMBER", "AMBER",    "Only Ron can close"),
    ("RED",   "CRITICAL", "No delegate exists"),
...
FINDINGS = [
    ("neg", "No paid leads — competitors own the entire paid search and LSA market"),
    ("neg", "Every close depends on Ron — consultations stall when he travels"),
    ("neg", "No delegate exists — the firm cannot run without Ron, even briefly"),
    ("pos", "Reputation already outpaces every named local competitor"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
# List 3 competitors + the client row is added automatically.
COMPETITORS = [
    ("Bornstein & Smith",  "50+ yrs",      "Super Lawyers rec."),
    ("Finn Law Group",     "Multi-market", "Builds city pages"),
    ("Fischer / Gordon",   "Few reviews",  "Thin content"),
]
CLIENT_REVIEWS      = "79 reviews"
CLIENT_REVIEWS_NOTE = "5.0★ Avvo, confirmed"
...
SMB_MODEL_DESC = (
    ...
    "must work together. When they do, Ron can step away and travel "
    "without the firm losing a beat."
)

GOAL_HEADLINE = "$500K → $1M+ revenue"
GOAL_DBM      = "Travel freely without the firm losing ground"
...
PRIORITIES = [
    (
        ...
        [
            "Launch Local Service Ads for estate planning",
            "Fix Google Business Profile category mismatch",
            "Rebuild the website with one clear call to action",
            "Add city pages for Hallandale & Ft. Lauderdale",
            "Turn your reputation into leads that find you first",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "69CD2B",
        [
            "Add self-scheduling so leads book without calling",
            "Move the contact form above the fold",
            "Create a backup plan for after-hours inquiries",
            "Track follow-up so no lead goes cold",
            "Close clients without being the only one who can",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "003A59",
        [
            "Track revenue by practice area, not just gross",
            "Add an FCOO Advisor to build delegation",
            "Weekly coaching to build your leadership team",
            "Know your real profit, not just revenue",
            "Step away and trust the firm still runs",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$4,997", "$5,697/mo",
        "Website rebuild · LSA · Google Ads · Local SEO",
        "0091C9",
    ),
    (
        "ELITE COACH PLUS",
        "$3,200", "$3,497/mo",
        "Weekly group coaching · Monthly 1:1 · Masterminds",
        "003A59",
    ),
]
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $5,500–$16,670/mo paid directly to Google/LSA"
)

AVG_CASE_VALUE     = "$1,500–$3,000"
CONSERVATIVE_LABEL = "Conservative  (6 cases/mo):"
CONSERVATIVE_RESULT = "$13,500 revenue · 2.5× ROAS"
AGGRESSIVE_LABEL   = "Aggressive  (23 cases/mo):"
AGGRESSIVE_RESULT  = "$51,750 revenue · 3.1× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Launch LSA for estate planning at $2,000/mo"),
    ("Day 14",  "Fix GBP category mismatch & begin local SEO"),
    ("Week 2",  "Begin weekly group coaching + 1:1 kickoff call"),
    ("Week 3",  "Website rebuild underway; fix homepage CTAs"),
    ("Month 3", "Google Ads live across both practice areas"),
]

CLOSING_QUOTE = (
    '"I\'d rather work less and get paid more — and actually be free '
    'to travel, not just answer calls from a different time zone."'
)
