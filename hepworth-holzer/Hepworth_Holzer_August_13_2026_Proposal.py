...
# Report metadata
FIRM_NAME      = "Hepworth Holzer, LLP"
SALES_REP      = "Michael Kopp"
OUTPUT_PATH    = "hepworth-holzer/Hepworth_Holzer_August_13_2026_Proposal.pptx"
...
PILLARS = [
    ("AMBER", "AMBER", "Narrow PPC only"),      # Lead Generation
    ("AMBER", "AMBER", "No lead attribution"),  # Intake
    ("AMBER", "AMBER", "No ops role"),          # Team
...
FINDINGS = [
    ("neg", "No lead-source attribution — every marketing dollar spent is a dollar the firm can't learn from"),
    ("neg", "First ChatGPT ad test launched with no targeting — all 5 inquiries were poor-fit"),
    ("neg", "Three different phone numbers across the web presence are suppressing local map rank"),
    ("pos", "Already has ~87 Google reviews — more than Litster Frost (~77) and Monteleone (~18)"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Feller & Wendt, LLC",            "143 reviews", "Dedicated truck-accident page"),
    ("Litster Frost Injury Lawyers",   "77 reviews",  "Deepest trucking content"),
    ("Monteleone Law Offices",         "18 reviews",  "Cites $8.5M truck verdict"),
]
CLIENT_REVIEWS      = "87 reviews"
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Win the AI-Search Channel First"

SMB_MODEL_DESC = (
    "All four pillars must work together. "
    "When they do, Kurt can prove which channel wins premium cases — and hand the next generation a firm that's ahead."
)

GOAL_HEADLINE = "More Trucking & Auto Cases"
GOAL_DBM      = "Protect the firm you're building for the next generation"
...
PRIORITIES = [
    (
        ...
        [
            "Launch targeted Google Ads for trucking & auto",
            "Relaunch ChatGPT ads with real radius targeting",
            "Fix NAP across site, Yelp, and Martindale",
            "Build AI-search content on landing pages",
            "Turn PPC into a pipeline you can prove works",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "69CD2B",
        [
            "Tag every lead to its real source",
            "Add a filter for premium case types",
            "Filter out cases you're moving away from",
            "Turn poor-fit leads into a repeatable process",
            "Know exactly which channel earns your next case",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "003A59",
        [
            "Start Elite Coach Plus group coaching",
            "Give marketing a home beyond Kurt",
            "Track cost per case by channel",
            "Prove the trucking/auto shift pays off",
            "Build a firm the next generation inherits ahead",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$7,497", "$8,997/mo",
        "Targeted ads · Attribution · AI-search content · NAP cleanup",
        "0091C9",
    ),
    (
        "ELITE COACH PLUS",
...
AVG_CASE_VALUE     = "$7,500 (default)"
CONSERVATIVE_LABEL = "Conservative  (~2 cases/mo):"
CONSERVATIVE_RESULT = "$16,900 revenue · 1.7× return"
AGGRESSIVE_LABEL   = "Aggressive  (~13-14 cases/mo):"
AGGRESSIVE_RESULT  = "$101,000 revenue · 2.5× return"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Kick off Elite Coach Plus coaching"),
    ("Day 14",  "Launch targeted trucking/auto ad campaign"),
    ("Week 2",  "Stand up lead-source attribution"),
    ("Week 3",  "Fix NAP inconsistency across directories"),
    ("Month 3", "AI-search content live on landing pages"),
]

CLOSING_QUOTE = (
    '"The firm I hand to the next generation of partners is ahead of the market — not chasing it."'
)
