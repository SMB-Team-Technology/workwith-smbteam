...
# ═══════════════════════════════════════════════════════════════════
# FILL — All values pulled from completed audit sections
# ═══════════════════════════════════════════════════════════════════

FIRM_NAME   = "Soltesz Law Group"
SALES_REP   = "Jonathan Farace"
OUTPUT_PATH = "soltesz/SolteszLawGroup_May26_2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL",  "No ads · no 3-pack"),     # Lead Generation
    ("RED",   "CRITICAL",  "No intake team"),          # Intake
    ("RED",   "CRITICAL",  "Solo — all on Roland"),    # Team
    ("AMBER", "MONITOR",   "No financial tracking"),   # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# From section_executive_summary.html (top priorities) + section_05 positive
FINDINGS = [
    ("neg", "Invisible online — not in 3-pack for any criminal defense search"),
    ("neg", "FindLaw routes calls to Pernik; Martindale shows wrong firm"),
    ("neg", "No after-hours coverage — crisis leads go to competitors first"),
    ("pos", "9.7 Avvo rating + 100+ jury trials = instant trust when found"),
]

# From section_06_lead_generation.html + research notes
COMPETITORS = [
    ("Taylor & Dolowich",            "54 reviews", "5.0 stars · top-reviewed in Salinas"),
    ("Law Offices of William Pernik", "50 reviews", "4.9 stars · Roland's former firm"),
    ("The Worthington Law Centre",   "34 reviews",  "4.7 stars · board-certified"),
]
CLIENT_REVIEWS      = "~2 reviews"
CLIENT_REVIEWS_NOTE = "← You are here"

# From section_03_firm_overview.html (stage assessment)
STAGE_TEXT = "Stage 3: Solo Practitioner  →  Goal: Stage 6, Law Firm Owner"

# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Build Your Independent Firm"

SMB_MODEL_DESC = (
    "Lead generation, intake, a self-managing team, and a profit plan — "
    "all four must work together. "
    "When they do, Roland's credentials become visible to the clients "
    "who need them most, and the firm generates cases whether he is in "
    "the office or not."
)

GOAL_HEADLINE = "Stage 3 → Law Firm Owner"
GOAL_DBM      = "Cases that come to you — not through someone else"

PRIORITIES = [
    (
        "Build the", "Marketing Engine", "1D4ED8",
        [
            "Turn Google into a case pipeline that runs daily",
            "Get Google Screened — own the top of the SERP",
            "Spanish ads — own Salinas's 55% Hispanic market",
            "Fix FindLaw + Martindale routing calls to Pernik",
            "Build 20+ Google reviews in 90 days",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Answer every after-hours crisis call — not voicemail",
            "Build an intake script that converts first contact",
            "Follow-up sequence for leads who don't sign fast",
            "Web form so prospects can reach you after hours",
            "Track every lead from first call to signed case",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Know which case types earn the most per hour",
            "Build baseline: revenue, cost, and take-home",
            "Document intake so you can hire and delegate it",
            "Hire intake coordinator as case volume grows",
            "Track ad ROI — know what every dollar returns",
        ],
    ),
]

# ── Slide 3 ──────────────────────────────────────────────────────
# Prices from section_11_next_steps.html investment grid
PACKAGES = [
    (
        "FULL SERVICE MARKETING — STARTER",
        "$4,847", "$5,697/mo",
        "Website · Google Ads (EN+ES) · LSA · Local SEO · Meta Ads",
        "1D4ED8",
    ),
    (
        "ELITE COACH PLUS",
        "$3,200", "$3,497/mo",
        "Weekly coaching · Masterminds · Quarterly + annual workshops",
        "6D28D9",
    ),
]
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $5,000–$10,000/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE      = "$4,000 (est.)"
CONSERVATIVE_LABEL  = "Conservative  (~7 cases/mo):"
CONSERVATIVE_RESULT = "~$28,000 revenue · 5.6× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (~16 cases/mo):"
AGGRESSIVE_RESULT   = "~$64,000 revenue · 6.4× ROAS"

# From section_11 first-90-days block (condensed to ≤55 chars each)
TIMELINE = [
    ("Day 1",    "Google Ads launch — English + Spanish"),
    ("Day 14",   "Apply for Google Screened LSA verification"),
    ("Week 2",   "Google review outreach to former clients begins"),
    ("Week 3",   "Website rebuild — practice area pages go live"),
    ("Month 3",  "Coaching kickoff — baseline metrics + intake plan"),
]

# DBM closing quote — translated from exec-closing to client voice
CLOSING_QUOTE = (
    '"The day your phone rings with a DUI case from someone who found you on Google — '
    'not from a former colleague — that is the day Soltesz Law Group becomes a firm."'
)
