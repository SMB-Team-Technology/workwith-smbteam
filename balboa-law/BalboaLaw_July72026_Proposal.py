"""
Audit PowerPoint — Balboa Law
SMB Team Growth Audit Proposal
...
# ═══════════════════════════════════════════════════════════════════
# FILL — data extracted from completed audit sections
# ═══════════════════════════════════════════════════════════════════

FIRM_NAME   = "Balboa Law"
SALES_REP   = "Jonathan Farace"
OUTPUT_PATH = "balboa-law/BalboaLaw_July72026_Proposal.pptx"

WEBSITE_SCREENSHOT_PATH = None

# ── Slide 1 ──────────────────────────────────────────────────────
# Urgency score: 8 (from section_05)
URGENCY_SCORE = "8"

# Pillars: (status, label, detail) — copied from section_05 sc-cards
PILLARS = [
    ("RED",   "CRITICAL", "No website or GBP yet"),     # Lead Generation
    ("RED",   "CRITICAL", "No intake system in place"),  # Intake
    ("RED",   "CRITICAL", "Gerald runs everything"),     # Team
    ("AMBER", "MONITOR",  "Goal set; no P&L yet"),       # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — consequence-first (from section_executive_summary)
FINDINGS = [
    ("neg", "No website, GBP, or reviews — invisible at launch in Houston PI"),
    ("neg", "Physical office TBD blocks LSA, 3-Pack, and GBP activation"),
    ("neg", "No intake system — ad spend cannot convert without it"),
    ("pos", "Bilingual brand built AI-first from day one — no legacy to undo"),
]

# Competitors — 3 named, from section_06 and research notes
COMPETITORS = [
    ("Ramji Law Group",   "2,383 reviews", "5.0★ · highest in Houston PI"),
    ("Arnold & Itkin LLP","1,346 reviews", "4.6★ · $25B recovered brand"),
    ("Zehl & Associates", "1,262 reviews", "5.0★ · bilingual PPC + LSA"),
]
CLIENT_REVIEWS      = "0 reviews"
CLIENT_REVIEWS_NOTE = "← Balboa Law at launch"

# Stage strip
STAGE_TEXT = "Stage 3: Solo Practitioner  →  Goal: Stage 6, Law Firm Owner"

# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $300K–$1M in Year One"

SMB_MODEL_DESC = (
    "Lead generation, intake, team, and profit must all work together. "
    "Miss any one pillar and growth stalls — regardless of ad spend. "
    "When all four are running, Gerald can step back and let the firm "
    "generate income without his daily involvement."
)

GOAL_HEADLINE = "$0 → $300K–$1M net profit"
GOAL_DBM      = "Real income — without being consumed by it"

PRIORITIES = [
    (
        "Build the", "Marketing Engine", "1D4ED8",
        [
            "Launch bilingual Google Ads before Sept 1",
            "Secure physical office — unlocks LSA + 3-Pack",
            "Build bilingual website on balboa.com now",
            "Launch Spanish Meta campaign from day one",
            "Start review generation from the first referral",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Build automated intake before ads go live",
            "Sub-5-min bilingual response to every lead",
            "Define the attorney escalation protocol",
            "Bilingual follow-up sequence for all leads",
            "Track every inquiry — no lead falls through",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Formalize referral attorney agreements now",
            "Cash flow model for 12–36 month settlements",
            "Weekly coaching: KPIs and case-mix strategy",
            "Automate operations — step back by month 3",
            "Track cost-per-referral from day one",
        ],
    ),
]

# ── Slide 3 ──────────────────────────────────────────────────────
# Packages: (label, bundled_price, retail_price, services_line, accent_color_hex)
PACKAGES = [
    (
        "FULL SERVICE MARKETING — STARTER",
        "$4,847", "$5,697/mo",
        "Bilingual site · Google Ads (EN+ES) · LSA · Meta · GBP",
        "1D4ED8",
    ),
    (
        "ELITE COACH",
        "$2,600", "$3,497/mo",
        "Weekly coaching · PI masterminds · Profit plan · Workshops",
        "6D28D9",
    ),
]
...
AVG_CASE_VALUE      = "$20,000 (referral fee)"
CONSERVATIVE_LABEL  = "Conservative  (2–3 cases/mo):"
CONSERVATIVE_RESULT = "$40K–$60K pipeline · 4–6× return"
AGGRESSIVE_LABEL    = "Aggressive  (5–7 cases/mo):"
AGGRESSIVE_RESULT   = "$125K–$175K pipeline · 5–7× return"

# Timeline — 5 milestones from section_11 first 90 days
TIMELINE = [
    ("Day 1",    "Launch bilingual Google Ads campaign"),
    ("Day 14",   "Website build kicks off on balboa.com"),
    ("Week 3",   "GBP + LSA verification begins at office"),
    ("Month 1",  "Automated bilingual intake system live"),
    ("Month 3",  "Review velocity + coaching KPIs on track"),
]

# Closing quote — client-facing DBM vision (from exec-closing + transcript)
CLOSING_QUOTE = (
    '"The firm that generates real income without consuming either of us — '
