...
# Report metadata
FIRM_NAME      = "Hayoon Kane Law Firm"
SALES_REP      = "Nick Holderman"
OUTPUT_PATH    = "hayoon-kane/HayoonKaneLawFirm_June25_2026_Proposal.pptx"
...
PILLARS = [
    ("AMBER", "AMBER", "Meta-only, no Google Ads"),   # Lead Generation
    ("AMBER", "AMBER", "20% book-to-hire rate"),       # Intake
    ("AMBER", "AMBER", "Owner runs every consult"),    # Team
    ("AMBER", "AMBER", "No profit visibility"),         # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

FINDINGS = [
    ("neg", "4 competitors run Google Ads daily — this firm does not appear to"),
    ("neg", "Hayoon runs every consult personally — hard ceiling on growth"),
    ("neg", "Entire paid pipeline is one Meta channel — one change shuts it down"),
    ("pos", "159 Google reviews at 4.9 stars — highest of any named competitor"),
]

COMPETITORS = [
    ("New Frontier Immigration Law", "AZ-based",       "T-Visa Google Ads into LV"),
    ("MC Law Group",                 "79 Google reviews", "4.8 stars · running ads"),
    ("Reza Athari & Associates",     "59 Yelp reviews", "27 yrs · 2 LV offices"),
]
CLIENT_REVIEWS      = "159 Google reviews"
CLIENT_REVIEWS_NOTE = "4.9 stars ← You are here"
...
SMB_MODEL_DESC = (
    "Lead generation, intake, team, and profit — all four pillars "
    "must work together. When they do, Hayoon can finally step back "
    "from every consultation and build the income that makes the "
    "San Francisco home — and the expanded community impact — real."
)

GOAL_HEADLINE = "$700K → $1.5M revenue"
GOAL_DBM      = "More families helped — and the home in San Francisco"

PRIORITIES = [
    (
        "Build the", "Marketing Engine", "1D4ED8",
        [
            "Launch Korean-language Google Ads — uncontested niche",
            "Apply for Google Screened LSA — appear above everything",
            "Target T-Visa searchers: $11K/case, high-intent only",
            "Activate SF Korean market with existing landing page",
            "Restructure Meta to full-funnel: retarget + educate",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Replace low-intent Meta leads with Google searchers",
            "Build follow-up system for the 80% who don't sign yet",
            "Move contact form above the fold on the homepage",
            "Create bilingual intake script for Korean clients",
            "Track consult-to-hire rate weekly — measure the gain",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Document consultation process — delegate, don't bottleneck",
            "Hire bilingual Consultation Specialist using coaching script",
            "Calculate true cost per T-Visa case — know your margin",
            "Set monthly KPIs: cases closed, close rate, revenue",
            "Build the profit plan that funds the SF home purchase",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$4,847", "$5,697/mo",
        "Google Ads · LSA · Meta management · website optimization",
        "1D4ED8",
    ),
    (
        "ELITE COACH PLUS",
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $3,000–$12,000/mo paid directly to Google/LSA/Meta"
)

AVG_CASE_VALUE     = "$11,000"
CONSERVATIVE_LABEL = "Conservative  (~4 cases/mo):"
CONSERVATIVE_RESULT = "~$44,000 revenue · ~15× ROAS"
AGGRESSIVE_LABEL   = "Aggressive  (~18 cases/mo):"
AGGRESSIVE_RESULT  = "~$198,000 revenue · ~17× ROAS"

TIMELINE = [
    ("Day 1",   "Google Ads + LSA application submitted"),
    ("Day 14",  "Website contact form optimized for conversions"),
    ("Week 2",  "Meta restructured to full-funnel retargeting"),
    ("Week 3",  "Coaching kickoff + consultation framework built"),
    ("Month 3", "Consultation Specialist hired and ramped"),
]

CLOSING_QUOTE = (
    '"More cases accepted, more families protected — '
    'and the income to finally make San Francisco real."'
)
