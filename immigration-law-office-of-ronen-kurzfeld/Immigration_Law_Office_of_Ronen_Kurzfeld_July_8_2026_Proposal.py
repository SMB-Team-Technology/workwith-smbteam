"""
Audit PowerPoint — Immigration Law Office of Ronen Kurzfeld
...
# ═══════════════════════════════════════════════════════════════════
# FILL DATA — All values copied from completed audit sections
# ═══════════════════════════════════════════════════════════════════

FIRM_NAME   = "Immigration Law Office of Ronen Kurzfeld"
SALES_REP   = "Randy Gold"
OUTPUT_PATH = "immigration-law-office-of-ronen-kurzfeld/Immigration_Law_Office_of_Ronen_Kurzfeld_July_8_2026_Proposal.pptx"

WEBSITE_SCREENSHOT_PATH = None

# ── Slide 1 ──────────────────────────────────────────────────────
# Copied from section_05_growth_health.html urgency-score element
URGENCY_SCORE = "8"

# Pillar status — traffic light class → status, label, one-line detail (≤20 chars)
# Copied from section_05 sc-card traffic circles and first finding bullets
PILLARS = [
    ("RED",   "CRITICAL", "3 channels collapsed"),   # Lead Generation
    ("RED",   "CRITICAL", "Owner runs intake"),       # Intake
    ("AMBER", "AMBER",    "No leadership layer"),     # Team
    ("AMBER", "AMBER",    "Collections leaking"),     # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — consequence-first; copied from section_executive_summary.html top priorities
FINDINGS = [
    ("neg", "Organic page 6+, Maps inconsistent, PPC ineffective — 50% lead drop"),
    ("neg", "Ronen personally handles intake & billing — no delegation in place"),
    ("neg", "Monday.com broken: leads fall out, lawyers missing filing deadlines"),
    ("pos", "371 Google reviews at 4.9★ — stronger than most Toronto competitors"),
]

# Competitors — copied from section_06_lead_generation.html competitor data
COMPETITORS = [
    ("Mamann Sandaluk LLP",       "783 reviews", "4.8★ · Dominates organic & Maps"),
    ("Matkowsky Immigration Law",  "600+ reviews","4.8★ · Google/Microsoft retargeting"),
    ("VisaPlace",                  "1,100+ reviews","National · digital-first leader"),
]
CLIENT_REVIEWS      = "371 reviews · 4.9★"
CLIENT_REVIEWS_NOTE = "← You are here"

# Copied from section_03_firm_overview.html stage label
STAGE_TEXT = "Stage 4: Small Business Manager  →  Goal: Stage 6, Law Firm Owner"

# ── Slide 2 ──────────────────────────────────────────────────────
# Derived from research notes revenue goal ($4–5M CAD)
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $4–5M CAD"

SMB_MODEL_DESC = (
    "Lead Generation, Intake, Team, and Profit must work together. "
    "One broken pillar limits what all the others can produce. "
    "When all four work, Ronen can step back from the day-to-day "
    "and trust the firm grows without him in the middle of every function."
)

# Copied from research notes: revenue stated $2–3M CAD, goal $4–5M CAD
GOAL_HEADLINE = "$2–3M CAD  →  $4–5M CAD"
# DBM translated to client-facing aspirational language
GOAL_DBM      = "Step back — and trust the firm runs"

# Priority columns — derived from section_11 quick wins and sections 06–09
PRIORITIES = [
    (
        "Build the", "Marketing Engine", "1D4ED8",
        [
            "Restore rankings: fix NAP & consolidate domains",
            "Launch LSA — 371 reviews qualify for top placement",
            "Rebuild PPC: Express Entry & spousal terms",
            "Meta retargeting: recover visitors who didn't convert",
            "Turn Google into a lead pipeline that runs itself",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Automate retainer follow-up for non-signers",
            "Designate an intake lead with a documented process",
            "Define call routing & after-hours coverage",
            "Fix Monday.com workflow so no lead falls out",
    ...
    (
        ...
        [
            "Build AI use policy: eliminate citation risk",
            "Automate invoice reminders — stop the leak",
            "Create team accountability: end daily escalations",
            "Track profit monthly: know what you actually keep",
            "Delegate billing — free Ronen's time for growth",
        ],
    ),
]

# ── Slide 3 ──────────────────────────────────────────────────────
# Copied exactly from section_11_next_steps.html investment grid
PACKAGES = [
    (
        "FULL SERVICE MARKETING — GROWTH",
        "$7,397", "$8,997/mo",
        "Website · Google Ads · LSA · Meta · Local SEO recovery",
        "1D4ED8",
    ),
    (
        "MASTER'S CIRCLE",
        "$4,600", "$4,997/mo",
        "Weekly coaching · Masterminds · Annual workshop",
        "6D28D9",
    ),
]

# Copied from section_11 investment-total and savings callout
BUNDLE_TOTAL   = "$11,997 / mo"
BUNDLE_SAVINGS = "Save $1,997/mo by bundling"

# Copied from section_11 Block 4 recommended ad spend row
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $3,000–$12,000/mo paid directly to Google/Meta"
)

# Copied from section_11 Block 4 ROI projections
AVG_CASE_VALUE      = "$5,000"
CONSERVATIVE_LABEL  = "Conservative  (~3 cases/mo):"
CONSERVATIVE_RESULT = "$15,000 revenue · 5× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (~16 cases/mo):"
AGGRESSIVE_RESULT   = "$80,000 revenue · 7× ROAS"

# First 90 days — condensed from section_11 Block 5, each ≤55 chars
TIMELINE = [
    ("Day 1",    "PPC & LSA campaigns live — paid leads flowing"),
    ("Day 14",   "Website rebuild & NAP cleanup launched"),
    ("Week 2",   "Retainer follow-up sequence activated"),
    ("Week 3",   "Master's Circle kickoff: delegation roadmap set"),
    ("Month 3",  "Organic recovery tracked; AI use policy deployed"),
]

# Closing quote — vivid, specific, in the owner's voice (from DBM and transcript)
CLOSING_QUOTE = (
    '"Intake off my plate. Billing off my plate. The firm grows — '
    'and I trust it runs correctly whether or not I am there."'
)
