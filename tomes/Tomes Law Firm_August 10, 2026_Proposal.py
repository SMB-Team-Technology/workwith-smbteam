...
# Report metadata
FIRM_NAME      = "Tomes Law Firm, PC"
SALES_REP      = "Jonathan Farace"
OUTPUT_PATH    = "tomes/Tomes Law Firm_August 10, 2026_Proposal.pptx"
...
PILLARS = [
    ("AMBER", "AMBER", "No defined channel"),   # Lead Generation
    ("RED",   "CRITICAL", "No intake process"), # Intake
    ("RED",   "CRITICAL", "Constant oversight"),# Team
    ("RED",   "CRITICAL", "No margin data"),    # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "A 7-person team still needs Frances managing every day"),
    ("neg", "No defined intake process — leads get inconsistent follow-up"),
    ("neg", "No profit margin or resolved revenue figure exists yet"),
    ("pos", "99 reviews at 4.9 stars outperforms nearly every named competitor"),
]

# Competitor table
COMPETITORS = [
    ("Lyons & Associates",           "124 reviews", "4.6★ · multi-office"),
    ("Detzky, Hunter & DeFillippo",  "71 reviews",  "4.9★ · same street"),
    ("Robert Manchel, Esq.",         "45 reviews",  "5.0★ · bankruptcy-only"),
]
CLIENT_REVIEWS      = "99 reviews"
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Run the Firm Without You"

SMB_MODEL_DESC = (
    "All four pillars must work together — but right now, the team and intake "
    "pillars need to run without Frances before marketing can scale safely. "
    "When they do, Frances can finally step away and trust the firm runs."
)

GOAL_HEADLINE = "60-hour weeks → a firm that runs itself"
GOAL_DBM      = "Step away and trust the team runs"

# Each priority: (line1, line2, accent_color_hex, [5 bullet strings])
PRIORITIES = [
    (
        "Build a", "Self-Managing Team", "1D4ED8",
        [
            "FCOO Advisor audits your team & process",
            "Replace WhatsApp chaos with one system",
            "Enforce the 180-page manual you built",
            "Stop retraining — cut turnover for good",
            "A team that runs without you checking in",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Define what happens after the phone rings",
            "Close the after-hours coverage gap",
            "Convert more of the leads you already have",
            "No new marketing dollars needed first",
            "Stop chasing leads yourself — trust the process",
        ],
    ),
    (
        "Install Profit", "Visibility", "6D28D9",
        [
            "Resolve the $650K vs $250K discrepancy",
            "Build practice-area case value baselines",
            "Add a Fractional CFO Advisor in Phase 3",
            "See real margin, not just top-line revenue",
            "Know your 60-hour weeks are building wealth",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$5,694", "$7,294/mo",
        "Weekly coaching · FCOO Advisor · Masterminds · Workshops",
        "6D28D9",
    ),
]

BUNDLE_TOTAL   = "$5,694 / mo"
BUNDLE_SAVINGS = "Save $1,600/mo by bundling"

AD_SPEND_NOTE = (
    "No ad spend this phase — lead generation resumes in Phase 2, once operations are stable"
)

AVG_CASE_VALUE     = "Deferred to Phase 2"
CONSERVATIVE_LABEL = "Phase 1 ad spend:"
CONSERVATIVE_RESULT = "$0 — foundation first"
AGGRESSIVE_LABEL   = "Phase 2 ad spend:"
AGGRESSIVE_RESULT  = "Scoped after ops stabilize"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Elite Coach Plus + FCOO Advisor kickoff"),
    ("Day 14",  "FCOO Advisor completes operational audit"),
    ("Week 2",  "New accountability system replaces WhatsApp"),
    ("Week 3",  "Intake follow-up process defined & live"),
    ("Month 3", "Baseline profit picture established"),
]

CLOSING_QUOTE = (
    '"You take a real vacation, and the cases keep coming in — handled by '
    'a team you trust, not one you have to manage from your phone."'
)
