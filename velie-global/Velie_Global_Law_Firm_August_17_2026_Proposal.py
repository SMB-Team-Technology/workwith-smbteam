...
# Report metadata
FIRM_NAME      = "Velie Global Law Firm"
SALES_REP      = "Jacob Meissner"
OUTPUT_PATH    = "velie-global/Velie_Global_Law_Firm_August_17_2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL", "Referrals only"),
    ("RED",   "CRITICAL", "No process"),
    ("AMBER", "AMBER",    "Owner overwhelmed"),
    ("RED",   "CRITICAL", "Margin unknown"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Dallas relies entirely on referrals — no lead gen or intake system"),
    ("neg", "Jon personally handles daily Dallas litigation, blocking firm leadership"),
    ("neg", "No financial tracking exists — the 20–30% profitability goal has no baseline"),
    ("pos", "Norman's proven, tech-driven intake system already shows what's possible"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("GoSuits Trial Attorneys", "High-volume", "Aggressive PI marketing, Dallas"),
    ("Winningham & Associates", "40+ yr firm", "OKC immigration authority"),
    ("DFW Injury Lawyers",      "Established", "Dallas-branded PI competitor"),
]
CLIENT_REVIEWS      = "Unverified"
CLIENT_REVIEWS_NOTE = "← Verify live before pitch"

# Stage strip (right panel, Slide 1)
STAGE_TEXT = "Stage 4: Small Business Manager  →  Goal: Stage 6, Law Firm Owner"

# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach 20–30% Profitability"

SMB_MODEL_DESC = (
    "Lead generation, intake, team, and profit all have to work together — the "
    "exact system already proven in Norman just needs to run in Dallas too. When "
    "it does, Jon can finally step back from casework and lead the firm he built."
)

GOAL_HEADLINE = "Dallas $700K → $1M–$1.5M"
GOAL_DBM      = "Step back from casework and lead the firm"

# Each priority: (line1, line2, accent_color_hex, [5 bullet strings])
PRIORITIES = [
    (
        "Systemize", "Dallas Operations", "0091C9",
        [
            "Replicate Norman's proven intake process in Dallas",
            "Define client selection & case screening criteria",
            "Build a documented org structure around Pam's role",
            "Stand up baseline financial tracking firm-wide",
            "Turn Pam's audit into a system she can run",
        ],
    ),
    (
        "Integrate", "The Tech Stack", "69CD2B",
        [
            "Connect Clio, QuickBooks, and Stripe into one stack",
            "Automate manual billing and intake workflows",
            "Deploy AI Workforce Pro skills for client experience",
            "Eliminate the $8,400/yr cost of disconnected tools",
            "Give Jon and Pam one system instead of three",
        ],
    ),
    (
        "Build Toward", "Profit & Scale", "003A59",
        [
            "Track real margins toward the 20–30% goal",
            "Fix Dallas's referral-only lead generation gap",
            "Consolidate brand identity across web properties",
            "Launch a Dallas marketing engine once ops are live",
            "Lay the groundwork for New York and San Francisco",
        ],
    ),
]
...
PACKAGES = [
    (
        "RECOMMENDED OPERATIONS PACKAGE",
        "$3,297", "$3,797/mo",
        "Fractional COO · Intake rebuild · Org structure · Coaching",
        "0091C9",
    ),
    (
        "RECOMMENDED TECHNOLOGY PACKAGE",
        "$4,997", "$5,797/mo",
        "Fractional CTO · Clio/QuickBooks/Stripe integration · AI Skills",
        "003A59",
    ),
]
...
AD_SPEND_NOTE = (
    "+ Future Phase 2 ad spend: $13,000–$30,000/mo once Dallas systems are live"
)

AVG_CASE_VALUE     = "$5,500"
CONSERVATIVE_LABEL = "Conservative  (6 cases/mo):"
CONSERVATIVE_RESULT = "$33,000 revenue · 2.5× ROAS"
AGGRESSIVE_LABEL   = "Aggressive  (16 cases/mo):"
AGGRESSIVE_RESULT  = "$88,000 revenue · 2.9× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "COO & CTO kickoff: map Dallas ops end to end"),
    ("Day 14",  "Begin replicating Norman's intake in Dallas"),
    ("Week 2",  "Integrate Clio, QuickBooks, and Stripe"),
    ("Week 3",  "Weekly group coaching sessions begin"),
    ("Month 3", "Baseline financial reporting stood up"),
]

CLOSING_QUOTE = (
    '"Dallas runs itself — the way Norman already does — so I can finally lead '
    'this firm instead of living inside every case."'
)
