"""
Audit PowerPoint — The Thomson Law Firm
...
FIRM_NAME   = "The Thomson Law Firm"
SALES_REP   = "Michael Kopp"
OUTPUT_PATH = "the-thomson/TheThomsonLawFirm_May262026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL",      "No ads · 3× review gap"),
    ("AMBER", "ACTION NEEDED", "Form buried; no 24/7"),
    ("AMBER", "ACTION NEEDED", "Owner is the hub"),
    ("AMBER", "ACTION NEEDED", "No ROI visibility"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

FINDINGS = [
    ("neg", "No Google/LSA/Meta ads — competitors own every PI keyword"),
    ("neg", "101 reviews vs. 316+ — not in 3-pack for any core PI term"),
    ("neg", "Form below fold + no after-hours — losing cases every day"),
    ("pos", "5.0★ + $21M verdict — strongest credentials in the market"),
]

COMPETITORS = [
    ("Crandall & Katt",        "316–336 reviews", "Dominates 3-pack · est. 1979"),
    ("Monge & Associates",     "201–217 reviews", "National chain · high velocity"),
    ("The Mark Hurt Law Firm", "174+ reviews",    "11 offices · SW Virginia overlap"),
]
CLIENT_REVIEWS      = "~101 reviews · 5.0 ★"
...
SMB_MODEL_DESC = (
    ...
    "When all four pillars are in place, Paul can step back from the daily grind "
    "and trust that qualified catastrophic cases keep coming in."
)

GOAL_HEADLINE = "$750K → $1.5M revenue"
GOAL_DBM      = "Focus on the cases that matter — trust the firm handles the rest"

PRIORITIES = [
    (
        "Build the", "Marketing Engine", "1D4ED8",
        [
            "Turn Google into a daily PI case pipeline",
            "Launch LSA — appear above all organic results",
            "Meta video ads built around Paul's $21M story",
            "Systematic reviews to overtake the 3-pack",
            "Geo landing pages: Salem, Blacksburg, Abingdon",
        ],
    ),
    (
        ...
        [
            ...
            "24/7 intake — first to answer wins the case",
            "Callback protocol for after-hours serious cases",
            "Intake coordinator pre-qualifies each prospect",
            "Stop losing urgent cases to whoever answers first",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Build a team Paul trusts to run without him",
            "KPI dashboard: case source + close rate data",
            "Case-level profit tracking by practice type",
            "SOPs so the firm runs on Paul's standards",
            "Know what the firm earns — and what it is worth",
        ],
    ),
]
...
PACKAGES = [
    ...
    (
        "ELITE COACH PLUS",
        "$2,200", "$3,497/mo",
        "Weekly coaching · Intake build · KPI dashboard · Workshops",
        "6D28D9",
    ),
]
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $5,700–$15,000/mo paid directly to Google/Meta/LSA"
)

AVG_CASE_VALUE      = "$25,000 (working est.)"
CONSERVATIVE_LABEL  = "Conservative  (4–5 cases/mo):"
CONSERVATIVE_RESULT = "$112,500 revenue · ~20× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (14–15 cases/mo):"
AGGRESSIVE_RESULT   = "$362,500 revenue · ~24× ROAS"

TIMELINE = [
    ("Day 1",    "Onboarding call + ads strategy finalized"),
    ("Day 14",   "Google Ads & LSA campaigns go live"),
    ("Week 2",   "Website rebuild begins — form above the fold"),
    ("Week 3",   "Post-settlement review request workflow live"),
    ("Month 3",  "After-hours intake + KPI dashboard active"),
]

CLOSING_QUOTE = (
    '"29 years of exceptional work deserves more than a referral waiting game — '
    'it deserves a firm that runs itself and a pipeline that never stops."'
)
