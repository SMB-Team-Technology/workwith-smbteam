...
Output: truman-law/TRUMAN_LAW_05282026_Proposal.pptx
"""
...
# Report metadata
FIRM_NAME      = "TRUMAN LAW"
SALES_REP      = "Randy Gold"
OUTPUT_PATH    = "truman-law/TRUMAN_LAW_05282026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL", "Invisible in 3-pack"),   # Lead Generation
    ("RED",   "CRITICAL", "No after-hours capture"), # Intake
    ("AMBER", "CAUTION",  "Solo, no team layer"),    # Team
    ("AMBER", "CAUTION",  "No financial tracking"),  # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Invisible on Google for all practice areas — zero digital leads captured today"),
    ("neg", "~20 reviews vs. 131 (Keystone) — locked out of the local 3-pack entirely"),
    ("neg", "Old address still in directories — NAP inconsistency suppresses SEO rankings"),
    ("pos", "5.0-star rating — every client who finds Truman Law becomes a loyal advocate"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Keystone Law Group",   "131 reviews", "4.9★ · owns 3-pack"),
    ("Walia Law Group",      "100 reviews", "4.9★ · S. Asian mkt"),
    ("Alpine Legal Services","100+ reviews","geo-pages every city"),
]
CLIENT_REVIEWS      = "~20 reviews"
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $75,000/Month"

SMB_MODEL_DESC = (
    "Lead generation, intake, a self-managing team, and a profit plan — "
    "all four must work together. "
    "When they do, Harkirat can build the recognized firm Surrey's South Asian "
    "community turns to first — without bottlenecking every file personally."
)

GOAL_HEADLINE = "$400K → $800K revenue"
GOAL_DBM      = "Recognized firm — community turns to you first"
...
PRIORITIES = [
    (
        ...
        [
            "Turn Google into a 24/7 client pipeline",
            "Launch LSA — top of page, pay per call only",
            "Fix GBP and close the 100-review gap fast",
            "Geo-pages for Burnaby, Langley, and Delta",
            "Punjabi/Hindi campaign — category of one",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Add above-fold form — stop losing high-intent visits",
            "After-hours capture — call back before competitors do",
            "Intake scripts so you stop fielding every first call",
            "Track leads to measure conversion rate each month",
            "Respond inside 1 hour — beat every firm in Surrey",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Hire intake coordinator — reclaim your daily hours",
            "Document every process before the first hire starts",
            "Track profit by practice area — know your real ROI",
            "Identify highest-margin work and scale it first",
            "Build the plan that lets you step back, not work more",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$4,847", "$5,697/mo",
        "Website · Google Ads · LSA · Local SEO · Meta retargeting",
        "1D4ED8",
    ),
    (
        "ELITE COACH PLUS",
...
BUNDLE_TOTAL   = "$7,047 / mo"
BUNDLE_SAVINGS = "Save $2,147/mo by bundling"

AD_SPEND_NOTE = (
    "+ Recommended ad spend: $3,200–$5,000/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE      = "$2,500"
CONSERVATIVE_LABEL  = "Conservative  (3 cases/mo):"
CONSERVATIVE_RESULT = "$7,500 revenue · 2.3× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (6 cases/mo):"
AGGRESSIVE_RESULT   = "$15,000 revenue · 3.0× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Coaching kickoff — set 90-day growth targets"),
    ("Day 14",  "Google Ads + LSA go live across Surrey"),
    ("Week 2",  "Website + practice area landing pages launch"),
    ("Week 3",  "GBP optimized, review generation campaign live"),
    ("Month 3", "3-pack progress reviewed, intake system active"),
]

CLOSING_QUOTE = (
    "“The community is there. The reputation is real. "
    "It’s time to build the firm that Surrey reaches for first.”"
)
