...
# Report metadata
FIRM_NAME      = "The Sharp Firm"
SALES_REP      = "Dan Bryant"
OUTPUT_PATH    = "the-sharp-firm/The_Sharp_Firm_August_11_2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL", "No paid leads yet"),      # Lead Generation
    ("AMBER", "AMBER",    "Manual triage only"),      # Intake
    ("RED",   "CRITICAL", "No staff, no CRM"),        # Team
    ("RED",   "CRITICAL", "No cost tracking"),        # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Screening 5-10 med/mal calls daily, rejecting 99%"),
    ("neg", "No Meta pixel, remarketing, or LSA for target cases"),
    ("neg", "Thomas J. Henry & Jim Adler dominate paid search"),
    ("pos", "4.9-star, 97-review reputation across 30+ years"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
# List 3 competitors + the client row is added automatically.
COMPETITORS = [
    ("Thomas J. Henry Injury Attorneys", "3,083 reviews", "Dominates PPC & volume"),
    ("McMinn Personal Injury Lawyers",   "390 reviews",   "Owns truck/18-wheeler niche"),
    ("Carabin & Shaw, P.C. (Austin)",    "38 reviews",    "Smaller, but active in niche"),
]
CLIENT_REVIEWS      = "97 reviews"
...
SMB_MODEL_DESC = (
    ...
    "When they do, Lance can finally step away from screening every call himself."
)

GOAL_HEADLINE = "$570K → $1.14M revenue"
GOAL_DBM      = "Step away from screening every call himself"
...
PRIORITIES = [
    (
        ...
        [
            "Launch Meta retargeting & lead gen campaigns",
            "Set up Local Service Ads for truck & auto cases",
            "Fix NAP listings across Avvo, FindLaw, Yelp",
            "Turn referral network into a warm ad audience",
            "Turn your reputation into cases that find you",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Centralize leads in one tracked system",
            "Build a fast follow-up sequence for new leads",
            "Route auto/truck leads to the front of the line",
            "Add structured after-hours lead capture",
            "Stop spending your days screening the wrong calls",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Start Elite Coach Plus weekly group coaching",
            "Track cost per lead and cost per case",
            "Build delegation so intake doesn't run through you",
            "Plan the next hire once volume outpaces you",
            "Get free time back without revenue slipping",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$4,997", "$5,697/mo",
        "Meta retargeting · Meta lead gen · LSA · Local SEO",
        "1D4ED8",
    ),
    (
        "ELITE COACH PLUS",
        "$3,200", "$3,497/mo",
        "Weekly group coaching · PI mastermind · Workshops",
        "6D28D9",
    ),
]
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $10,000–$18,000/mo paid directly to Meta/LSA"
)

AVG_CASE_VALUE      = "$5,000–$10,000"
CONSERVATIVE_LABEL  = "Conservative  (7 cases/mo):"
CONSERVATIVE_RESULT = "$52,500 revenue · 5.25× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (16 cases/mo):"
AGGRESSIVE_RESULT   = "$120,000 revenue · 6.67× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Meta Pixel installed, campaigns queued"),
    ("Day 14",  "Meta retargeting & lead gen campaigns live"),
    ("Week 2",  "LSA setup complete, NAP cleanup underway"),
    ("Week 3",  "Referral list uploaded, remarketing ads live"),
    ("Month 3", "Coaching cadence set, lead tracking in place"),
]

CLOSING_QUOTE = (
    '"I want my phone ringing with the cases I actually want — '
    'not more calls I have to turn away."'
)
