...
# Report metadata
FIRM_NAME      = "Kamaluddin Law"
SALES_REP      = "Jacob Meissner"
OUTPUT_PATH    = "kamaluddin-law/Kamaluddin Law_August 18, 2026_Proposal.pptx"
...
PILLARS = [
    ...
    ("RED",   "CRITICAL", "No delegation today"),
    ("RED",   "CRITICAL", "Owner is bottleneck"),
    ("AMBER", "AMBER",    "Case-by-case fees"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Invisible in all 6 of 6 local searches run in this market"),
    ("neg", "Every case evaluation and fee negotiation runs through Nahla alone"),
    ("neg", "A GBP verification block is hiding the listing from local search"),
    ("pos", "Review profile compares favorably to every named competitor here"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Weinstock Immigration Lawyers", "Strong reviews", "Wins \"best Atlanta\" search"),
    ("Orgesi Pandeli / Delgado Law",  "Sponsored ads",   "Owns paid search slots"),
    ("Hammers Accident & Injury",     "Strong reviews",  "Dominates PI in Smyrna"),
]
CLIENT_REVIEWS      = "Strong reviews"
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $200,000/Month"

SMB_MODEL_DESC = (
    "All four pillars — leads, intake, team, and profit — must work together. "
    "When all four work, Nahla can lead a legacy firm instead of running every case herself."
)

GOAL_HEADLINE = "$1.2M → $2.4M revenue"
GOAL_DBM      = "Step into the CEO role you've earned"
...
PRIORITIES = [
    (
        ...
        [
            ...
            "Fix GBP category & resolve verification block",
            "Correct the NAP listing conflict on directories",
            "Launch LSA to convert reviews into new calls",
            "Add geo-targeted pages for Marietta & Cobb County",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "69CD2B",
        [
            "Move the case evaluation form above the fold",
            "Build a delegated intake process",
            "Create standard fee & payment-plan scripts",
            "Add a defined follow-up sequence for leads",
            "Free Nahla from answering every call herself",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "003A59",
        [
            "Start Elite Coach Plus delegation training",
            "Define roles beyond Nahla for casework",
            "Standardize the fee model by practice area",
            "Track profit & acquisition cost per case type",
            "Build the bench of attorneys for your legacy firm",
        ],
    ),
]
...
PACKAGES = [
    ...
    (
        "ELITE COACH PLUS",
        "$3,200", "$3,497/mo",
        "Weekly group coaching · masterminds · delegation & hiring plan",
        "003A59",
    ),
]
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $8,000–$24,000/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE     = "$4,500"
CONSERVATIVE_LABEL = "Conservative  (13 cases/mo):"
CONSERVATIVE_RESULT = "$56,800 revenue · 7.1× ROAS"
AGGRESSIVE_LABEL   = "Aggressive  (46 cases/mo):"
AGGRESSIVE_RESULT  = "$207,000 revenue · 8.6× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Launch Google Ads & LSA for immigration law"),
    ("Day 14",  "Correct GBP category & resolve verification"),
    ("Week 2",  "Fix NAP conflict & clean up directory listings"),
    ("Week 3",  "Move case form above the fold; add bio page"),
    ("Month 3", "Kick off Elite Coach Plus delegation plan"),
]

CLOSING_QUOTE = (
    '"I want to build a legacy firm — with a team of attorneys I\'ve trained — '
    'so I can finally lead instead of doing it all myself."'
)
