...
# Report metadata
FIRM_NAME      = "Davidek Law Firm Pllc"
SALES_REP      = "Randy Gold"
OUTPUT_PATH    = "davidek/Davidek Law Firm Pllc_July 24, 2026_Proposal.pptx"
...
PILLARS = [
    ("AMBER", "AMBER", "Ad spend unclear"),
    ("AMBER", "AMBER", "Close rate slipping"),
    ("AMBER", "AMBER", "3 of 5 attorneys"),
    ("AMBER", "AMBER", "Payroll over 50%"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Payroll above 50% of revenue is driving margins to single digits"),
    ("neg", "Two of five attorney seats have sat open for over a year"),
    ("neg", "Paid ads are fully agency-managed with no in-house visibility"),
    ("pos", "156 reviews and a 4.9 rating already beat every named competitor"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("J.M. Dickerson Law Firm", "59 reviews", "5.0 · most visible"),
    ("Carroll Law Group",       "39 reviews", "4.9 · Medicaid APT niche"),
    ("Cludius Law",             "Avvo 5.0",   "EP boutique · own pages"),
]
CLIENT_REVIEWS      = "156 reviews"
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $4M"

SMB_MODEL_DESC = (
    "All four pillars — leads, intake, team, and profit — have to work "
    "together. When they do, Dirk can finally step back from agency "
    "guesswork and legacy cleanup and build the $4M firm worth selling."
)

GOAL_HEADLINE = "$2.7M → $4M revenue"
GOAL_DBM      = "A firm worth building toward a sale"
...
PRIORITIES = [
    (
        ...
        [
            "Launch Dominate ads with real reporting",
            "Fix the split phone number issue",
            "Add a contact form above the fold",
            "Build Austin pages before 2027 launch",
            "Turn ad spend into leads you can see",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Filter tire-kicker inquiries fast",
            "Protect the ~80% close rate",
            "Add after-hours coverage",
            "Standardize follow-up for every lead",
            "Turn booked appointments into signed clients",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Fill the two open attorney seats",
            "Turn CFO data into an executed plan",
            "Cut payroll toward the 25% target",
            "Build leadership beyond the CFO",
            "Free Dirk from running every seat",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$10,497", "$12,497/mo",
        "Google Ads · LSA · Meta Ads · Website · Austin pages",
        "1D4ED8",
    ),
    (
        "MASTER'S CIRCLE",
...
AVG_CASE_VALUE      = "$2,250 (est.)"
CONSERVATIVE_LABEL  = "Conservative  (~17 cases/mo):"
CONSERVATIVE_RESULT = "$35–40K revenue · ~11x ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (~168 cases/mo):"
AGGRESSIVE_RESULT   = "$370–380K revenue · ~27x ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Launch Dominate ad campaigns, full reporting"),
    ("Day 14",  "Fix phone number, add contact form"),
    ("Week 2",  "Kick off Master's Circle group coaching"),
    ("Week 3",  "Begin Austin geo-page build"),
    ("Month 3", "Intake filtering live, CFO plan underway"),
]

CLOSING_QUOTE = (
    '"The demand is already there. The systems to turn it into a $4M, '
    '20%-margin firm are not yet — but they can be."'
)
