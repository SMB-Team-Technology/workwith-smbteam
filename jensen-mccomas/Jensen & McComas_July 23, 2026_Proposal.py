...
# Report metadata
FIRM_NAME      = "Jensen & McComas, LLC"
SALES_REP      = "Jacob Meissner"
OUTPUT_PATH    = "jensen-mccomas/Jensen & McComas_July 23, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL", "No formal marketing"),
    ("RED",   "CRITICAL", "Leaky intake process"),
    ("AMBER", "AMBER",    "No ops owner yet"),
    ("AMBER", "AMBER",    "No profit plan yet"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Google Business Profile has zero reviews vs. established local rivals"),
    ("neg", "A leaky intake process is the stated cause of the firm's growth plateau"),
    ("neg", "No profit plan tracks $85K Corporate matters vs. $20K T&E engagements"),
    ("pos", "Already diagnosed intake and hiring as the priority before marketing"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Levin & Gann, P.A.",       "BBB A+",        "AV Preeminent · decades est."),
    ("PK Law (Pessin Katz)",     "24 attorneys",  "Best Lawyers 2026"),
    ("Caruso & Zerwitz",         "Est. 1999",     "Long-tenured T&E practice"),
]
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $3M"

SMB_MODEL_DESC = (
    "All four pillars must work together — for Jensen & McComas, that starts "
    "with intake and team, not marketing. When Kristine can add five hires "
    "without building every system herself, the firm is ready to scale."
)

GOAL_HEADLINE = "$1.2M → $3M revenue"
GOAL_DBM      = "A firm built to hold what's coming next"
...
PRIORITIES = [
    (
        ...
        [
            "Map today's intake workflow end to end",
            "Build response-time and follow-up rules",
            "Give the AI intake tool a real process first",
            "Turn more referrals into signed clients",
            "Free her from chasing every lead herself",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Build the hiring plan for 5 new roles",
            "Give operations a real owner besides her",
            "Onboard admins without breaking today's team",
            "Weekly coaching to build leadership habits",
            "Let her step back from building every system",
        ],
    ),
    (
        "Build the", "Marketing Engine (Phase 2)", "1D4ED8",
        [
            "Launch Google Ads for Corporate & T&E",
            "Claim Google reviews competitors already have",
            "Fix NAP errors across Avvo, BBB, directories",
            "Add Local Service Ads once reviews are live",
            "Turn referrals into a real growth engine",
        ],
    ),
]
...
PACKAGES = [
    (
        "FCOO ADVISOR",
        "$3,297", "$3,797/mo",
        "Intake design · hiring plan · weekly group coaching",
        "6D28D9",
    ),
]
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend (Phase 2): $5,500-$8,500/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE      = "$20,000"
CONSERVATIVE_LABEL  = "Conservative  (6 cases/mo):"
CONSERVATIVE_RESULT = "$120,000 revenue · 21.8x ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (11 cases/mo):"
AGGRESSIVE_RESULT   = "$220,000 revenue · 25.9x ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "FCOO kickoff — map today's intake workflow"),
    ("Day 14",  "Draft the defined intake process & rules"),
    ("Week 2",  "Build hiring plan for the 5 scoped roles"),
    ("Week 3",  "First weekly group coaching session"),
    ("Month 3", "NAP & directory cleanup complete"),
]

CLOSING_QUOTE = (
    '"I want to build this firm to hold what\'s coming next — '
    'not just grow it and hope it holds together."'
)
