...
# Report metadata
FIRM_NAME      = "Alexander & Alexander"
SALES_REP      = "Randy Gold"
OUTPUT_PATH    = "alexander-alexander/Alexander & Alexander_July 9, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "RED",   "No tracked ad system"),   # Lead Generation
    ("RED",   "RED",   "Wil runs pre-suit deals"),  # Intake
    ("AMBER", "AMBER", "No associate yet"),   # Team
    ("AMBER", "AMBER", "No formal profit plan"),  # Profit Plan
]
...
FINDINGS = [
    ("neg", "No tracked paid leads — past PPC/LSA abandoned, no tracking"),
    ("neg", "Wil personally runs pre-suit negotiations, ~$520K/yr cost"),
    ("neg", "Theos & Jeffcoat building geo pages in your own market"),
    ("pos", "121 five-star reviews — best reputation in the trade area"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
# List 3 competitors + the client row is added automatically.
COMPETITORS = [
    ("Young & Thurmond",  "119 reviews", "5.0 stars · Aiken, SC"),
    ("Theos Law Firm",    "96 reviews",  "4.8 stars · Barnwell page"),
    ("Jeffcoat Injury",   "Multi-loc.",  "Orangeburg SEO pages"),
]
CLIENT_REVIEWS      = "121 reviews"
CLIENT_REVIEWS_NOTE = "← You are here"

# Stage strip (right panel, Slide 1)
STAGE_TEXT = "Now: $1.2M/yr, owner-run  →  Goal: $3-4M/yr, self-managing firm"

# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $3-4M/Year"

SMB_MODEL_DESC = (
    "All four pillars — marketing, intake, team, and profit — must work "
    "together. When all four pillars work, Wil can finally step away and "
    "trust the firm runs itself."
)

GOAL_HEADLINE = "$1.2M → $3-4M revenue"
...
PRIORITIES = [
    (
        ...
        [
            "Launch tracked Google Ads, all 3 practice areas",
            "Activate Local Service Ads on 121 five-star reviews",
            "Build geo-pages for Aiken, Barnwell, Orangeburg",
            "Add Meta retargeting so no website visitor is lost",
            "Turn reviews into a lead engine that runs without Wil",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Stand up full lead tracking on every channel",
            "Build a defined follow-up process for every lead",
            "Add after-hours coverage for inbound calls",
            "See exactly which leads convert and which don't",
            "Stop losing cases the firm already paid to generate",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Delegate pre-suit negotiations off Wil's plate",
            "Weekly coaching to build a self-managing team",
            "Build a real, tracked profit plan past $1.2M flat",
            "Add KPI scorecards so the team runs on its own",
            "Free up the hours for Wil's two-week vacation",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$7,397", "$8,997/mo",
        "Google Ads · LSA · Local SEO geo-pages · Meta Ads · tracking",
        "1D4ED8",
    ),
    (
        "ELITE COACH PLUS",
...
AVG_CASE_VALUE     = "$21,000"
CONSERVATIVE_LABEL = "Conservative  (2 cases/mo):"
CONSERVATIVE_RESULT = "$43,000 revenue · 5.7× ROAS"
AGGRESSIVE_LABEL   = "Aggressive  (10 cases/mo):"
AGGRESSIVE_RESULT  = "$210,000 revenue · 7.0× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Launch tracked Google Ads & LSA campaigns"),
    ("Day 14",  "Full conversion tracking live on every channel"),
    ("Week 2",  "Geo-pages built for Aiken, Barnwell, Orangeburg"),
    ("Week 3",  "Elite Coach Plus starts delegating pre-suit work"),
    ("Month 3", "Follow-up process live — no lead falls through"),
]

CLOSING_QUOTE = (
    '"A firm that runs itself well enough that I can take two weeks off '
