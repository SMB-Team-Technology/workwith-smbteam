...
# Report metadata
FIRM_NAME      = "Erkan & Sullivan"
SALES_REP      = "Michael Kopp"
OUTPUT_PATH    = "erkan-sullivan-ryan/Erkan_Sullivan_August_12_2026_Proposal.pptx"
...
PILLARS = [
    ("AMBER", "AMBER", "Only 212 visitors/mo"),   # Lead Generation
    ("AMBER", "AMBER", "No process defined"),      # Intake
    ("AMBER", "AMBER", "Partners do it all"),       # Team
    ("AMBER", "AMBER", "No revenue baseline"),      # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Only 212 organic visitors/mo — a 6-9x gap behind named competitors"),
    ("neg", "Zero paid channels ever run — no PPC, LSA, or Meta ads"),
    ("neg", "A landmark case that helped 27,000 people has never built the brand"),
    ("pos", "5.0-star, 154-review reputation — stronger than most local rivals"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Paul M. King (kingatlaw.com)",       "40+ reviews",       "Same Lowell/Andover market"),
    ("Gilman Law, P.C.",                   "213-470 reviews",   "Out-reviews client 3x"),
    ("Law Offices of Stephen Neyman",      "1,900 visitors/mo", "9x our organic traffic"),
]
CLIENT_REVIEWS      = "154 reviews"
CLIENT_REVIEWS_NOTE = "← 5.0★, but invisible online"

# Stage strip (right panel, Slide 1)
STAGE_TEXT = "Stage 4: Small Business Manager  →  Goal: Stage 6, Law Firm Owner"

# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $20K-$30K Case Values"

SMB_MODEL_DESC = (
    "All four pillars — lead generation, intake, team, and profit — must work "
    "together. When they do, Murat and Ryan can finally build the premium, "
    "high-margin practice they've earned instead of chasing volume."
)

GOAL_HEADLINE = "~$15K → $20K-$30K case value"
GOAL_DBM      = "Take only the cases that build the brand"
...
PRIORITIES = [
    (
        ...
        [
            "Launch Google Ads for OUI/DUI searches",
            "Turn the landmark case into a PR asset",
            "Launch Meta retargeting for site visitors",
            "Fix local SEO & duplicate listing",
    ...
    (
        ...
        [
            "Set a same-day contact standard",
            "Add after-hours lead coverage",
            "Build a follow-up sequence for leads",
            "Track every inquiry to close or lost",
            "Prep intake for 25-30 cases/month",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "003A59",
        [
            "Define case-routing for the new associate",
            "Track profitability by case type",
            "Build toward the $800/hr partner target",
            "Weekly coaching to build the growth plan",
            "Free partners for high-margin work",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$4,997", "$5,697/mo",
        "SEO · PPC · AI/AEO optimization · Meta retargeting",
        "0091C9",
    ),
    (
        "ELITE COACH PLUS",
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $6,700-$13,600/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE     = "$15,000"
CONSERVATIVE_LABEL = "Conservative  (5 cases/mo):"
CONSERVATIVE_RESULT = "$79,000 revenue · 11.9× ROAS"
AGGRESSIVE_LABEL   = "Aggressive  (13 cases/mo):"
AGGRESSIVE_RESULT  = "$195,000 revenue · 14.3× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Launch Google PPC for OUI/DUI keywords"),
    ("Day 14",  "Launch Meta retargeting campaign"),
    ("Week 2",  "Kick off Elite Coach Plus coaching"),
    ("Week 3",  "Rebuild site content around landmark case"),
    ("Month 3", "Structured review-gen program running"),
]

CLOSING_QUOTE = (
    '"A brand strong enough to impress judges deserves clients who find it '
    'before they find anyone else."'
)
