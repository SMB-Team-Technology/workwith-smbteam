...
NOTE FOR THIS FIRM: only one package (Elite Coach) is recommended — the
Call-Purpose Override applies (see section_11_workings.txt). PACKAGES below
intentionally contains a single tuple; the template's second card slot is
simply left empty.
"""
...
# Report metadata
FIRM_NAME      = "Passarini Law, PLLC"
SALES_REP      = "Nick Holderman"
OUTPUT_PATH    = "passarini-law/Passarini Law, PLLC_August 11, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED", "CRITICAL", "Marketing paused"),     # Lead Generation
    ("RED", "CRITICAL", "No intake process"),    # Intake
    ("RED", "CRITICAL", "No ops structure"),     # Team
    ("RED", "CRITICAL", "No revenue baseline"),  # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "No documented workflow exists from intake through off-boarding"),
    ("neg", "Andrea works ~80 hrs/week with no operations structure independent of her"),
    ("neg", "No revenue baseline, close rate, or case value to measure the growth goal against"),
    ("pos", "20+ years of credibility and a growing, in-demand immigration caseload"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Melissa Dacunha, P.A.",  "87 reviews", "family law & immigration"),
    ("Renata Castro / USA4ALL","46 reviews", "immigration boutique"),
    ("Mark R. Manceri, P.A.",  "14 reviews", "probate specialist"),
]
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Double or Triple Revenue"

SMB_MODEL_DESC = (
    "SMB Team builds growth firms across four pillars — Lead Generation, "
    "Intake, Team, and Profit. For Passarini Law, the priority right now is "
    "Team and Profit: the systems that let Andrea step back and retire on her own terms."
)

GOAL_HEADLINE = "~$250K → 2–3× Revenue"
GOAL_DBM      = "Step back from the firm — and finally retire on her terms"

# Each priority: (line1, line2, accent_color_hex, [5 bullet strings])
PRIORITIES = [
    (
        "Build the", "Marketing Foundation", "1D4ED8",
        [
            "Fix NAP errors on Avvo & Justia",
            "Resolve the passarinilaw.site redirect",
            "Launch a review-request habit at case close",
            "Build geo-pages for nearby cities (Phase 2)",
            "Grow visibility now — safely, with zero ad spend",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Document the intake-to-off-boarding workflow",
            "Move the contact form above the fold on site",
            "Give paralegals scripts, not guesswork",
            "Add a defined follow-up sequence for new leads",
            "Free Andrea from handling every intake call",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Turn tribal knowledge into written playbooks",
            "Build role clarity for the paralegal team",
            "Track monthly revenue & case value in Clio",
            "Set a 90-day capacity checkpoint before scaling",
            "Give Andrea a real number behind her retirement plan",
        ],
    ),
]
...
PACKAGES = [
    (
        "ELITE COACH",
        "$2,600", "$3,497/mo",
        "Weekly coaching · Intake playbooks · Accountability structure",
        "6D28D9",
    ),
]
...
AD_SPEND_NOTE = (
    "+ Phase 2 ad spend (not now): $3,500–$14,000/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE      = "~$4,250"
CONSERVATIVE_LABEL  = "Conservative  (5 cases/mo):"
CONSERVATIVE_RESULT = "$22,000 revenue · 6.3× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (25 cases/mo):"
AGGRESSIVE_RESULT   = "$106,000 revenue · 7.6× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Kick off Elite Coach with a process-mapping session"),
    ("Day 14",  "Draft first playbooks for family law & immigration"),
    ("Week 2",  "Assign clear paralegal roles for intake & follow-up"),
    ("Week 3",  "Start monthly revenue & case-value tracking in Clio"),
    ("Month 3", "Capacity checkpoint — decide when to add marketing"),
]

CLOSING_QUOTE = (
    '"Five years from now, this firm runs without me — and I finally get to retire on my own terms."'
)
