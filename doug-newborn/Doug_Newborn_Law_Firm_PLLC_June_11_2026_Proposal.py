...
# Report metadata
FIRM_NAME      = "Doug Newborn Law Firm, PLLC"
SALES_REP      = "Dan Bryant"
OUTPUT_PATH    = "doug-newborn/Doug_Newborn_Law_Firm_PLLC_June_11_2026_Proposal.pptx"
...
PILLARS = [
    ("AMBER", "AMBER", "PI has no paid digital"),        # Lead Generation
    ("AMBER", "AMBER", "Contact form buried below fold"), # Intake
    ("AMBER", "AMBER", "No KPIs / Doug escalation"),      # Team
    ("AMBER", "AMBER", "No P&L dashboard"),               # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "PI invisible in paid search — Lerner & Rowe captures every Tucson injury keyword"),
    ("neg", "7 attorneys, no KPIs — recoverable billing runs untracked at $350/hr"),
    ("neg", "Clio Draft paid and stalled 12+ months — $8-12K invested, zero ROI"),
    ("pos", "850+ Google reviews — strongest review moat in Tucson Probate and EP"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Lerner & Rowe Injury Attorneys", "870 reviews", "PI · heavy PPC · Tucson"),
    ("Zanes Law Injury Lawyers",       "400+ reviews", "PI · 2003 · $500M+ won"),
    ("Law Office of David I. Karp",    "9.0 Avvo",     "Probate · Oracle Rd"),
]
CLIENT_REVIEWS      = "850+ reviews"
CLIENT_REVIEWS_NOTE = "← You are here"

# Stage strip (right panel, Slide 1)
STAGE_TEXT = "Stage 5: Law Firm CEO  →  Goal: Stage 6, Law Firm Owner"

# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $10M Revenue"

SMB_MODEL_DESC = (
    "Lead Generation, Intake, Team, and Profit each depend on the others. "
    "Strengthen all four and the firm grows without more of Doug's personal time — "
    "until he can step back and trust it keeps running."
)

GOAL_HEADLINE = "$5M → $10M revenue"
GOAL_DBM      = "Step back — trust the firm runs"
...
PRIORITIES = [
    (
        ...
        [
            "Launch PI paid search — your story vs. every PI term",
            "Turn Google into a PI pipeline that runs without you",
            "Geo pages for Marana, Oro Valley — more suburb leads",
            "Meta Pixel: recapture PPC visitors who didn't call",
            "NAP audit — fix citation conflicts hurting rankings",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Above-fold contact form — capture after-hours leads",
            "Define PI intake pipeline separate from Doug's cases",
            "Track close rate by practice — find where leads die",
            "After-hours coverage: every lead gets a same-day call",
            "Every missed lead costs $900 CAC — fix follow-up now",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "KPIs + utilization targets for all 7 attorneys",
            "Weekly cadence: huddles, dashboards run without Doug",
            "Drive Clio Draft to completion — 30-day sprint",
            "Practice area P&L — managed weekly, not quarterly",
            "FCOO owns the operational week; Doug owns his time",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$15,997", "$18,997/mo",
        "PPC + LSA · SEO · Geo pages · Meta Pixel · Web CRO",
        "1D4ED8",
    ),
    (
        "MASTER'S CIRCLE + FCOO DIRECTOR",
        "$5,394", "$7,794/mo",
        "FCOO 9hrs/mo · KPIs · Clio sprint · Master's Circle",
        "6D28D9",
    ),
]

BUNDLE_TOTAL   = "$21,391 / mo"
BUNDLE_SAVINGS = "Save $5,400/mo by bundling"

AD_SPEND_NOTE = (
    "+ Recommended ad spend: $10,000–$40,000/mo paid directly to Google/Meta/LSA"
)

AVG_CASE_VALUE      = "$7,000"
CONSERVATIVE_LABEL  = "Conservative  (~6 cases/mo):"
CONSERVATIVE_RESULT = "$42,000 revenue · 4.2× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (~30 cases/mo):"
AGGRESSIVE_RESULT   = "$200,000 revenue · 5× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Kickoff: PI ads strategy + FCOO intro call"),
    ("Day 14",  "PI Google Ads + LSA campaigns go live"),
    ("Week 2",  "Contact form above fold + Meta Pixel installed"),
    ("Week 3",  "KPIs + utilization targets set for all attorneys"),
    ("Month 3", "Clio Draft sprint complete; geo pages indexed"),
]

CLOSING_QUOTE = (
    '"The firm has the revenue, the team, and the reviews — '
    'what you want now is to step back on a Friday and know Monday runs without you."'
)
