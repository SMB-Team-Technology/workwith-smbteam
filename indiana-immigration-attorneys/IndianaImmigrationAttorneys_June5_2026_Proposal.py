"""
Proposal PPTX — Indiana Immigration Attorneys
Generated from completed audit sections.
"""
...
# ═══════════════════════════════════════════════════════════════════
# FILL — All values extracted from audit sections
# ═══════════════════════════════════════════════════════════════════

FIRM_NAME   = "Indiana Immigration Attorneys"
SALES_REP   = "Jacob Meissner"
OUTPUT_PATH = "indiana-immigration-attorneys/IndianaImmigrationAttorneys_June5_2026_Proposal.pptx"

WEBSITE_SCREENSHOT_PATH = None

# ── Slide 1 ──────────────────────────────────────────────────────
# Urgency score: copied from section_05 urgency-score element
URGENCY_SCORE = "5"

# Pillars: (status, label, one-line detail ~20 chars)
# Copied from section_05 sc-card traffic circles and first finding bullet
PILLARS = [
    ("RED",   "CRITICAL", "No paid lead system"),
    ("RED",   "CRITICAL", "Atty handles intake"),
    ("AMBER", "AMBER",    "2-person team only"),
    ("RED",   "CRITICAL", "No financial tracking"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings: copied from section_executive_summary top priorities
# 3 neg (consequence-first) + 1 pos (genuine strength from section_05)
FINDINGS = [
    ("neg", "16 reviews vs. 105–792 competitors — invisible in local search"),
    ("neg", "No paid ads — open north Indiana market left unclaimed"),
    ("neg", "Sole intake point: lead volume growth bottlenecks at Libby"),
    ("pos", "Perfect 5.0 Google rating — strongest trust signal in market"),
]

# Competitors: from section_06 competitor callouts and research notes
COMPETITORS = [
    ("Flora Legal Group",       "406 reviews", "multilingual · statewide"),
    ("Banks & Brower",          "792 reviews", "highest vol in Indiana"),
    ("Law Office of C. Grimes", "105 reviews", "5.0 rating · more reach"),
]
CLIENT_REVIEWS      = "16 reviews"
CLIENT_REVIEWS_NOTE = "← You are here"

# Stage strip: from research notes Stage 3 → Stage 6
STAGE_TEXT = "Stage 3: Solo Practitioner  →  Goal: Stage 6, Law Firm Owner"

# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Build a Sustainable Firm"

SMB_MODEL_DESC = (
    "Lead Generation, Intake, Team, and Profit must all work "
    "together. Missing any one, growth stalls regardless of "
    "effort. When all four work, Libby can serve every north "
    "Indiana family who needs her — without the firm depending "
    "on her personal hours to survive."
)

# Copied from research notes DBM and revenue context
GOAL_HEADLINE = "Stage 3 → Stage 6 Firm"
GOAL_DBM      = "Serve more families — sustainably"

PRIORITIES = [
    (
        "Build the", "Marketing Engine", "1D4ED8",
        [
            "Turn Google into a lead pipeline in north Indiana",
            "Spanish-language ads reach the underserved community",
            "Claim the LSA #1 spot before any competitor does",
            "Build 50+ reviews in 90 days — close the gap",
            "Geo pages for Kokomo & nearby cities — own the region",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "After-hours intake captures urgent asylum/visa cases",
            "Track every lead — stop silent revenue disappearing",
            "Free Libby from intake calls to focus on casework",
            "Same-day response converts at 3× the rate of next-day",
            "Intake system runs before more leads start arriving",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Track revenue per case type — know what pays best",
            "Profit plan created before next growth investment",
            "Elite Coach accountability cadence every week",
            "Intake specialist role defined before hire is needed",
            "Financial clarity to answer: can I afford to hire?",
        ],
    ),
]

# ── Slide 3 ──────────────────────────────────────────────────────
# Package cards: copied from section_11 investment-grid
# (label, bundled_price, retail_price, services_line, accent_color_hex)
PACKAGES = [
    (
        "FULL SERVICE MARKETING — ESSENTIALS",
        "$2,397", "N/A stand-alone",
        "Google Ads · LSA · Meta/Spanish · GBP · Landing Pages",
        "1D4ED8",
    ),
    (
        "ELITE COACH",
        "$1,600", "$2,497/mo",
        "Weekly coaching · Immigration mastermind · Workshops",
        "6D28D9",
    ),
]

# Copied from section_11 investment-total and savings callout
BUNDLE_TOTAL   = "$3,997 / mo"
BUNDLE_SAVINGS = "Save $897/mo by bundling"

# Copied from section_11 Block 4 ad spend range row
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $3,000–$7,000/mo paid directly to Google/Meta"
)

# Copied from section_11 Block 4 ROI table
AVG_CASE_VALUE      = "~$3,500"
CONSERVATIVE_LABEL  = "Conservative  (3–4 cases/mo):"
CONSERVATIVE_RESULT = "~$14,000 revenue · ~4.7x ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (10–11 cases/mo):"
AGGRESSIVE_RESULT   = "~$37,000 revenue · ~5x ROAS"

# From section_11 Block 5 first-90-days bullets (condensed to ≤55 chars)
TIMELINE = [
    ("Day 1",    "Google Search campaigns + coach kickoff"),
    ("Day 14",   "Submit LSA Google Screened application"),
    ("Week 3",   "GBP review request strategy deployed"),
    ("Week 4",   "Geo landing pages live: Kokomo + nearby cities"),
    ("Month 3",  "50+ reviews added; Spanish campaigns running"),
]

# Client-facing closing quote — from exec-closing, translated to owner's voice
CLOSING_QUOTE = (
    '"Every family in north-central Indiana who needs an immigration attorney '
    'is one Google search away. The system that makes sure they find you — '
