...
# Report metadata
FIRM_NAME      = "The Fowles Law Firm"
SALES_REP      = "Dan Bryant"
OUTPUT_PATH    = "the-fowles/The Fowles Law Firm_September 15, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL", "~4% local share of voice"),
    ("AMBER", "AMBER",    "Iris covers intake 24/7"),
    ("RED",   "CRITICAL", "1-week vacation test"),
    ("AMBER", "AMBER",    "No profit target set"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Two named competitors hold ~100% and ~87% of local map-pack visibility"),
    ("neg", "Iris runs intake solo, 24/7, with no after-hours backup"),
    ("neg", "Firm can't run more than ~1 week without Justin — no hiring process or KPIs"),
    ("pos", "Iris's ~30-32% close rate shows real strength once a lead reaches her"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Doug Atkinson", "~100% map-pack", "Dominates local 3-pack"),
    ("Brian Foley Law", "100+ (self-reported)", "Former Chief Prosecutor"),
]
CLIENT_REVIEWS      = "Unverified"
CLIENT_REVIEWS_NOTE = "← Recommend live check"

# Stage strip (right panel, Slide 1)
STAGE_TEXT = "Stage 4: Small Business Manager  →  Goal: Stage 6, Law Firm Owner"

# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $1M in 3 Years"

SMB_MODEL_DESC = (
    "All four pillars — leads, intake, team, and profit — have to work "
    "together. When they do, Justin can finally step back and trust the firm runs without him."
)

GOAL_HEADLINE = "$550K → $1M revenue"
GOAL_DBM      = "Step back to part-time, be there for his kids"
...
PRIORITIES = [
    (
        ...
        [
            "Launch LSA — zero paid leads today",
            "Add PPC for DWI, drug, DV terms",
            "Close map-pack gap vs. Doug Atkinson",
            "Turn Google into a pipeline that runs itself",
            "Build reviews to match Brian Foley Law",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "69CD2B",
        [
            "Add after-hours coverage — stop lost calls",
            "Fix buried contact form on website",
            "Resolve 2.5-month Lawmatics CRM lockout",
            "Give Iris real backup, not just backup hours",
            "Free Justin from personally checking intake",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "003A59",
        [
            "Document hiring criteria for attorney #2",
            "Set KPIs so growth doesn't ride on Justin",
            "Build a profit plan, not just revenue",
            "Turn the $1M goal into real take-home pay",
            "Create leadership beyond Iris alone",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$4,997", "$5,697/mo",
        "LSA · Google PPC · Local SEO · Website Conversion Fixes",
        "0091C9",
    ),
    (
        "ELITE COACH",
        "$2,600", "$3,497/mo",
        "Weekly Group Coaching · Masterminds · Annual Workshop",
        "003A59",
    ),
]
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $7,500–$11,700/mo paid directly to Google/LSA"
)

AVG_CASE_VALUE     = "$5,000"
CONSERVATIVE_LABEL = "Conservative  (7 cases/mo):"
CONSERVATIVE_RESULT = "$35,000 revenue · 4.7× ROAS"
AGGRESSIVE_LABEL   = "Aggressive  (16-17 cases/mo):"
AGGRESSIVE_RESULT  = "$82,500 revenue · 7.1× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Kickoff call — align on LSA + PPC plan"),
    ("Day 14",  "LSA live for DWI, drug, DV terms"),
    ("Week 2",  "Fix contact form + city-page overlap"),
    ("Week 3",  "Elite Coach sessions begin"),
    ("Month 3", "PPC live, review-generation running"),
]

CLOSING_QUOTE = (
    '"I want this firm to run without me — so I can be there for my kids, '
    'not stuck at a desk."'
)
