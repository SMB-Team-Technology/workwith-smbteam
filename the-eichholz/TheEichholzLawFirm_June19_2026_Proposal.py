"""
Audit PowerPoint — The Eichholz Law Firm
...
FIRM_NAME   = "The Eichholz Law Firm"
SALES_REP   = "Michael Kopp"
OUTPUT_PATH = "the-eichholz/TheEichholzLawFirm_June19_2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL",  "Ads paused; gap widening"),
    ("AMBER", "AMBER",     "40% over ideal caseload"),
    ("AMBER", "AMBER",     "Stalemate blocks growth"),
    ("RED",   "CRITICAL",  "-$130K net loss; balloon"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

FINDINGS = [
    ("neg", "Projecting -$130K net loss; $2.1M balloon due end of 2027"),
    ("neg", "Casino (1,057 reviews) dominates every Savannah PI search while Eichholz ads are paused"),
    ("neg", "Case managers at 40% over ideal capacity — follow-up delays cost signed cases"),
    ("pos", "390 Google reviews at 4.4★ — strong reputation foundation to build on"),
]

COMPETITORS = [
    ("Jamie Casino Injury Attorneys", "1,057 reviews", "4.8★ · TV + Google + Meta"),
    ("Mike Hostilo Law Firm",         "628 reviews",   "4.6★ · Heavy regional media"),
    ("Roden Law",                     "60 reviews",    "4.9★ · High-quality boutique"),
]
CLIENT_REVIEWS      = "390 reviews"
CLIENT_REVIEWS_NOTE = "← You are here"

STAGE_TEXT = "Stage 4: Small Business Manager  →  Goal: Stage 6, Law Firm Owner"

# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $13M"

SMB_MODEL_DESC = (
    "SMB Team aligns all four pillars — marketing, intake, "
    "team, and profit — into one connected system. "
    "When all four work, David can step back from the "
    "daily grind and watch The Eichholz Law Firm reclaim "
    "its position as the dominant PI practice in the region."
)

GOAL_HEADLINE = "$5.2M → $13M revenue"
GOAL_DBM      = "Reclaim the top PI firm in Savannah"

PRIORITIES = [
    (
        "Build the", "Marketing Engine", "1D4ED8",
        [
            "Re-enter Savannah PI Google Ads today",
            "Earn Google Screened via LSA enrollment",
            "Build review velocity — close the 667-review gap",
            "Fix 3-number NAP inconsistency in directories",
            "Turn TV spot library into active Meta ads",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Rebalance case loads: 143 → 100-case ideal",
            "Add after-hours call coverage — 0 missed leads",
            "7-day follow-up for every new inquiry",
            "Resolve referral source rejection issue",
            "Optimize website form-to-call for mobile",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Set payroll target: 42% → 25% of revenue",
            "End COO-owner stalemate with governance plan",
            "Monthly profit review — not year-end surprise",
            "Build reserve plan for 2027 balloon payment",
            "KPI scorecard for each manager and attorney",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$15,997", "$18,997/mo",
        "Google Ads · LSA · Meta Ads · Local SEO · Website",
        "1D4ED8",
    ),
    (
        "ELITE COACH PLUS",
...
BUNDLE_TOTAL   = "$19,197 / mo"
BUNDLE_SAVINGS = "Save $3,297/mo by bundling"

AD_SPEND_NOTE = (
    "+ Recommended ad spend: $10,000–$40,000/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE      = "$15,000 (est.)"
CONSERVATIVE_LABEL  = "Conservative  (12 cases/mo):"
CONSERVATIVE_RESULT = "$180,000 revenue · 18× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (56 cases/mo):"
AGGRESSIVE_RESULT   = "$840,000 revenue · 21× ROAS"

TIMELINE = [
    ("Day 1",    "Google Ads + LSA campaigns launch live"),
    ("Day 14",   "Website mobile optimization complete"),
    ("Week 3",   "Directory NAP standardized across all platforms"),
    ("Month 1",  "Review velocity program active; 20+ reviews/mo"),
    ("Month 3",  "Coaching KPIs set; profit plan benchmarks in place"),
]

CLOSING_QUOTE = (
    "“You built this firm to $13 million once. "
    "The market still needs what you built — "
    "it just needs you to come back for it.”"
)
