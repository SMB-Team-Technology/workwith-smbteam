"""
Audit PowerPoint — Scott Levine Law
SMB Team Growth, Profit, and Freedom Roadmap
Proposal Date: June 26, 2026
"""
...
# ═══════════════════════════════════════════════════════════════════
# FILL — All placeholders resolved for Scott Levine Law
# ═══════════════════════════════════════════════════════════════════

FIRM_NAME   = "Scott Levine Law"
SALES_REP   = "Dan Bryant"
OUTPUT_PATH = "scott-levine-law/ScottLevineLaw_June262026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL",  "No ads · 1 review found"),
    ("RED",   "RED",       "No intake staff"),
    ("RED",   "RED",       "Solo — no team"),
    ("AMBER", "AMBER",     "No cost-per-case data"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

FINDINGS = [
    ("neg", "No digital presence — zero ads, ~1 review, competitors have 89+"),
    ("neg", "404 errors block Google from indexing key practice area content"),
    ("neg", "Every inquiry runs through Scott — no intake structure in place"),
    ("pos", "SB800 co-author: a credential no competitor in this market holds"),
]

COMPETITORS = [
    ("Artemis Law Group",       "89 reviews",    "Del Mar · 2 mi · 4.9★"),
    ("Naumann Law Firm",        "40+ yr firm",   "HOA focus · $500M recovered"),
    ("Thorsnes Bartolotta McG", "$2B recovered", "Tier 1 ranked · 10 attorneys"),
]
CLIENT_REVIEWS      = "~1 confirmed"
CLIENT_REVIEWS_NOTE = "← You are here"

STAGE_TEXT = "Stage 3: Solo Practitioner  →  Goal: Stage 6, Law Firm Owner"

# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Build a Consistent Case Pipeline"

SMB_MODEL_DESC = (
    "SMB Team builds four pillars in parallel — marketing, intake, "
    "team, and profit. Each one feeds the others. Growth in only one "
    "creates chaos, not scale. When all four work together, Scott's "
    "35-year credential set generates cases consistently — and he can "
    "finally choose which ones to take."
)

GOAL_HEADLINE = "~$2M → $4M revenue goal"
GOAL_DBM      = "Cases come in — on his terms"

PRIORITIES = [
    (
        "Build the", "Marketing Engine", "1D4ED8",
        [
            "Turn Google into a live case pipeline",
            "Launch SB800 ads — no competitor can claim this",
            "Fix 404s so Google can index the content",
            "Build city pages: SD, Encinitas, La Jolla",
            "1,000+ past clients asked for reviews — unlock LSA",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Add after-hours coverage — leads won't wait",
            "Simplify contact form — fewer fields, more leads",
            "Build follow-up for prospects who don't sign day one",
            "Define intake criteria before leads reach Scott",
            "Every intake hour costs a litigation hour",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Hire intake coordinator — free Scott for litigation",
            "Set up cost-per-case tracking from day one",
            "Build KPI dashboard: leads → cases → revenue",
            "Know profit per case type before scaling spend",
            "Scale where profit follows — not just revenue",
        ],
    ),
]
...
CONSERVATIVE_RESULT = "~$100K revenue · ~33× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (9–10 cases/mo):"
AGGRESSIVE_RESULT   = "~$500K revenue · ~42× ROAS"

TIMELINE = [
    ("Day 1",    "Coaching kickoff — KPIs and campaign strategy set"),
    ("Day 14",   "Google Ads live — SB800 and HOA defect keywords"),
    ("Week 2",   "Website rebuild starts — fix 404s, add landing pages"),
    ("Week 3",   "Review outreach to 1,000+ past clients begins"),
    ("Month 3",  "20+ reviews live, GBP optimized, LSA eligibility built"),
]

CLOSING_QUOTE = (
    '"The credentials are already there — 35 years, $50M recovered, and the law written in your name. '
    'The only thing missing is the system that makes them visible to every client searching right now."'
)
