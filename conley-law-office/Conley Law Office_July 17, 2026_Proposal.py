...
Output: conley-law-office/Conley Law Office_July 17, 2026_Proposal.pptx
"""
...
# Report metadata
FIRM_NAME      = "Conley Law Office"
SALES_REP      = "Nick Holderman"
OUTPUT_PATH    = "conley-law-office/Conley Law Office_July 17, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED", "CRITICAL", "No paid ads found"),
    ("RED", "CRITICAL", "Buried contact form"),
    ("RED", "CRITICAL", "Solo attorney only"),
    ("RED", "CRITICAL", "Trust acct errors"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Misapplied trust funds already occurred — a bigger risk as NC rules tighten"),
    ("neg", "Zero paid leads across family law, criminal defense, or PI — referrals only"),
    ("neg", "Ginny is the only attorney — the firm can't run or sell without her"),
    ("pos", "12 years as a prosecutor: real, provable credibility to build on"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Summers & Associates", "210 reviews", "Largest local review base"),
    ("Klie Law Offices",     "38-110 reviews", "4-city footprint"),
    ("Raber Law Offices",    "25+ reviews", "Direct PI/family competitor"),
]
CLIENT_REVIEWS      = "Unverified"
CLIENT_REVIEWS_NOTE = "← Confirm live"

# Stage strip (right panel, Slide 1)
STAGE_TEXT = "Stage 3: Solo Practitioner  →  Goal: Stage 6, Law Firm Owner"

# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $70K/Month"

SMB_MODEL_DESC = (
    "Leads, intake, team, and profit all have to work together. "
    "When they do, Ginny can step back from day-to-day casework and build a firm ready to sell."
)

GOAL_HEADLINE = "$425K → $850K revenue"
GOAL_DBM      = "A firm ready to hand off — or sell"
...
PRIORITIES = [
    (
        ...
        [
            "Launch Google Ads for all 3 practice areas",
            "Fix the NAP mismatch across directories",
            "Claim and optimize your Google profile",
            "Add Local Service Ads for pay-per-lead",
    ...
    (
        ...
        [
            "Move contact form to the homepage",
            "Add after-hours coverage for new leads",
            "Build a trackable intake process",
            "Stop losing leads to faster competitors",
            "Free your time from answering every call",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Finish the trust account cleanup",
            "Complete the Clio migration",
            "Build a leadership layer before NC opens",
            "Track real profit, not just revenue",
            "Build the firm a buyer can step into",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$4,847", "$5,697/mo",
        "Website · Local SEO · Google PPC · LSA · Meta Ads",
        "1D4ED8",
    ),
    (
        "ELITE COACH PLUS",
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $9,200-$16,700/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE      = "$6,500"
CONSERVATIVE_LABEL  = "Conservative  (2-3 cases/mo):"
CONSERVATIVE_RESULT = "$14,300 revenue · 1.6x ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (4-5 cases/mo):"
AGGRESSIVE_RESULT   = "$31,200 revenue · 1.9x ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Launch PPC & LSA across all practice areas"),
    ("Day 14",  "Contact form moves to the homepage"),
    ("Week 2",  "GBP claimed, optimized, NAP fixed"),
    ("Week 3",  "Review-generation process live"),
    ("Month 3", "Coaching cadence set, trust cleanup done"),
]

CLOSING_QUOTE = (
    '"In five years, I want a firm someone would actually want to buy — '
    'not just one that\'s busy."'
)
