...
# Report metadata
FIRM_NAME      = "Guymon Law"
SALES_REP      = "Jacob Meissner"
OUTPUT_PATH    = "guymon-law/Guymon Law_August 11, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL", "Invisible in search"),
    ("RED",   "CRITICAL", "Manual, 5+ systems"),
    ("AMBER", "AMBER",    "AI use is ad hoc"),
    ("AMBER", "AMBER",    "No stated goal"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Intake spans 5+ disconnected systems, costing hours every week"),
    ("neg", "Didn't rank in any local search test run for this session"),
    ("neg", "Staff outside Amber are hesitant to adopt AI on their own"),
    ("pos", "16 years in business with an A+ BBB rating and two real offices"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Arizona Law Group",  "400+ reviews", "4.8★ · Scottsdale"),
    ("Owens & Perkins",    "240 reviews",  "4.8★ · Scottsdale"),
    ("Genesis Family Law", "190+ reviews", "~5.0★ · Scottsdale"),
]
CLIENT_REVIEWS      = "13–14 (proxy)"
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $1.14M/Year"

SMB_MODEL_DESC = (
    "All four pillars work together, but for Guymon Law the fastest lever is "
    "capacity: freeing hours through AI so the team does more without adding "
    "headcount. When that capacity is real, Amber can finally pay her people more."
)

GOAL_HEADLINE = "$570K → $1.14M revenue"
GOAL_DBM      = "Pay your team more without new hires"

# Each priority: (line1, line2, accent_color_hex, [5 bullet strings])
PRIORITIES = [
    (
        "Deploy the", "AI Workforce", "1D4ED8",
        [
            "Launch Fractional CTO-led automation build",
            "Automate intake & conflict-checks across systems",
            "Standardize AI use firm-wide, not just ad hoc",
            "Free Amber from performance reviews & tutorials",
            "Turn freed capacity into real pay raises",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Time", "0F766E",
        [
            "Move contact form above the fold",
            "Connect 5+ intake systems into one workflow",
            "Confirm PageSpeed & GBP data manually",
            "Track leads through to signed cases",
            "Cut manual conflict-check time firm-wide",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Set a stated revenue goal to plan against",
            "Track close rate & average case value",
            "Add FCOO Advisor as AI scales operations",
            "Build a profit plan tied to pay raises",
            "Turn capacity gains into provable ROI",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$3,297", "$3,797/mo",
        "Dedicated CTO · Custom builds · Foundation Sprint",
        "1D4ED8",
    ),
    (
        "FULL SERVICE MARKETING — STARTER (SECONDARY)",
        "$4,997", "$5,697/mo",
        "GBP · Local SEO · Google Ads · Reporting",
        "6D28D9",
    ),
]
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $3,500–$8,300/mo — optional, if Marketing is added"
)

AVG_CASE_VALUE     = "$4,000"
CONSERVATIVE_LABEL = "Conservative  (4 cases/mo):"
CONSERVATIVE_RESULT = "$17,500 revenue · 5.0× ROAS"
AGGRESSIVE_LABEL   = "Aggressive  (14 cases/mo):"
AGGRESSIVE_RESULT  = "$57,200 revenue · 6.9× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Kick off Fractional CTO engagement"),
    ("Day 14",  "Complete AI Foundation Sprint mapping"),
    ("Week 2",  "Contact form moved above the fold"),
    ("Week 3",  "First automation build (intake/conflict-check) live"),
    ("Month 3", "Marketing Starter launched, if added"),
]

CLOSING_QUOTE = (
    '"We want to give our team more capacity — and pay them more — '
