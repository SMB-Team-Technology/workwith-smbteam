"""
Proposal PowerPoint — Florida Law Advisors
...
FIRM_NAME   = "Florida Law Advisors"
SALES_REP   = "Jacob Meissner"
OUTPUT_PATH = "florida-law-advisors/FloridaLawAdvisors_May26_2026_Proposal.pptx"
...
# Copied from section_05 sc-cards: (status, label, first finding trimmed to ~20 chars)
PILLARS = [
    ("RED",   "CRITICAL",  "No paid ads — any channel"),
    ("AMBER", "AMBER",     "Six-field form loses leads"),
    ("AMBER", "AMBER",     "No management layer"),
    ("AMBER", "AMBER",     "Revenue unconfirmed"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Copied from section_executive_summary top priorities (consequence-first)
FINDINGS = [
    ("neg", "Absent from paid search, LSA, and 3-pack across every Tampa practice area"),
    ("neg", "NAP inconsistency (Advisers/Advisors) suppresses local SEO for all 3 offices"),
    ("neg", "Six-field form and no live chat create abandonment before prospects contact"),
    ("pos", "4.9-star rating, 147 Trustpilot reviews, A+ BBB — strong trust foundation"),
]

# From section_06: named competitors with review data
COMPETITORS = [
    ("All Family Law Group, P.A.", "155 Google reviews", "3-pack + ads + LSA"),
    ("Gina Rosato Law Firm PA",    "798 WalletHub ratings", "Tampa bankruptcy dominant"),
    ("Maney | Gordon | Zeller PA", "40+ yrs, Board-Cert.", "Top immigration Tampa"),
]
CLIENT_REVIEWS      = "~100 Google reviews"
CLIENT_REVIEWS_NOTE = "← You are here"

# From research notes: Stage 4 → Stage 6
STAGE_TEXT = "Stage 4: Small Business Manager  →  Goal: Stage 6, Law Firm Owner"

# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Scale All 3 Offices"

SMB_MODEL_DESC = (
    "Lead Generation, Intake, Team, and Profit must all work together. "
    "When they do, your firm generates cases, converts them, and runs "
    "profitably — without requiring you to be in every room."
)

GOAL_HEADLINE = "Stage 4 → Stage 6 Law Firm Owner"
GOAL_DBM      = "Build the firm that runs without me"

PRIORITIES = [
    (
        "Build the", "Marketing Engine", "1D4ED8",
        [
            "Turn Google into a client pipeline that runs without you",
            "LSA: top-of-page leads, pay-per-call, starting now",
            "Spanish ads — lower CPL, less competition, high close",
            "Close review gap: 30+ new Google reviews in 60 days",
            "Fix NAP inconsistency — unlock 3-pack for all 3 offices",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Replace 6-field form with name + phone above the fold",
            "15-min response standard across all 3 offices",
            "Centralize lead routing — no inquiry falls through",
            "Turn eBook signups into a nurtured lead sequence",
            "Build intake that converts leads without your help",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Org chart + KPIs: 3 offices run without daily input",
            "Track profit by case type — know what actually pays",
            "Quarterly planning cadence replaces reactive managing",
            "Phase 2: Fractional COO added as case volume scales",
            "Build a firm with real value — not just a practice",
        ],
    ),
]

# ── Slide 3 ──────────────────────────────────────────────────────
# Copied exactly from section_11 investment grid
PACKAGES = [
    (
        "FULL SERVICE MARKETING STARTER",
        "$4,847", "$5,697/mo",
        "Google Ads · LSA · Meta · GBP · 3 locations",
        "1D4ED8",
    ),
    (
        "ELITE COACH PLUS",
        "$3,200", "$3,497/mo",
        "Weekly coaching · KPIs · Masterminds · Workshops",
        "6D28D9",
    ),
]
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $3,000–$5,000/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE      = "$3,000–$5,000"
CONSERVATIVE_LABEL  = "Conservative  (1–2 cases/mo):"
CONSERVATIVE_RESULT = "$3,500–$7,000 revenue · ~2× ROAS"
AGGRESSIVE_LABEL    = "Aggressive  (2–4 cases/mo):"
AGGRESSIVE_RESULT   = "$7,000–$14,000 revenue · ~3× ROAS"

# Condensed from section_11 Block 5 first-90-days (≤55 chars each)
TIMELINE = [
    ("Day 1",   "Google Ads + LSA campaigns submitted for launch"),
    ("Day 14",  "Website CTAs simplified, location pages live"),
    ("Week 2",  "GBP review requests sent to all 3 locations"),
    ("Week 3",  "Intake protocol deployed across all 3 offices"),
    ("Month 3", "30+ new reviews · 3-pack eligibility reviewed"),
]

# Client-facing DBM closing — vivid, specific, in the owner's voice
CLOSING_QUOTE = (
    '"Three offices. Five attorneys. The infrastructure is already built — '
    "now let's put the system in place that lets it generate revenue, "
    'convert clients, and grow without requiring you to run every piece of it."'
)
