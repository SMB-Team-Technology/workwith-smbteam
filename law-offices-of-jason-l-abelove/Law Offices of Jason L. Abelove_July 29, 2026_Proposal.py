...
# Report metadata
FIRM_NAME      = "Law Offices of Jason L. Abelove"
SALES_REP      = "Jonathan Farace"
OUTPUT_PATH    = "law-offices-of-jason-l-abelove/Law Offices of Jason L. Abelove_July 29, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "CRITICAL", "Referrals only, no system"),   # Lead Generation
    ("RED",   "CRITICAL", "You answer every call"),       # Intake
    ("RED",   "CRITICAL", "Just you + 1 attorney"),       # Team
    ("AMBER", "AMBER",    "Credit line maxed out"),       # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Criminal defense hire costs $13K/mo with zero digital presence"),
    ("neg", "Major criminal defense firm sits one floor away in your building"),
    ("neg", "No CRM — leads and follow-up are tracked on pen and paper"),
    ("pos", "$620K-$6M case results and a #3 ChatGPT ranking already earned"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Borrelli & Associates",   "39-49 reviews", "Employment law rival"),
    ("Barket Epstein et al.",   "9 attorneys",   "Same building · Crim. Def."),
    ("Schwartzapfel Lawyers",   "683 reviews",   "Review benchmark"),
]
CLIENT_REVIEWS      = "26 reviews"
...
SMB_MODEL_DESC = (
    ...
    "Missing any one means growth stalls no matter how much you spend on ads. "
    "When all four work, Jason can finally step back from answering every call himself."
)

GOAL_HEADLINE = "Personal practice → Sellable asset"
GOAL_DBM      = "Step back — and the firm still runs"
...
PRIORITIES = [
    (
        ...
        [
            "Launch a criminal defense page & ad campaign",
            "Grow reviews past 26 (rivals already have 39+)",
            "Consolidate duplicate Yelp listings",
            "Put your $620K-$6M case results to work",
            "Launch Meta retargeting & video ads",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Stand up a CRM — no more pen and paper",
            "Build a defined lead follow-up sequence",
            "Add after-hours coverage for missed calls",
            "Move the contact form above the fold",
            "Make call-answering a team process, not just you",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Build a case pipeline for your new hire",
            "Turn the $13K/mo cost into a profit center",
            "Build a practice-area-level profit plan",
            "Protect the margin your exit value depends on",
            "Start delegating what only you do today",
        ],
    ),
]
...
PACKAGES = [
    ...
    (
        "ELITE COACH PLUS",
        "$3,200", "$3,497/mo",
        "Group coaching · Masterminds · Profit planning",
        "6D28D9",
    ),
]
...
CONSERVATIVE_RESULT = "$16,000 revenue · 2.9x ROAS"
AGGRESSIVE_LABEL   = "Aggressive  (21 cases/mo):"
AGGRESSIVE_RESULT  = "$84,000 revenue · 3.8x ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Launch criminal defense Google Ads"),
    ("Day 14",  "Criminal defense page live on the site"),
    ("Week 2",  "Yelp listings consolidated, reviews grow"),
    ("Week 3",  "Elite Coach Plus coaching kickoff"),
    ("Month 3", "CRM and lead follow-up process live"),
]

CLOSING_QUOTE = (
    '"From a personal practice into a sellable asset — '
    'not just a bigger paycheck, but the freedom to finally step back."'
)
