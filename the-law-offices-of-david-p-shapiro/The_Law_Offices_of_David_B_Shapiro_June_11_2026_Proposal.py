...
# Report metadata
FIRM_NAME      = "The Law Offices of David B. Shapiro"
SALES_REP      = "Jacob Meissner"
OUTPUT_PATH    = "the-law-offices-of-david-p-shapiro/The_Law_Offices_of_David_B_Shapiro_June_11_2026_Proposal.pptx"
...
PILLARS = [
    ...
    ("RED", "CRITICAL", "2 AM calls, no coverage"),
    ("RED", "CRITICAL", "C or D team — no A-team"),
    ("RED", "CRITICAL", "$15K–$70K/mo swings"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "TLS cert error — every referred visitor sees a security warning before reading a word"),
    ("neg", "100% referral-only — invisible to every Baltimore prospect who searches PI or criminal defense"),
    ("neg", "2 AM calls handled personally — weekend arrests missed, 150-case backlog cannot advance"),
    ("pos", "37 years · Supreme Court argument · 84 Google reviews — deep earned Baltimore reputation"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Bob Katz Law",          "190+ reviews", "4.9 stars · Baltimore PI"),
    ("Seth Okin",             "212 reviews",  "5.0 stars · criminal defense"),
    ("The Snyder Law Group",  "101 reviews",  "4.8 stars · PI & accidents"),
]
CLIENT_REVIEWS      = "84 Google reviews"
CLIENT_REVIEWS_NOTE = "← You are here"

# Stage strip (right panel, Slide 1)
STAGE_TEXT = "Stage 3: Solo Practitioner  →  Goal: Law Firm Owner"
...
SMB_MODEL_DESC = (
    ...
    "a gap in any one stalls the others. "
    "When all four pillars are in place, David can step into the CEO role he described: "
    "the firm runs, the cases move, and Maine is actually on the calendar."
)

GOAL_HEADLINE = "~$510K → $1.2M revenue"
GOAL_DBM      = "Step away — Maine home, café, real time off"
...
PRIORITIES = [
    (
        ...
        [
            "Fix TLS cert — stop losing every referred visitor",
            "Activate LSA — pay per lead, not per click",
            "Activate the signed LegalMatch contract today",
            "Launch Google Ads — PI & criminal searches",
            "Grow from 84 to 150+ Google reviews",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "After-hours coverage for 2 AM criminal arrests",
            "Digital intake form captures every web visitor",
            "Speed-to-lead tracking on every inbound call",
            "Route weekend PI calls — never miss a case",
            "Convert more of the referrals already earned",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Build the A-team David described on the call",
            "Hire associate to advance the 150-case backlog",
            "Weekly KPI scorecards — know your numbers",
            "PI backlog tracker — $3.75M revenue roadmap",
            "Free David to be CEO — Maine home is the goal",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$4,847", "$5,697/mo",
        "Website · Google Ads · LSA · Meta Ads · GBP · SEO",
        "1D4ED8",
    ),
    (
        "ELITE COACH PLUS",
...
CONSERVATIVE_RESULT = "$22,000 revenue · 3.0× ROAS"
...
TIMELINE = [
    ("Day 1",   "Google Ads for PI & criminal defense go live"),
    ("Day 14",  "LSA with Google Screened verification launches"),
    ("Week 2",  "TLS cert fixed + website rebuild begins"),
    ("Week 3",  "LegalMatch activated + after-hours intake live"),
    ("Month 3", "Coaching kickoff + A-team accountability builds"),
]

CLOSING_QUOTE = (
    '"37 years of reputation, a full caseload, and a clear goal — '
    'build the A-team, get the calls covered, and finally spend a real week in Maine."'
)
