...
# Report metadata
FIRM_NAME      = "Runion Injury Law"
SALES_REP      = "Nick Holderman"
OUTPUT_PATH    = "runion-injury-law/Runion Injury Law_August 26, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "RED",   "SEO vendor underperforming"),
    ("AMBER", "AMBER", "No follow-up process known"),
    ("RED",   "RED",   "Owner is the only operator"),
    ("AMBER", "AMBER", "Revenue/margins unknown"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
# Review counts are unconfirmed aggregator data for this firm — qualitative framing only, no specific digits.
FINDINGS = [
    ("neg", "Core search terms surface competitors, not Runion"),
    ("neg", "Current SEO vendor spend is not producing results"),
    ("neg", "No confirmed follow-up process for website leads"),
    ("pos", "Genuine localized pages already live in two markets"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
# Review counts are UNCONFIRMED aggregator data for Runion and all 3 named
# competitors — qualitative framing used instead of specific digit counts.
COMPETITORS = [
    ("Phillips Law Group",  "Large volume",  "3 decades · citywide brand"),
    ("Zanes Law",           "Strong volume", "Dedicated pages per city"),
    ("Lamber Goodnow",      "Strong volume", "Top organic visibility"),
]
CLIENT_REVIEWS      = "Smaller base"
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reclaim Your Time"

SMB_MODEL_DESC = (
    "An operator running the day-to-day systems is what turns a plan into "
    "results. When someone else owns execution, Derick can finally step away and trust the firm keeps moving."
)

GOAL_HEADLINE = "Full-time operator → 2 weeks/mo remote"
...
PRIORITIES = [
    (
        ...
        [
            "Fix or replace the underperforming SEO vendor",
            "Turn Google into leads that don't need your time",
            "Turn on Local Service Ads for lower-risk leads",
            "Send paid traffic to geo-pages already built",
            "Repurpose existing video into paid social ads",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "69CD2B",
        [
            "Define who owns follow-up on every website lead",
            "Add after-hours coverage so leads don't go cold",
            "Build a same-day callback standard",
            "Convert leads you already paid to generate",
            "Track leads from form to signed case",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "003A59",
        [
            "Bring in an FCOO Director to run daily ops",
            "Execute the operational playbook already built",
            "Establish baseline revenue and margin visibility",
            "Build the leadership structure the firm lacks",
            "Turn 2 weeks a month remote into a real plan",
        ],
    ),
]
...
# marketing engagement — see section_11_workings.txt. There is one package
# (FCOO Director, standalone) and no ad spend/ROI to project. The "retail"
# strikethrough price is left blank (no bundle comparison applies), and the
# ad-spend/ROI card below is repurposed as an engagement snapshot — see the
# two labeled edits to the layout engine near build_slide3() for why.
#
# Package cards — (label, bundled_price, retail_price, services_line, accent_color_hex)
PACKAGES = [
    (
        "FCOO DIRECTOR — STANDALONE",
        "$5,797", "standalone",
        "Ops leadership · SOPs · hiring support · coaching · workshops",
        "003A59",
    ),
]

BUNDLE_TOTAL   = "$5,797 / mo"
BUNDLE_SAVINGS = "No bundle discount — standalone engagement"

AD_SPEND_NOTE = (
    "No ad spend in this proposal — scoped as an operational engagement, not marketing."
)

AVG_CASE_VALUE     = "Fractional COO — standalone"
CONSERVATIVE_LABEL = "Sprint structure:"
CONSERVATIVE_RESULT = "5-17 hrs/mo, front-loaded 3-4 months"
AGGRESSIVE_LABEL   = "Workshop cadence:"
AGGRESSIVE_RESULT  = "Quarterly virtual + 1 annual in-person"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Kickoff: full operational + vendor audit begins"),
    ("Day 14",  "Intake follow-up process live, zero ad spend"),
    ("Week 2",  "Baseline financials (revenue, close rate) confirmed"),
    ("Week 3",  "Operational playbook implementation begins"),
    ("Month 3", "Quarterly workshop checkpoint (Scottsdale, late Sept)"),
]

CLOSING_QUOTE = (
    '"Two weeks a month, fully remote — and the firm still runs without me."'
)
...
FONT = "Poppins"
LOGO_PATH = os.path.join(os.path.dirname(__file__), "..", "Design Files", "smb_team_logo.png")
...
def build_slide3(prs):
    ...
    # Relabeled for this deal: no ad spend is recommended (Fractional COO
    # engagement, not a marketing engagement) — see section_11_workings.txt.
    add_text(slide, "ENGAGEMENT AT A GLANCE",
             5.14, 1.18, 4.52, 0.24, 8, ROI_GREEN, bold=True)
    add_text(slide, "Engagement type:", 5.14, 1.46, 2.10, 0.28, 9, rgb("1E293B"), bold=True)
