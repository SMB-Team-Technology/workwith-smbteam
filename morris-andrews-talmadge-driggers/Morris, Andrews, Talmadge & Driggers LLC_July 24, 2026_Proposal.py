...
# Report metadata
FIRM_NAME      = "Morris, Andrews, Talmadge & Driggers LLC"
SALES_REP      = "Jacob Meissner"
OUTPUT_PATH    = "morris-andrews-talmadge-driggers/Morris, Andrews, Talmadge & Driggers LLC_July 24, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "RED",   "80% from referrals"),      # Lead Generation
    ("RED",   "RED",   "Vacant intake role"),       # Intake
    ("AMBER", "AMBER", "Four chiefs, no ops"),       # Team
    ("AMBER", "AMBER", "35% fixed overhead"),        # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "mcatlaw.com domain is broken — losing referral leads today"),
    ("neg", "Vacant intake role delays filed lawsuits and demands"),
    ("neg", "Dothan GBP unverified — invisible in local search"),
    ("pos", "25+ years of trusted reputation across two AL offices"),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("Stokes Stemle, LLC",           "119 reviews", "4.9★ · Dothan leader"),
    ("The Cochran Firm — Dothan",    "66 reviews",  "National brand"),
    ("Andy Citrin Injury Attorneys", "144 reviews", "4.7★ · Mobile leader"),
]
CLIENT_REVIEWS      = "3 reviews"
...
SMB_MODEL_DESC = (
    "All four pillars — lead gen, intake, team, and profit — must work "
    "together. When all four work, MCAT's partners can finally prove the "
    "ROI needed to scale and dominate the Black Belt market."
)

GOAL_HEADLINE = "$4-6M → $10M+ revenue"
GOAL_DBM      = "Prove it works, then dominate the region"
...
PRIORITIES = [
    (
        ...
        [
            "Launch tracked Google + Meta campaigns",
            "Verify and rebuild the Dothan GBP",
            "Fix the broken mcatlaw.com domain",
            "Clean up NAP across all directories",
            "Target high-value Black Belt venues",
        ],
    ),
    (
        "Fix Intake &", "Stop Losing Cases", "0F766E",
        [
            "Hire a dedicated intake specialist",
            "End the paralegal intake bottleneck",
            "Add after-hours call coverage",
            "Track lead-to-client conversion",
            "Free paralegals for lawsuits & demands",
        ],
    ),
    (
        ...
        [
            ...
            "Unify the four-partner practices",
            "Build a profit plan from new revenue",
            "Fund growth without cutting partner pay",
            "Turn four chiefs into one growth engine",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$15,997", "$18,997/mo",
        "Google+Meta ads · Local SEO · GBP · Website · Call tracking",
        "1D4ED8",
    ),
    (
        "FCOO ADVISOR",
        "$3,297", "$3,797/mo",
        "Dedicated fractional COO · Intake team build-out · Coaching",
        "6D28D9",
    ),
]
...
AD_SPEND_NOTE = (
    "+ Recommended ad spend: $17,500–$125,000/mo paid directly to Google/Meta"
)

AVG_CASE_VALUE     = "$7,500"
CONSERVATIVE_LABEL = "Conservative  (~5 cases/mo):"
CONSERVATIVE_RESULT = "$37,500 revenue · 2.1× ROAS"
AGGRESSIVE_LABEL   = "Aggressive  (~40 cases/mo):"
AGGRESSIVE_RESULT  = "$300,000 revenue · 2.4× ROAS"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Launch Google + Meta campaigns"),
    ("Day 14",  "Verify Dothan GBP, fix mcatlaw.com"),
    ("Week 2",  "Begin FCOO engagement"),
    ("Week 3",  "Start building dedicated intake team"),
    ("Month 3", "Full campaigns + intake team live"),
]

CLOSING_QUOTE = (
    '"We built this firm on trust for 25 years. Now we want the proof — '
    'and the systems — to dominate the region."'
)
