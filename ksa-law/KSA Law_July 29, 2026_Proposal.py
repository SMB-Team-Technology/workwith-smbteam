...
OUTPUT_PATH    = "ksa-law/KSA Law_July 29, 2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "RED",   "100% from referrals"),
    ("RED",   "RED",   "No proposal process"),
    ("AMBER", "AMBER", "Litigation capacity"),
    ("RED",   "RED",   "Self-funds expenses"),
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Cash flow strain forces Kelly to personally fund firm expenses monthly."),
    ("neg", "An unmeasured proposal close rate risks losing large VC-fund engagements."),
    ("neg", "The litigation team is already at capacity, months after launch."),
    ("pos", "Kelly grew KSA Law from solo to 7 people in 4 months on referrals alone."),
]

# Competitor table — list of (name, "XXX reviews", "brief note")
COMPETITORS = [
    ("MSB Law",       "Best Law Firms x3",     "Ranked in 3 practice areas"),
    ("Cook Ellis LLC", "4.6* (TrustAnalytica)", "15+ yrs, same client base"),
    ("Other KC firms", "not benchmarked",       "GBP data limited this session"),
]
...
# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Reach $5M ARR"

SMB_MODEL_DESC = (
    "SMB Team's model works across four pillars — Lead Generation, Intake, "
    "Team, and Profit — building the systems that let a firm scale without "
    "more hours from its owner. When Kelly's proposal process and financial "
    "visibility are in place, she can finally step back from personally "
    "funding KSA Law's growth."
)

GOAL_HEADLINE = "$570K → $5M ARR"
GOAL_DBM      = "Step away from personally funding the firm's growth"

# Each priority: (line1, line2, accent_color_hex, [5 bullet strings])
PRIORITIES = [
    (
        "Strengthen Your", "Digital Presence", "1D4ED8",
        [
            "Fix the bot-wall blocking ksagc.com",
            "Claim your Google Business Profile",
            "Build directory listings under KSA Law",
            "Turn referral traffic into booked calls",
            "Give VC-backed founders proof before they call",
        ],
    ),
    (
        "Fix the Proposal &", "Close More Deals", "0F766E",
        [
            "Set a baseline close rate on GC proposals",
            "Build a repeatable pitch framework",
            "Add a defined follow-up cadence",
            "Free Kelly from closing every deal alone",
            "Turn today's referrals into signed retainers",
        ],
    ),
    (
        "Install Team &", "Profit Systems", "6D28D9",
        [
            "Build real cash flow visibility with an FCFO",
            "Stop Kelly from self-funding expenses",
            "Relieve the litigation capacity bottleneck",
            "Build a leadership layer beneath Kelly",
            "Hit break-even, then profitability, on schedule",
        ],
    ),
]
...
PACKAGES = [
    (
        ...
        "$3,297", "$3,797/mo",
        "Cash flow plan · Profit plan · Group coaching included",
        "1D4ED8",
    ),
    (
        "ELITE COACH PLUS",
        "$3,200", "$3,497/mo",
        "Weekly 1:1 coaching · Proposal strategy · Accountability",
        "6D28D9",
    ),
]

BUNDLE_TOTAL   = "$6,497 / mo"
BUNDLE_SAVINGS = "Save $797/mo by bundling"

AD_SPEND_NOTE = (
    "Marketing was not part of this call — scoped separately once this foundation is in place"
)

AVG_CASE_VALUE      = "N/A — retainer/project model"
CONSERVATIVE_LABEL  = "Ad spend (this phase):"
CONSERVATIVE_RESULT = "Not part of this phase"
AGGRESSIVE_LABEL    = "Future phase (Roadmap 3/4):"
AGGRESSIVE_RESULT   = "Marketing scoped separately"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "Kick off Elite Coach Plus proposal working session"),
    ("Day 14",  "Begin FCFO Advisor cash flow & financials review"),
    ("Week 2",  "Map litigation team capacity plan"),
    ("Week 3",  "Confirm ARR, website & GBP open research items"),
    ("Month 3", "Identify first internal leader beneath Kelly"),
]

CLOSING_QUOTE = (
    '"Break-even by October. Profitability by year-end. And a firm built '
    'to hit $5M — without me covering the gap myself."'
)
