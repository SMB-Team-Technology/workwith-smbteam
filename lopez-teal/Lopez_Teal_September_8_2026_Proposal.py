...
NOTE ON THIS FIRM'S DECK: Lopez Teal's package decision (per Nick's Phase 1
lock-in, commit e754036) is FCOO Advisor, stand-alone — no marketing package,
no ad spend. The template's Slide 3 ROI card and Slide 1 review-count column
assume a marketing/ad-spend engagement, which does not apply here. Rather than
fabricate ad-spend ROI figures or unconfirmed review counts, the two hardcoded
labels inside the ROI card (only) were relabeled in this firm-specific copy to
present the real, transcript-grounded "sale multiple" opportunity instead.
Design Files/audit_pptx_template.py itself was NOT modified.
"""
...
# Report metadata
FIRM_NAME      = "Lopez Teal Real Estate Law & Title"
SALES_REP      = "Nick Holderman"
OUTPUT_PATH    = "lopez-teal/Lopez_Teal_September_8_2026_Proposal.pptx"
...
PILLARS = [
    ("RED",   "RED",   "100% referral-based"),   # Lead Generation
    ("AMBER", "AMBER", "No CRM/delegation"),       # Intake
    ("RED",   "RED",   "Owner is sole rainmaker"), # Team
    ("AMBER", "AMBER", "No firm-wide profit plan"), # Profit Plan
]
PILLAR_NAMES = ["Lead Generation", "Intake", "Team", "Profit Plan"]

# Key findings — list of ("neg"|"pos", "one-sentence finding")
FINDINGS = [
    ("neg", "Annette is the sole rainmaker, working 50-60+ hours a week"),
    ("neg", "100% of new business runs through personal referrals"),
    ("neg", "Directories still list the firm under its prior name/address"),
    ("pos", "Already tracks file profitability: $15k luxury vs. $2k standard"),
]

# Competitor table — list of (name, review-count-string, note)
# Review counts for Lopez Teal and named competitors are UNCONFIRMED per
# research notes (no live Google Maps access this session) — no specific
# count is printed as fact anywhere in this deck.
COMPETITORS = [
    ("Strang Tryson, PLLC",       "Unconfirmed", "Has Coconut Grove SEO page"),
    ("Jacqueline A. Salcines, PA", "Unconfirmed", "26-yr Coral Gables brand"),
    ("Almazan Law",                "Unconfirmed", "Same luxury RE niche"),
]
CLIENT_REVIEWS      = "Unconfirmed"
CLIENT_REVIEWS_NOTE = "← Verify live before call"

# Stage strip (right panel, Slide 1)
STAGE_TEXT = "Stage 3: Solo Practitioner  →  Goal: Stage 6, Law Firm Owner"

# ── Slide 2 ──────────────────────────────────────────────────────
SLIDE_2_TITLE = "Your Growth Plan: 3 Priorities to Build a Sellable Firm"

SMB_MODEL_DESC = (
    "All four pillars must work together. Missing any one means the firm "
    "stalls no matter how strong the reputation is. When delegation and a "
    "profit plan are in place, Annette can finally step back and trust the "
    "firm still runs — and still sells."
)

GOAL_HEADLINE = "2-3x  →  4-5x sale multiple"
GOAL_DBM      = "Step back and still have a firm worth selling"

# Each priority: (line1, line2, accent_color_hex, [5 bullet strings])
PRIORITIES = [
    (
        "Reduce Owner", "Dependence", "0091C9",
        [
            "Operational assessment of referral & file handoffs",
            "Build first delegation plan off Annette's plate",
            "Stand up referral tracking — no lead lost",
            "Join weekly group coaching sessions",
            "Join practice area masterminds",
        ],
    ),
    (
        "Build a Firm-Wide", "Profit Plan", "69CD2B",
        [
            "Turn file-level insight into firm-wide margin view",
            "Track progress toward the 4-5x sale multiple",
            "Attach a real number to 'sellable'",
            "Structure numbers for buyer due diligence",
            "Review profit plan against goals quarterly",
        ],
    ),
    (
        "Prepare For", "Diversified Growth", "003A59",
        [
            "Evaluate institutional/lender client relationships",
            "Clean up directory listings under the current brand",
            "Scope geo-targeted content for Coral Gables niche",
            "Revisit sale-readiness against Phase 1-2 milestones",
            "Introduce owned lead gen once dependence is fixed",
        ],
    ),
]
...
# Only one package is being sold this round (FCOO Advisor, stand-alone —
# Nick's locked decision, no marketing package attached). Retail price is
# left blank rather than showing a false "was/now" discount, since nothing
# is being bundled.
PACKAGES = [
    (
        "FCOO ADVISOR — STAND-ALONE",
        "$3,797", "",
        "Fractional COO advisory · weekly group coaching · masterminds",
        "003A59",
    ),
]

BUNDLE_TOTAL   = "$3,797 / mo"
BUNDLE_SAVINGS = "No marketing package included this round"

AD_SPEND_NOTE = (
    "No paid ad spend included — marketing is not part of this engagement"
)

# ROI card relabeled for this firm: no marketing/ad-spend package is being
# sold, so "return on ad spend" does not apply. These two variables (plus
# the CONSERVATIVE/AGGRESSIVE label variables below) reuse the same visual
# slots to show the real, transcript-grounded opportunity instead —
# reducing referral-only fragility to reach a higher sale multiple.
AVG_CASE_VALUE     = "2-3x revenue (referral-only, today)"
CONSERVATIVE_LABEL = "Target sale multiple:"
CONSERVATIVE_RESULT = "4-5x revenue (diversified)"
AGGRESSIVE_LABEL   = "This engagement:"
AGGRESSIVE_RESULT  = "$3,797/mo · ~3.5% of revenue"

# Timeline — 5 items: (milestone_label, action_text)
TIMELINE = [
    ("Day 1",   "FCOO Advisor kickoff & ops assessment"),
    ("Day 14",  "First delegation plan takes shape"),
    ("Week 2",  "Referral tracking process stood up"),
    ("Week 3",  "Firm-wide profit plan structure begins"),
    ("Month 3", "Group coaching + sale-multiple tracking live"),
]

CLOSING_QUOTE = (
    '"A firm someone else could run — and I could finally sell."'
)

# ═══════════════════════════════════════════════════════════════════
# LAYOUT ENGINE — DO NOT MODIFY BELOW THIS LINE
# (Two exceptions, both marked inline: the ROI-card heading and the
# "Average case value:" label were relabeled for this firm's single,
# non-marketing package — see note at top of file. No other layout,
# positioning, sizing, or styling code was changed.)
# ═══════════════════════════════════════════════════════════════════
...
def build_slide3(prs):
    ...
    # ROI card — relabeled for this firm (see note at top of file): no
    # marketing/ad-spend package is being sold this round, so the heading
    # and "Average case value:" label were changed to present the real
    # sale-multiple opportunity instead of fabricating ad-spend ROI.
    add_rect(slide, 4.96, 1.12, 4.82, 1.44, fill=ROI_BG)
    add_text(slide, "WHY FCOO ADVISOR, WHY NOW",
             5.14, 1.18, 4.52, 0.24, 8, ROI_GREEN, bold=True)
    add_text(slide, "Current sale multiple:", 5.14, 1.46, 2.10, 0.28, 9, rgb("1E293B"), bold=True)
